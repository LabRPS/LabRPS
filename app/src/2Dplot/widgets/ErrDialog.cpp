...
void ErrDialog::errorColumnNames() {
  ...
  if (xErrBox->isChecked()) {
    error_columns_ = app_->columnList(LabRPS::xErr);
  } else {
    error_columns_ = app_->columnList(LabRPS::yErr);
  }
  ...
  if (columnBox->isChecked()) {
      ...
      if (selectplotdata.table == graphdata->gettable() &&
        ...
        if (xErrBox->isChecked()) {
          if (selectplotdata.xcol->dataType() !=
              LabRPS::ColumnDataType::TypeDouble) {
    ...
    foreach (LineSpecial2D *ls, lslist) {
      ...
      if (selectplotdata.table == graphdata->gettable() &&
        ...
        Column *errors = new Column("1", LabRPS::Numeric);
        (xErrBox->isChecked()) ? errors->setPlotDesignation(LabRPS::xErr)
                               : errors->setPlotDesignation(LabRPS::yErr);
...
Column *ErrDialog::getErrorCol(Table *parent) {
  ...
  if (errors->dataType() != LabRPS::ColumnDataType::TypeDouble) {
    QMessageBox::critical(app_, tr("Error"),
