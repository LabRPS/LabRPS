...
Axis2D *AxisRect2D::addAxis2DifNeeded(Column *col) {
  ...
  switch (col->dataType()) {
    case LabRPS::ColumnDataType::TypeDouble:
      tickertype = Axis2D::TickerType::Value;
      break;
    case LabRPS::ColumnDataType::TypeString:
      tickertype = Axis2D::TickerType::Text;
      break;
    case LabRPS::ColumnDataType::TypeDateTime: {
      tickertype = Axis2D::TickerType::DateTime;
    } break;
    case LabRPS::ColumnDataType::TypeMonth:
    case LabRPS::ColumnDataType::TypeDay:
      tickertype = Axis2D::TickerType::DateTime;
      break;
  }

  if (col->plotDesignation() == LabRPS::X) {
  ...
  } else if (col->plotDesignation() == LabRPS::Y) {
...
Vector2D *AxisRect2D::addVectorPlot(const Vector2D::VectorPlot &vectorplot,
  ...
  if (!hasMinimumDataPointsToPlot(
          ...
          from, to))
    return nullptr;
  if (y1Data->dataType() != LabRPS::ColumnDataType::TypeDouble ||
      x2Data->dataType() != LabRPS::ColumnDataType::TypeDouble ||
      y2Data->dataType() != LabRPS::ColumnDataType::TypeDouble)
    return nullptr;
...
StatBox2D *AxisRect2D::addStatBox2DPlot(Table *table, Column *ycol,
  ...
  if (ycol->dataType() != LabRPS::ColumnDataType::TypeDouble) {
...
Bar2D *AxisRect2D::addHistogram2DPlot(const AxisRect2D::BarType &type,
                                      ...
                                      const int from, const int to,
                                      Axis2D *xAxis, Axis2D *yAxis) {
  if (yData->dataType() != LabRPS::ColumnDataType::TypeDouble) {
    QMessageBox::warning(nullptr, tr("Column datatype not allowed!"),
                         tr("selected columns %1 must be of datatype Numeric!")
                             .arg(yData->name()));
    return nullptr;
...
Pie2D *AxisRect2D::addPie2DPlot(const Graph2DCommon::PieStyle &style,
  ...
  if (xData->dataType() != LabRPS::ColumnDataType::TypeString ||
      yData->dataType() != LabRPS::ColumnDataType::TypeDouble) {
    QMessageBox::warning(
...
bool AxisRect2D::axisColumTypeCompatibilityCheck(Axis2D *axis, Column *col,
  ...
  if ((getXAxes2D().contains(axis) &&
       col->plotDesignation() == LabRPS::PlotDesignation::X) ||
      (getYAxes2D().contains(axis) &&
       col->plotDesignation() == LabRPS::PlotDesignation::Y)) {
    switch (axis->gettickertype_axis()) {
      case Axis2D::TickerType::Text:
        if (col->dataType() == LabRPS::ColumnDataType::TypeString &&
      ...
      case Axis2D::TickerType::Log:
        if (col->dataType() == LabRPS::ColumnDataType::TypeDouble)
          return true;
        break;
      case Axis2D::TickerType::DateTime:
      case Axis2D::TickerType::Time:
        if (col->dataType() == LabRPS::ColumnDataType::TypeDateTime ||
            col->dataType() == LabRPS::ColumnDataType::TypeDay ||
            col->dataType() == LabRPS::ColumnDataType::TypeMonth)
          return true;
        break;
    }
