...
void Table::setPlotDesignation(LabRPS::PlotDesignation pd) {
...
void Table::setColumnTypes(const QStringList &ctl) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    switch (l[0].toInt()) {
      ...
      // 3, Month = 4, Day = 5};
      case 0:
        column(i)->setColumnMode(LabRPS::Numeric);
        break;
      case 1:
        column(i)->setColumnMode(LabRPS::Text);
        break;
      case 2:
      case 3:
      case 6:
        column(i)->setColumnMode(LabRPS::DateTime);
        break;
      case 4:
        column(i)->setColumnMode(LabRPS::Month);
        break;
      case 5:
        column(i)->setColumnMode(LabRPS::Day);
        break;
    }
  }
}
...
int Table::firstXCol() {
  for (int j = 0; j < numCols(); j++) {
    if (column(j)->plotDesignation() == LabRPS::X) return j;
  }
  return -1;
}
...
QStringList Table::YColumns() {
  ...
  QStringList names;
  for (int i = 0; i < numCols(); i++) {
    if (column(i)->plotDesignation() == LabRPS::Y)
...
QStringList Table::selectedYColumns() {
  ...
  QStringList names;
  for (int i = 0; i < numCols(); i++) {
    if (isColumnSelected(i) && column(i)->plotDesignation() == LabRPS::Y)
      names << name() + "_" + column(i)->name();
  }
  return names;
}
...
QStringList Table::selectedZColumns() {
  QStringList names;
  for (int i = 0; i < numCols(); i++) {
    if (isColumnSelected(i) && column(i)->plotDesignation() == LabRPS::Z)
      names << name() + "_" + column(i)->name();
  }
  return names;
}
...
QStringList Table::selectedErrColumns() {
  ...
  for (int i = 0; i < numCols(); i++) {
    if (isColumnSelected(i) &&
        (column(i)->plotDesignation() == LabRPS::xErr ||
         column(i)->plotDesignation() == LabRPS::yErr))
      names << name() + "_" + column(i)->name();
  }
  return names;
}
...
void Table::insertCols(int start, int count) {
  ...
  for (int i = 0; i < count; i++)
    cols << new Column(QString::number(i + 1), LabRPS::Numeric);
...
void Table::addCol(LabRPS::PlotDesignation pd) {
  d_future_table->addColumn();
  column(d_future_table->columnCount() - 1)
      ->setColumnMode(
          LabRPS::Numeric);  // in case we ever change the default
...
double Table::cell(int row, int col) {
  ...
  if (!colPtr->isInvalid(row)) {
    if (colPtr->columnMode() == LabRPS::Text) {
...
bool Table::noXColumn() {
  return d_future_table->columnCount(LabRPS::X) == 0;
}

bool Table::noYColumn() {
  return d_future_table->columnCount(LabRPS::Y) == 0;
}
...
void Table::closeEvent(QCloseEvent *e) {
  if (askOnClose) {
    switch (QMessageBox::information(this, tr("LabRPS"),
...
LabRPS::PlotDesignation Table::colPlotDesignation(int col) {
  return column(col)->plotDesignation();
}

void Table::setColPlotDesignation(int col, LabRPS::PlotDesignation d) {
...
LabRPS::ColumnMode Table::columnType(int col) {
...
void Table::setColumnType(int col, LabRPS::ColumnMode mode) {
...
QString Table::columnFormat(int col) {
  // TODO: obsolete, remove in 0.3.0
  Column *col_ptr = column(col);
  if (col_ptr->columnMode() != LabRPS::DateTime &&
      col_ptr->columnMode() != LabRPS::Month &&
      col_ptr->columnMode() != LabRPS::Day)
    return QString();
...
void Table::importASCII(const QString &fname, const QString &sep,
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    for (int i = overwritten_cols; i < temp->columnCount(); i++) {
      ...
      Column *new_col = new Column(temp->column(i)->name(), LabRPS::Numeric);
