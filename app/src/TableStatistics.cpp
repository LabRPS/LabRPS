...
TableStatistics::TableStatistics(ScriptingEnv *env, QWidget *parent,
  ...
  if (d_type == TableStatistics::StatRow) {
    ...
    for (int i = 0; i < 10; i++) setColumnType(i, LabRPS::Numeric);
    ...
    for (int i = 0; i < 2; i++) setColumnType(i, LabRPS::Text);

    for (int i = 2; i < 12; i++) setColumnType(i, LabRPS::Numeric);
  ...
  setColPlotDesignation(0, LabRPS::X);
}
  ...
  if (d_type == TableStatistics::StatRow) {
      ...
      for (int destRow = 0; destRow < d_targets.size(); destRow++) {
        ...
        for (int col = 0; col < columns; col++) {
          if (d_base->column(col)->rowCount() > srcRow &&
              d_base->column(col)->columnMode() == LabRPS::Numeric &&
    ...
    for (int destRow = 0; destRow < d_targets.size(); destRow++) {
      if (colName == QString(d_base->name()) + "_" +
        ...
        if (col->columnMode() != LabRPS::Numeric) return;
