...
RPSScene::RPSScene(ScriptingEnv* env, const QString& label, QWidget* parent,
           const char* name, Qt::WindowFlags f)
    : MyWidget(label, parent, name, f),
      ObjectBrowserTreeItem(QVariant::fromValue<RPSScene*>(this),
...
RPSScene::~RPSScene() { delete dummywindow_; }

QString RPSScene::getItemName() { return name(); }

QIcon RPSScene::getItemIcon() {
  return IconLoader::load("edit-note", IconLoader::LightDark);
}

QString RPSScene::getItemTooltip() { return name(); }

void RPSScene::init(ScriptingEnv* env) {
  autoExec = false;
  QDateTime dt = QDateTime::currentDateTime();
  setBirthDate(dt.toString(Qt::LocalDate));

  sceneView_ = new SceneView(this);
  setWidget(sceneView_);

  setGeometry(0, 0, 500, 200);
 // connect(view_, &ScriptEdit::textChanged, this, &RPSScene::modifiedView);
}

//QString RPSScene::getText() { return textedit_->toPlainText().trimmed(); }

void RPSScene::modifiedView() { emit modifiedWindow(this); }

void RPSScene::save(QXmlStreamWriter* xmlwriter) {
...
bool RPSScene::load(XmlStreamReader* xmlreader) {
...
void RPSScene::setAutoexec(bool exec) { autoExec = exec; }
