...
ApplicationWindow::ApplicationWindow()
  ...
  comboxbox_simu_toolbarbtn_->addItems(supportedRandomPhenomena);
...
void ApplicationWindow::change3DData() {
  ...
  ad->setWindowTitle(tr("Choose data set"));
  // TODO: string list -> Column * list
  ad->setCurveNames(columnsList(LabRPS::Z));
  ad->exec();
}
...
Layout3D *ApplicationWindow::dataPlot3D(Table *table,
  ...
  foreach (Column *col, selectedcols) {
    if (col->plotDesignation() == LabRPS::PlotDesignation::X)
      ...
      ycol = col;
    else if (col->plotDesignation() == LabRPS::PlotDesignation::Z)
      zcol = col;
  }
...
void ApplicationWindow::openTemplate() {
  ...
  filter += tr("LabRPS Matrix Template") + " (*.amt);;";
...
bool ApplicationWindow::saveProject() {
  ...
  setWindowTitle("LabRPS - " + projectname);
  savedProject();
  ui_->actionUndo->setEnabled(false);
  ui_->actionRedo->setEnabled(false);
...
void ApplicationWindow::saveAsTemplate() {
  ...
  if (isActiveSubWindow(mywidget, SubWindowType::MatrixSubWindow))
    ...
    filter = tr("LabRPS2D Graph Template") + " (*.apt);;";
  ...
  foreach (str, args) {
    if (str == "-v" || str == "--version") {
      QString s = LabRPS::versionString() + LabRPS::extraVersion() +
                  ...
                  tr("Original author") + ": " +
                  LabRPS::originalAuthorWithMail() + "\n";

#ifdef Q_OS_WIN
      hide();
      QMessageBox::information(this, tr("LabRPS") + " - " + tr("Version"),
                               s);
#else
      ...
      s += "-a " + tr("or") + " --about: " + tr("about LabRPS application") +
           "\n";
      s += "-h " + tr("or") + " --help: " + tr("show command line options") +
           "\n";
      s += "-l=XX " + tr("or") +
           " --lang=XX: " + tr("start LabRPS in language") +
           " XX ('en', 'fr', 'de', ...)\n";
      s += "-v " + tr("or") +
...
#ifdef Q_OS_WIN
      hide();
      QMessageBox::information(this, tr("LabRPS - Help"), s);
#else
      std::cout << s.toStdString();
#endif
      exit(0);
      ...
      QString abt =
          tr("LabRPS is a free cross-platform program for "
             ...
             "random sea surface, seismic ground motion,....") +
          "\n" +
          tr("LabRPS uses code from AlphaPlot 1.02, which is the fork of "
             "of SciDAVis 1.D9, which also inturn is the fork of "
             "QtiPlot 0.9-rc.") +
          "\n";
#ifdef Q_OS_WIN
      hide();
      QMessageBox::information(this, tr("LabRPS - about"), abt);
#else
    ...
    if (!appTranslator->load("labrps_" + appLanguage, qmPath))
      if (!appTranslator->load("labrps_" + appLanguage))
        if (!appTranslator->load("labrps_" + lng, qmPath))
          if (!appTranslator->load("labrps_" + lng))
            if (!appTranslator->load("labrps_" + slng, qmPath))
              appTranslator->load("labrps_" + slng);
...
void ApplicationWindow::folderProperties() {
  ...
  if (!current_folder->parent()) {
    ...
    properties.description = tr("This is an LabRPS project");
    ...
    properties.description = tr("This is an LabRPS Folder");
  }
  ...
  if (isActiveSubWindow(window, SubWindowType::MatrixSubWindow)) {
    ...
    properties.content =
        QString(tr("%1x%2 Resolution,\n%3 Grids"))
            ...
            .arg(static_cast<Layout3D *>(window)->size().width())
            .arg(false);
    properties.description = tr("This is an LabRPS 3D Graph");
  }
...
void ApplicationWindow::searchForUpdates() {
  int choice = QMessageBox::question(
      ...
      tr("LabRPS will now try to determine whether a new version of "
         "LabRPS is available. Please modify your firewall settings in "
         "order to allow LabRPS to connect to the internet.") +
