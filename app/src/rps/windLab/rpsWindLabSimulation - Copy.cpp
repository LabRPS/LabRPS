...
void RPSWindLabSimulation::spectrumWindOutput()
{
	...
	connect(simulationOutputWorker, SIGNAL(spectrumXModelGraph(mat)), this, SLOT(spectrumXModelOut(mat)), Qt::DirectConnection);
...
void RPSWindLabSimulation::CoherenceFunctionOutput()
{
		...
		GetWindLabData().timeIndex <= GetWindLabData().numberOfTimeIncrements)
	{
		...
		QString arrayName = tr("Coherence (%1, %2, %3, %4)").arg(GetWindLabData().locationJ).arg(GetWindLabData().locationK).arg(GetWindLabData().frequencyIndex).arg(GetWindLabData().timeIndex);

		// allocate memory for the table
		Table *table = app->newTable(arrayName, GetWindLabData().numberOfFrequency, 2);

		//   app->logInfo = (QString::number(GetWindLabSimulationOutputWorker()->dFrequencyVector(0)));
		//   app->showResults(true);

		// fill the table with computed coherence
		for (int i = 0; i < GetWindLabData().numberOfFrequency; i++)
		{
			table->setCellValue(i, 0, GetWindLabSimulationOutputWorker()->dFrequencyVector(i));
...
void RPSWindLabSimulation::spatialPositionOut()
{
	...
	// prepare the name of the table
	QString arrayName = tr("Locations (%1, %2, %3, %4)").arg(GetWindLabData().locationJ).arg(GetWindLabData().locationK).arg(GetWindLabData().frequencyIndex).arg(GetWindLabData().timeIndex);
...
void RPSWindLabSimulation::meanWindVelocityOut()
{
		...
		timeIndex <= GetWindLabData().numberOfTimeIncrements)
	{
		...
		// prepare the name of the table
		QString arrayName = tr("Mean (%1, %2, %3, %4)").arg(GetWindLabData().locationJ).arg(GetWindLabData().locationK).arg(GetWindLabData().frequencyIndex).arg(GetWindLabData().timeIndex);
...
void RPSWindLabSimulation::frequencyDistributionOut()
{
		...
		frequencyIndex == GetWindLabData().numberOfFrequency + 1)
	{
		...
		{
			table->setCellValue(i, 0, i + 1);
			table->setCellValue(i, 1, GetWindLabSimulationOutputWorker()->dFrequencyVector(i));
		}
...
void RPSWindLabSimulation::spectrumXModelOut(mat psdMatrix)
{
	RPSSimulation *rpsSimulator = (RPSSimulation *)this->parent();
	ApplicationWindow *app = (ApplicationWindow *)rpsSimulator->parent();
	...
	)
	{
		...
		// prepare the name of the table
		QString arrayName = tr("SpectrumX (%1, %2, %3, %4)").arg(GetWindLabData().locationJ).arg(GetWindLabData().locationK).arg(GetWindLabData().frequencyIndex).arg(GetWindLabData().timeIndex);
		...
		{
			table->setCellValue(i, 0, GetWindLabSimulationOutputWorker()->dFrequencyVector(i));
			table->setCellValue(i, 1, GetWindLabSimulationOutputWorker()->dPSDVector(i));
		}
	...
	)
	{
		...
		QMessageBox::warning(this, "hum",QString::number(psdMatrix.rows()));
		// fill the matrix with computed coherence
		for (int i = 0; i < GetWindLabData().numberOfSpatialPosition; i++)
		{
			for (int j = 0; j < GetWindLabData().numberOfSpatialPosition; j++)
			{
				//matrix->setText(i, j, QString::number(psdMatrix(i, j)));
			}
		}
	...
	)
	{
		...
		for (int i = 0; i < GetWindLabData().numberOfTimeIncrements; i++)
		{
			...
			for (int j = 0; j < GetWindLabData().numberOfSpatialPosition; j++)
			{
				table->setCellValue(i, j+1, GetWindLabSimulationOutputWorker()->dPSDMatrix(i,j));
			}
		}
	...
	//emit simulationOutputWorker->finished();
