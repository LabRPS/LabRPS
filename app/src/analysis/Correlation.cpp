...
void Correlation::setDataFromTable(Table *t, const QString &colName1,
  ...
  if (col1 < 0) {
    ...
    d_init_err = true;
    return;
  } else if (d_table->columnType(col1) != LabRPS::ColumnMode::Numeric ||
             d_table->columnType(col2) != LabRPS::ColumnMode::Numeric) {
    QString description;
    (col1 == col2)
    ...
    QMessageBox::warning(qobject_cast<ApplicationWindow *>(parent()),
                         tr("LabRPS") + " - " + tr("Error"), description);
    d_init_err = true;
    return;
  }
...
void Correlation::addResultCurve() {
  ...
  d_table->setColPlotDesignation(cols, LabRPS::X);
