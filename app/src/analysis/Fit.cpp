...
Table *Fit::parametersTable(const QString &tableName) {
  ...
  t->column(0)->setColumnMode(LabRPS::Text);
  t->column(1)->setColumnMode(LabRPS::Numeric);
  t->column(2)->setColumnMode(LabRPS::Numeric);
  for (int i = 0; i < d_p; i++) {
  ...
  t->column(2)->setPlotDesignation(LabRPS::yErr);
