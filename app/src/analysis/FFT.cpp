...
QList<Column *> FFT::fftCurve() {
  ...
  if (!d_inverse) {
    ...
    columns << new Column(tr("Frequency"), LabRPS::Numeric);
    ...
    columns << new Column(tr("Time"), LabRPS::Numeric);
  ...
  columns << new Column(tr("Real"), LabRPS::Numeric);
  columns << new Column(tr("Imaginary"), LabRPS::Numeric);
  columns << new Column(tr("Amplitude"), LabRPS::Numeric);
  columns << new Column(tr("Angle"), LabRPS::Numeric);
  for (i = 0; i < d_n; i++) {
    i2 = 2 * i;
  ...
  delete[] amp;
  delete[] result;
  columns.at(0)->setPlotDesignation(LabRPS::X);
  columns.at(1)->setPlotDesignation(LabRPS::Y);
  columns.at(2)->setPlotDesignation(LabRPS::Y);
  columns.at(3)->setPlotDesignation(LabRPS::Y);
  columns.at(4)->setPlotDesignation(LabRPS::Y);
  return columns;
}
...
void FFT::setDataFromTable(Table *t, const QString &realColName,
  ...
  if (d_imag_col < 0) {
    if (d_table->columnType(d_real_col) != LabRPS::ColumnMode::Numeric) {
    ...
    if (d_table->columnType(d_real_col) != LabRPS::ColumnMode::Numeric ||
        d_table->columnType(d_imag_col) != LabRPS::ColumnMode::Numeric) {
      QMessageBox::warning(qobject_cast<ApplicationWindow *>(parent()),
                           tr("LabRPS") + " - " + tr("Error"),
