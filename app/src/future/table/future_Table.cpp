...
#endif
{
  ...
  for (int i = 0; i < columns; i++) {
    Column *new_col = new Column(QString::number(i + 1), LabRPS::Numeric);
    new_col->setPlotDesignation(i == 0 ? LabRPS::X : LabRPS::Y);
    cols << new_col;
  }
  appendColumns(cols);
  setRowCount(rows);
...
int Table::columnCount(LabRPS::PlotDesignation pd) const {
  int count = 0;
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      ...
      if (d_view->isCellSelected(first_row + r, first_col + c)) {
        ...
        } else if (col_ptr->dataType() == LabRPS::TypeDouble) {
    ...
    // selection will be expanded to the needed size from the current cell
    {
      ...
      if (last_col >= columnCount()) {
        ...
        for (int i = 0; i < last_col + 1 - columnCount(); i++) {
          Column *new_col = new Column(QString::number(i + 1), LabRPS::Text);
          new_col->setPlotDesignation(LabRPS::Y);
          cols << new_col;
        }
        appendColumns(cols);
...
void Table::fillSelectedCellsWithCustomRandomNumbers() {
  ...
  foreach (Column *col_ptr, d_view->selectedColumns()) {
    if (col_ptr->columnMode() != LabRPS::Numeric) {
...
void Table::setSelectionAs(LabRPS::PlotDesignation pd) {
  if (!d_view) return;
  WAIT_CURSOR;
...
void Table::setSelectedColumnsAsX() { setSelectionAs(LabRPS::X); }

void Table::setSelectedColumnsAsY() { setSelectionAs(LabRPS::Y); }

void Table::setSelectedColumnsAsZ() { setSelectionAs(LabRPS::Z); }

void Table::setSelectedColumnsAsYError() { setSelectionAs(LabRPS::yErr); }

void Table::setSelectedColumnsAsXError() { setSelectionAs(LabRPS::xErr); }

void Table::setSelectedColumnsAsNone() {
  setSelectionAs(LabRPS::noDesignation);
}
...
void Table::normalizeColumns(QList<Column *> cols) {
  ...
  foreach (Column *col, cols) {
    if (col->dataType() == LabRPS::TypeDouble) {
      double max = 0.0;
...
void Table::normalizeSelection() {
  ...
  foreach (Column *col_ptr, d_view->selectedColumns()) {
    int col = columnIndex(col_ptr);
    if (col_ptr->dataType() == LabRPS::TypeDouble)
...
int Table::colX(int col) {
  for (int i = col - 1; i >= 0; i--) {
    if (column(i)->plotDesignation() == LabRPS::X) return i;
  }
  int cols = columnCount();
...
int Table::colY(int col) {
  ...
  if (column(col)->plotDesignation() == LabRPS::xErr ||
      column(col)->plotDesignation() == LabRPS::yErr) {
    // look to the left first
    for (int i = col - 1; i >= 0; i--) {
      if (column(i)->plotDesignation() == LabRPS::Y) return i;
    }
    for (int i = col + 1; i < cols; i++) {
      if (column(i)->plotDesignation() == LabRPS::Y) return i;
    }
...
void Table::sortColumns(Column *leading, QList<Column *> cols, bool ascending) {
  ...
  if (leading == nullptr)  // sort separately
  {
    for (int i = 0; i < cols.size(); i++) {
      ...
      if (col->dataType() == LabRPS::TypeDouble) {
        ...
        delete temp_col;
      } else if (col->dataType() == LabRPS::TypeString) {
        int rows = col->rowCount();
        ...
        col->copy(temp_col, 0, 0, rows);
        delete temp_col;
      } else if (col->dataType() == LabRPS::TypeDateTime) {
        int rows = col->rowCount();
  ...
  {
    if (leading->dataType() == LabRPS::TypeDouble) {
      QList<QPair<double, int> > map;
    ...
    } else if (leading->dataType() == LabRPS::TypeString) {
      QList<QPair<QString, int> > map;
      int rows = leading->rowCount();
    ...
    } else if (leading->dataType() == LabRPS::TypeDateTime) {
      QList<QPair<QDateTime, int> > map;
      int rows = leading->rowCount();
...
int Table::Private::numColsWithPD(LabRPS::PlotDesignation pd) {
  int count = 0;
...
void Table::Private::updateHorizontalHeader(int start_col, int end_col) {
  ...
  if (numColsWithPD(LabRPS::X) > 1) {
    int x_cols = 0;
    for (int i = 0; i < d_column_count; i++) {
      if (d_columns.at(i)->plotDesignation() == LabRPS::X)
        composeColumnHeader(i, d_columns.at(i)->name() + "[X" +
                                   QString::number(++x_cols) + "]");
      else if (d_columns.at(i)->plotDesignation() == LabRPS::Y) {
        if (x_cols > 0)
          composeColumnHeader(i, d_columns.at(i)->name() + "[Y" +
                                     QString::number(x_cols) + "]");
        else
          composeColumnHeader(i, d_columns.at(i)->name() + "[Y]");
      } else if (d_columns.at(i)->plotDesignation() == LabRPS::Z) {
        if (x_cols > 0)
          composeColumnHeader(i, d_columns.at(i)->name() + "[Z" +
                                     QString::number(x_cols) + "]");
        else
          composeColumnHeader(i, d_columns.at(i)->name() + "[Z]");
      } else if (d_columns.at(i)->plotDesignation() == LabRPS::xErr) {
        if (x_cols > 0)
          composeColumnHeader(i, d_columns.at(i)->name() + "[xEr" +
                                     QString::number(x_cols) + "]");
        else
          composeColumnHeader(i, d_columns.at(i)->name() + "[xEr]");
      } else if (d_columns.at(i)->plotDesignation() == LabRPS::yErr) {
        if (x_cols > 0)
