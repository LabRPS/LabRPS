...
#  \brief The Addon Manager allows users to install workbenches, macros and plugins  made by other users
#  @{
...
class CommandAddonManager:
    """The main Addon Manager class and LabRPS command"""
    ...
    def GetResources(self) -> Dict[str, str]:
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Std_AddonMgr",
                "Manage external workbenches, macros, plugins and preference packs",
            ),
            "Group": "Tools",
        }
    ...
    def Activated(self) -> None:
        ...
        pref = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Addons")
        ...
        if not readWarning:
            warning_dialog = LabRPSGui.PySideUic.loadUi(
            ...
            download_plugins = pref.GetBool("DownloadPlugins", False)
            ...
            warning_dialog.checkBoxDownloadMacroMetadata.setChecked(download_plugins)
        ...
        if readWarning:
            # Check the connection in a new thread, so LabRPS stays responsive
    ...
    def launch(self) -> None:
        ...
        # create the dialog
        self.dialog = LabRPSGui.PySideUic.loadUi(
        ...
        self.macro_repo_dir = LabRPS.getUserMacroDir(True)
        self.plugin_repo_dir = LabRPS.getUserPluginDir(True)
        ...
        # set nice icons to everything, by theme with fallback to LabRPS icons
        ...
        # center the dialog over the LabRPS window
    ...
    def determine_cache_update_status(self) -> None:
        ...
        cache_path = LabRPS.getUserCachePath()
    ...
    def reject(self) -> None:
        ...
        if self.restart_required:
            ...
            m.setText(
                translate(
                    "AddonsInstaller",
                    "You must restart LabRPS for changes to take effect.",
                )
            )
            m.setIcon(m.Warning)
            ...
            ret = m.exec_()
            if ret == m.Ok:
                # restart LabRPS after a delay to give time to this dialog to close
                QtCore.QTimer.singleShot(1000, utils.restart_labrps)
    ...
    def startup(self) -> None:
        ...
        Second, the list of macros is downloaded from the LabRPS/LabRPS-macros repository and the wiki
        ...
        if pref.GetBool("DownloadPlugins", False):
            self.startup_sequence.append(self.load_plugin_metadata)
    ...
    def populate_plugins(self) -> None:
        plugin_cache_file = self.get_cache_file_name("plugin_cache.json")
        cache_is_bad = True
        if os.path.isfile(plugin_cache_file):
            size = os.path.getsize(plugin_cache_file)
            if size > 1000:  # Make sure there is actually data in there
                cache_is_bad = False
        if self.update_cache or cache_is_bad:
            self.update_cache = True
            self.plugin_worker = FillPluginListWorker(self.get_cache_file_name("Plugins"))
            self.plugin_worker.status_message_signal.connect(self.show_information)
            self.plugin_worker.progress_made.connect(self.update_progress_bar)
            self.plugin_worker.add_plugin_signal.connect(self.add_addon_repo)
            self.plugin_worker.finished.connect(self.do_next_startup_phase)
            self.plugin_worker.start()
        else:
            self.plugin_worker = LoadPluginsFromCacheWorker(
                self.get_cache_file_name("plugin_cache.json")
            )
            self.plugin_worker.add_plugin_signal.connect(self.add_addon_repo)
            self.plugin_worker.finished.connect(self.do_next_startup_phase)
            self.plugin_worker.start()
    ...
    def cache_plugin(self, repo: Addon):
        if not hasattr(self, "plugin_cache"):
            self.plugin_cache = []
        if repo.plugin is not None:
            self.plugin_cache.append(repo.plugin.to_cache())
        else:
            LabRPS.Console.PrintError(
                f"Addon Manager: Internal error, cache_plugin called on non-plugin {repo.name}\n"
            )
    ...
    def write_plugin_cache(self):
        if not hasattr(self, "plugin_cache"):
            return
        plugin_cache_path = self.get_cache_file_name("plugin_cache.json")
        with open(plugin_cache_path, "w") as f:
            f.write(json.dumps(self.plugin_cache, indent="  "))
            self.plugin_cache = []
    ...
    def load_plugin_metadata(self) -> None:
        if self.update_cache:
            self.load_plugin_metadata_worker = CachePluginCode(self.item_model.repos)
            self.load_plugin_metadata_worker.status_message.connect(
                self.show_information
            )
            self.load_plugin_metadata_worker.update_plugin.connect(
                self.on_package_updated
            )
            self.load_plugin_metadata_worker.progress_made.connect(
                self.update_progress_bar
            )
            self.load_plugin_metadata_worker.finished.connect(self.do_next_startup_phase)
            self.load_plugin_metadata_worker.start()
        else:
            self.do_next_startup_phase()
    ...
    def get_icon(self, repo: Addon, update: bool = False) -> QtGui.QIcon:
        ...
        if repo.repo_type == Addon.Kind.WORKBENCH:
            ...
            if repo.plugin and repo.plugin.icon:
                if os.path.isabs(repo.plugin.icon):
                    path = repo.plugin.icon
                    default_icon = QtGui.QIcon(":/icons/rpsplugin.svg")
                else:
                    path = os.path.join(
                        os.path.dirname(repo.plugin.src_filename), repo.plugin.icon
                    )
    ...
    class MissingDependencies:
        ...
        * Optional Python packages -> python_optional
        ...
        def __init__(self, repo: Addon, all_repos: List[Addon]):
            ...
            wbs = [wb.lower() for wb in LabRPSGui.listWorkbenches()]
                ...
                if dep.lower() + "workbench" not in wbs:
                    if dep.lower() == "plot":
                        ...
                        try:
                            ...
                            self.wbs.append(dep)
                            LabRPS.Console.PrintLog("Failed to import Plot module")
                    else:
                        self.wbs.append(dep)
    ...
    def update_allowed_packages_list(self) -> None:
        LabRPS.Console.PrintLog("Attempting to fetch remote copy of ALLOWED_PYTHON_PACKAGES.txt...\n")
        p = NetworkManager.AM_NETWORK_MANAGER.blocking_get(
            "https://raw.githubusercontent.com/LabRPS/LabRPS-addons/master/ALLOWED_PYTHON_PACKAGES.txt"
        )
        if p:
            LabRPS.Console.PrintLog("Remote ALLOWED_PYTHON_PACKAGES.txt file located, overriding locally-installed copy\n")
            ...
            LabRPS.Console.PrintLog("Could not fetch remote ALLOWED_PYTHON_PACKAGES.txt, using local copy\n")
    ...
    def handle_disallowed_python(self, python_required: List[str]) -> bool:
        ...
        if bad_packages:
            ...
            for package in self.allowed_packages:
                LabRPS.Console.PrintMessage(f"  * {package}\n")
    ...
    def report_missing_workbenches(self, addon_name: str, wbs) -> bool:
        if len(wbs) == 1:
            ...
            message = translate(
                "AddonsInstaller",
                "Addon '{}' requires '{}', which is not available in your copy of LabRPS.",
            ...
            message = (
                ...
                + translate(
                    "AddonsInstaller",
                    "Addon '{}' requires the following workbenches, which are not available in your copy of LabRPS:",
                ).format(addon_name)
                + "</p><ul>"
            )
            for wb in wbs:
    ...
    def display_dep_resolution_dialog(self, missing, repo: Addon) -> None:
        self.dependency_dialog = LabRPSGui.PySideUic.loadUi(
    ...
    def dependency_installation_failure(self, short_message: str, details: str) -> None:
        if hasattr(self, "dependency_installation_dialog"):
            self.dependency_installation_dialog.hide()
        LabRPS.Console.PrintError(details)
        QtWidgets.QMessageBox.critical(
            self.dialog,
    ...
    def install(self, repo: Addon) -> None:
        """installs or updates a workbench, macro, plugin, or package"""
        ...
        if (
            ...
            errors = []
            with tempfile.TemporaryDirectory() as dir:
                temp_install_succeeded, error_list = plugin.install(dir)
                if not temp_install_succeeded:
                    failed = True
                    errors = error_list

            if not failed:
                real_install_succeeded, errors = plugin.install(self.plugin_repo_dir)
                if not real_install_succeeded:
                    failed = True
                else:
            ...
            if not failed:
                message = translate(
                    "AddonsInstaller",
                    "Plugin successfully installed. The plugin is now available from the Plugins dialog.",
                )
                self.on_package_installed(repo, message)
            else:
                message = (
                    translate("AddonsInstaller", "Installation of plugin failed") + ":"
                )
                for error in errors:
                    message += "\n  * "
                    message += error
    ...
    def on_update_all_completed(self) -> None:
        ...
        def get_package_list(message: str, repos: List[Addon], threshold: int):
            ...
            if num_updates < threshold:
                ...
                for repo in repos:
                    LabRPS.Console.PrintMessage(f"{message}: {repo.name}\n")
            return result
    ...
    def executemacro(self, repo: Addon) -> None:
        ...
        if macro.is_installed():
            ...
            LabRPSGui.open(str(macro_path))
            self.dialog.hide()
            LabRPSGui.SendMsgToActiveView("Run")
        else:
            with tempfile.TemporaryDirectory() as dir:
    ...
    def remove(self, repo: Addon) -> None:
        """uninstalls a macro or plugin or workbench"""
            ...
            if addon.metadata is not None:
                ...
                if addon.macro.parsed:
                    if len(addon.macro.icon) == 0 and len(addon.macro.xpm) == 0:
                        LabRPS.Console.PrintLog(f"Macro '{addon.name}' does not have an icon\n")
            elif addon.repo_type == Addon.Kind.PLUGIN:
                if addon.plugin.parsed:
                    if len(addon.plugin.icon) == 0 and len(addon.plugin.xpm) == 0:
                        LabRPS.Console.PrintLog(f"Plugin '{addon.name}' does not have an icon\n")
            else:
                LabRPS.Console.PrintLog(f"Addon '{addon.name}' does not have a package.xml file\n")
    ...
    def validate_package_xml(self, addon:Addon):
        ...
        if len(errors) > 0:
            LabRPS.Console.PrintLog(f"Errors found in package.xml file for '{addon.name}'\n")
            for error in errors:
                LabRPS.Console.PrintLog(f"   * {error}\n")
