...
def ask_to_install_toolbar_button(repo: Addon) -> None:
    ...
    if not do_not_show_dialog and not button_exists:
        add_toolbar_button_dialog = LabRPSGui.PySideUic.loadUi(
...
def check_for_button(repo: Addon) -> bool:
    command = LabRPSGui.Command.findCustomCommand(repo.macro.filename)
    if not command:
        return False
    custom_toolbars = LabRPS.ParamGet(
...
def ask_for_toolbar(
    ...
    if ask:
        select_toolbar_dialog = LabRPSGui.PySideUic.loadUi(
...
def get_toolbar_with_name(name: str) -> object:
    top_group = LabRPS.ParamGet("User parameter:BaseApp/Workbench/Global/Toolbar")
    custom_toolbars = top_group.GetGroups()
    for toolbar in custom_toolbars:
        group = LabRPS.ParamGet(
...
def create_new_custom_toolbar() -> object:
    ...
    custom_toolbar = LabRPS.ParamGet(
...
def install_macro_to_toolbar(repo: Addon, toolbar: object) -> None:
    ...
    if repo.macro.comment:
        ...
        whatsThisText = translate(
            "AddonsInstaller", "A macro installed with the LabRPS Addon Manager"
        )
    statustipText = (
    ...
    if repo.macro.icon:
        if os.path.isabs(repo.macro.icon):
            ...
            macro_repo_dir = LabRPS.getUserMacroDir(True)
    ...
    command_name = LabRPSGui.Command.createCustomCommand(
        repo.macro.filename,
        menuText,
        tooltipText,
        whatsThisText,
    ...
    toolbar.SetString(command_name, "LabRPS")

    # Force the toolbars to be recreated
    wb = LabRPSGui.activeWorkbench()
    wb.reloadActive()
...
def remove_custom_toolbar_button(repo: Addon) -> None:
    ...
    LabRPSGui.Command.removeCustomCommand(command)
