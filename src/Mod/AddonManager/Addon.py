...
INTERNAL_WORKBENCHES["seismiclab"] = "SeismicLab"
INTERNAL_WORKBENCHES["sealab"] = "SeaLab"


class Addon:
    "Encapsulate information about a LabRPS addon"
    ...
    def __str__(self) -> str:
        result = f"LabRPS {self.repo_type}\n"
        ...
        if self.macro is not None:
            ...
            result += "Has linked Plugin object\n"
        return result
    ...
    @classmethod
    def from_plugin(self, plugin: Plugin):
        if plugin.is_installed():
            status = Addon.Status.UNCHECKED
        else:
            status = Addon.Status.NOT_INSTALLED
        instance = Addon(plugin.name, plugin.url, status, "master")
        instance.plugin = plugin
        instance.repo_type = Addon.Kind.PLUGIN
        instance.description = plugin.desc
        return instance

    @classmethod
    def from_cache(self, cache_dict: Dict):
        """Load basic data from cached dict data. Does not include Macro, Plugin or Metadata information, which must be populated separately."""

        mod_dir = os.path.join(LabRPS.getUserAppDataDir(), "Mod", cache_dict["name"])
    ...
    def load_metadata_file(self, file: str) -> None:
        if os.path.exists(file):
            metadata = LabRPS.Metadata(file)
            self.set_metadata(metadata)
        else:
            LabRPS.Console.PrintLog("Internal error: {} does not exist".format(file))
    ...
    def version_is_ok(self, metadata) -> bool:
        ...
        try:
            ...
            LabRPS.Console.PrintMessage(
                f"Metadata file for {self.name} has invalid LabRPSMin version info\n"
            )
        ...
        try:
            ...
            LabRPS.Console.PrintMessage(
                f"Metadata file for {self.name} has invalid LabRPSMax version info\n"
            )
    ...
    def extract_tags(self, metadata: LabRPS.Metadata) -> None:
    ...
    def contains_plugin(self) -> bool:
        """Determine if this package contains (or is) a plugin"""
    ...
    def is_disabled(self):
        ...
        stopfile = os.path.join(
            LabRPS.getUserAppDataDir(), "Mod", self.name, "ADDON_DISABLED"
        )
    ...
    def disable(self):
        ...
        with open(stopfile, "w") as f:
            f.write(
                "The existence of this file prevents LabRPS from loading this Addon. To re-enable, delete the file."
            )
