...
from addonmanager_plugin import Plugin
...
class ConnectionChecker(QtCore.QThread):
    ...
    def run(self):
        LabRPS.Console.PrintLog("Checking network connection...\n")
        ...
        LabRPS.Console.PrintLog(f"GitHub's zen message response: {result}\n")
        self.success.emit()
...
class UpdateWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        # update info lists
        global obsolete, macros_reject_list, plugins_reject_list, mod_reject_list, py2only
        p = NetworkManager.AM_NETWORK_MANAGER.blocking_get(
            "https://raw.githubusercontent.com/LabRPS/LabRPS-addons/master/addonflags.json"
        )
        if p:
            ...
            if "blacklisted" in j and "Plugin" in j["blacklisted"]:
                plugins_reject_list = j["blacklisted"]["Plugin"]
                        ...
                        try:
                            ...
                            if major < fc_major or (
                                ...
                                if "kind" not in item or item["kind"] == "mod":
                                    ...
                                    plugins_reject_list.append(item["name"])
                                else:
                                    LabRPS.Console.PrintMessage(
            ...
            LabRPS.Console.PrintError(message + "\n")
        ...
        addon_list = (
            LabRPS.ParamGet("User parameter:BaseApp/Preferences/Addons")
        ...
        p = NetworkManager.AM_NETWORK_MANAGER.blocking_get(
            "https://raw.githubusercontent.com/LabRPS/LabRPS-addons/master/.gitmodules"
        )
        if not p:
            return
        p = p.data().decode("utf8")
...
class LoadPackagesFromCacheWorker(QtCore.QThread):
    ...
    def run(self):
        metadata_cache_path = os.path.join(
            LabRPS.getUserCachePath(), "AddonManager", "PackageMetadata"
        )
...
class LoadPluginsFromCacheWorker(QtCore.QThread):
    add_plugin_signal = QtCore.Signal(object)
    ...
    def run(self):
        with open(self.cache_file, "r") as f:
            ...
            for item in dict_data:
                if QtCore.QThread.currentThread().isInterruptionRequested():
                    return
                new_plugin = Plugin.from_cache(item)
                repo = Addon.from_plugin(new_plugin)
                utils.update_plugin_installation_details(repo)
                self.add_plugin_signal.emit(repo)
...
class CheckWorkbenchesForUpdatesWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        self.current_thread = QtCore.QThread.currentThread()
        self.basedir = LabRPS.getUserAppDataDir()
...
class UpdateChecker:
        ...
        if os.path.exists(clonedir):
            ...
            with wb.git_lock:
                ...
                try:
                    ...
                    LabRPS.Console.PrintWarning(str(e) + "\n")
                    wb.set_status(Addon.Status.CANNOT_CHECK)
                else:
                    try:
    ...
    def check_package(self, package: Addon) -> None:
        ...
        if os.path.exists(clonedir):
            ...
            try:
                installed_metadata = LabRPS.Metadata(installed_metadata_file)
    ...
    def check_macro(self, macro_wrapper: Addon) -> None:
        ...
        test_file_one = os.path.join(
            LabRPS.getUserMacroDir(True), macro_wrapper.macro.filename
        )
        test_file_two = os.path.join(
            LabRPS.getUserMacroDir(True), "Macro_" + macro_wrapper.macro.filename
        )
    ...
    def check_plugin(self, plugin_wrapper: Addon) -> None:
        # Make sure this plugin has its code downloaded:
        try:
            if not plugin_wrapper.plugin.parsed and plugin_wrapper.plugin.on_git:
            ...
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Failed to fetch code for plugin '{name}'",
                ).format(name=plugin_wrapper.plugin.name)
                + "\n"
            )
            plugin_wrapper.set_status(Addon.Status.CANNOT_CHECK)
            return

        hasher1 = hashlib.sha1()
        hasher2 = hashlib.sha1()
        hasher1.update(plugin_wrapper.plugin.code.encode("utf-8"))
        new_sha1 = hasher1.hexdigest()
        test_file_one = os.path.join(
            LabRPS.getUserPluginDir(True), plugin_wrapper.plugin.filename
        )
        test_file_two = os.path.join(
            LabRPS.getUserPluginDir(True), "Plugin_" + plugin_wrapper.plugin.filename
        )
        if os.path.exists(test_file_one):
            with open(test_file_one, "rb") as f:
                contents = f.read()
            ...
            return
        if new_sha1 == old_sha1:
            plugin_wrapper.set_status(Addon.Status.NO_UPDATE_AVAILABLE)
        else:
            plugin_wrapper.set_status(Addon.Status.UPDATE_AVAILABLE)
...
class FillMacroListWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        if not self.current_thread.isInterruptionRequested():
            self.status_message_signal.emit(
                translate(
                    "AddonsInstaller",
                    "Retrieving macros from LabRPS/LabRPS-Macros Git repository",
                )
            )
    ...
    def retrieve_macros_from_git(self):
        """Retrieve macros from LabRPS-macros.git

        Emits a signal for each macro in
        https://github.com/LabRPS/LabRPS-macros.git
        """
        ...
        if not have_git or NOGIT:
            ...
            LabRPS.Console.PrintWarning(message + "\n")
            return

        try:
            if os.path.exists(self.repo_dir):
                if not os.path.exists(os.path.join(self.repo_dir, ".git")):
                    LabRPS.Console.PrintWarning(translate("AddonsInstaller","Attempting to change non-git Macro setup to use git\n"))
                    utils.repair_git_repo(
                        "https://github.com/LabRPS/LabRPS-macros.git", self.repo_dir
                    )
            ...
            LabRPS.Console.PrintMessage(f"{self.repo_dir}\n")
            LabRPS.Console.PrintMessage(
                translate(
        ...
        for dirpath, _, filenames in os.walk(self.repo_dir):
            ...
            for filename in filenames:
                if self.current_thread.isInterruptionRequested():
                    return
                if filename.lower().endswith(".rpsmacro"):
                    macro = Macro(filename[:-8])  # Remove ".RPSMacro".
                    macro.on_git = True
                    ...
                    LabRPS.Console.PrintLog(f"Found macro {repo.name}\n")
                    repo.url = "https://github.com/LabRPS/LabRPS-macros.git"
                    utils.update_macro_installation_details(repo)
                    self.add_macro_signal.emit(repo)
    ...
    def retrieve_macros_from_wiki(self):
        ...
        if not p:
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Error connecting to the Wiki, LabRPS cannot retrieve the Wiki macro list at this time",
                )
                + "\n"
            )
            return
...
class FillPluginListWorker(QtCore.QThread):
    """This worker populates the list of plugins"""
    ...
    def run(self):
        ...
        if not self.current_thread.isInterruptionRequested():
            self.status_message_signal.emit(
                translate(
                    "AddonsInstaller",
                    "Retrieving plugins from LabRPS/LabRPS-Plugins Git repository",
                )
            )
        ...
        self.status_message_signal.emit(
            translate("AddonsInstaller", "Done locating plugins.")
        )

    def retrieve_plugins_from_git(self):
        """Retrieve plugins from LabRPS-plugins.git

        Emits a signal for each plugin in
        https://github.com/LabRPS/LabRPS-plugins.git
        """
        ...
        try:
            if os.path.exists(self.repo_dir):
                if not os.path.exists(os.path.join(self.repo_dir, ".git")):
                    LabRPS.Console.PrintWarning(translate("AddonsInstaller","Attempting to change non-git Plugin setup to use git\n"))
                    utils.repair_git_repo(
                        "https://github.com/LabRPS/LabRPS-plugins.git", self.repo_dir
                    )
                gitrepo = git.Git(self.repo_dir)
                gitrepo.pull("--ff-only")
            else:
            ...
            LabRPS.Console.PrintMessage(
                translate(
                    "AddonsInstaller", "An error occurred updating plugins from GitHub, trying clean checkout..."
                )
                + f":\n{e}\n"
            )
            LabRPS.Console.PrintMessage(f"{self.repo_dir}\n")
            ...
            try:
                ...
                LabRPS.Console.PrintWarning(
                    translate(
                        "AddonsInstaller", "Failed to update plugins from GitHub -- try clearing the Addon Manager's cache."
                    )
                    + f":\n{str(e)}\n"
                )
                return
        ...
        for dirpath, _, filenames in os.walk(self.repo_dir):
            ...
            for filename in filenames:
                ...
                if filename.lower().endswith(".txt"):
                    plugin = Plugin(filename[:-4])  # Remove ".txt".
                    plugin.on_git = True
                    plugin.src_filename = os.path.join(dirpath, filename)
                    plugin.fill_details_from_file(plugin.src_filename)
                    repo = Addon.from_plugin(plugin)
                    LabRPS.Console.PrintLog(f"Found plugin {repo.name}\n")
                    repo.url = "https://github.com/LabRPS/LabRPS-plugins.git"
                    utils.update_plugin_installation_details(repo)
                    self.add_plugin_signal.emit(repo)

    def retrieve_plugins_from_wiki(self):
        """Retrieve plugins from the wiki

        Read the wiki and emit a signal for each found plugin.
        Reads only the page https://wiki.labrps.com/Plugins_recipes
        """

        p = NetworkManager.AM_NETWORK_MANAGER.blocking_get(
            "https://wiki.labrps.com/Plugins_recipes"
        )
        if not p:
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Error connecting to the Wiki, LabRPS cannot retrieve the Wiki plugin list at this time",
                )
                + "\n"
            )
            return
        p = p.data().decode("utf8")
        plugins = re.findall('title="(Plugin.*?)"', p)
        plugins = [mac for mac in plugins if ("translated" not in mac)]
        plugin_names = []
        for i, mac in enumerate(plugins):
            self.progress_made.emit(i, len(plugins))
            if self.current_thread.isInterruptionRequested():
                return
            macname = mac[6:]  # Remove "Plugin ".
            macname = macname.replace("&amp;", "&")
            if not macname:
                continue
            if (
                (macname not in plugins_reject_list)
                and ("recipes" not in macname.lower())
                and (macname not in plugin_names)
            ):
                plugin_names.append(macname)
                plugin = Plugin(macname)
                plugin.on_wiki = True
                plugin.parsed = False
                repo = Addon.from_plugin(plugin)
                repo.url = "https://wiki.labrps.com/Plugins_recipes"
                utils.update_plugin_installation_details(repo)
                self.add_plugin_signal.emit(repo)
...
class CachePluginCode(QtCore.QThread):
    """Download and cache the plugin code, and parse its internal metadata"""

    status_message = QtCore.Signal(str)
    update_plugin = QtCore.Signal(Addon)
    progress_made = QtCore.Signal(int, int)
    ...
    def run(self):
        self.status_message.emit(translate("AddonsInstaller", "Caching plugin code..."))

        self.repo_queue = queue.Queue()
        current_thread = QtCore.QThread.currentThread()
        num_plugins = 0
        for repo in self.repos:
            if repo.plugin is not None:
                self.repo_queue.put(repo)
                num_plugins += 1
                    ...
                    if not worker.wait(100):
                        LabRPS.Console.PrintWarning(
                            translate(
                                "AddonsInstaller",
                                "Addon Manager: a worker process failed to halt ({name})",
                            ).format(name=worker.plugin.name)
                            + "\n"
                        )
                return
            # Ensure our signals propagate out by running an internal thread-local event loop
        ...
        if len(self.failed) > 0:
            ...
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Out of {num_plugins} plugins, {num_failed} timed out while processing",
                ).format(num_plugins=num_plugins, num_failed=num_failed)
            )

    def update_and_advance(self, repo: Addon) -> None:
        if repo is not None:
            if repo.plugin.name not in self.failed:
                self.update_plugin.emit(repo)
            self.repo_queue.task_done()
            with self.lock:
                self.counter += 1
        ...
        try:
            next_repo = self.repo_queue.get_nowait()
            worker = GetPluginDetailsWorker(next_repo)
            worker.finished.connect(lambda: self.update_and_advance(next_repo))
            with self.lock:
                self.workers.append(worker)
                self.terminators.append(
    ...
    def terminate(self, worker) -> None:
        if not worker.isFinished():
            ...
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Timeout while fetching metadata for plugin {}",
                ).format(plugin_name)
                + "\n"
            )
            worker.blockSignals(True)
            ...
            if worker.isRunning():
                LabRPS.Console.PrintError(
                    translate(
                        "AddonsInstaller",
                        "Failed to kill process for plugin {}!\n",
                    ).format(plugin_name)
                )
            with self.lock:
                self.failed.append(plugin_name)
...
class GetPluginDetailsWorker(QtCore.QThread):
    """Retrieve the plugin details for a plugin"""
    ...
    def __init__(self, repo):
        ...
        QtCore.QThread.__init__(self)
        self.plugin = repo.plugin

    def run(self):

        self.status_message.emit(
            translate("AddonsInstaller", "Retrieving plugin description...")
        )
        if not self.plugin.parsed and self.plugin.on_git:
            self.status_message.emit(
                translate("AddonsInstaller", "Retrieving info from git")
            )
...
class InstallWorkbenchWorker(QtCore.QThread):
        ...
        with self.repo.git_lock:
            ...
            try:
                ...
                if self.repo.contains_workbench():
                    answer = translate(
                        "AddonsInstaller",
                        "Workbench successfully updated. Please restart LabRPS to apply the changes.",
                    )
                else:
                    answer = translate(
                        "AddonsInstaller",
    ...
    def run_git_clone(self, clonedir: str) -> None:
        ...
        LabRPS.Console.PrintMessage("Cloning repo...\n")
        if self.repo.git_lock.locked():
            LabRPS.Console.PrintMessage("Waiting for lock to be released to us...\n")
        ...
        with self.repo.git_lock:
            LabRPS.Console.PrintMessage("Lock acquired...\n")
            ...
            LabRPS.Console.PrintMessage("Initial clone complete...\n")
            if current_thread.isInterruptionRequested():
                return

            # Make sure to clone all the submodules as well
            if repo.submodules:
                LabRPS.Console.PrintMessage("Updating submodules...\n")
            ...
            if self.repo.branch in repo.heads:
                LabRPS.Console.PrintMessage("Checking out HEAD...\n")
                repo.heads[self.repo.branch].checkout()

            LabRPS.Console.PrintMessage("Clone complete\n")

        if self.repo.contains_workbench():
            answer = translate(
                "AddonsInstaller",
                "Workbench successfully installed. Please restart LabRPS to apply the changes.",
            )
        else:
            answer = translate(
                "AddonsInstaller",
...
class DependencyInstallationWorker(QtCore.QThread):
    """Install dependencies using Addonmanager for LabRPS, and pip for python"""
    ...
    def run(self):
        ...
        if self.python_required or self.python_optional:
            ...
            #   B) The executable located in the same bin directory as LabRPS and called "python3"
            #   C) The executable located in the same bin directory as LabRPS and called "python"
            ...
            prefs = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Addons")
            ...
            if pip_failed:
                ...
                return
            LabRPS.Console.PrintMessage(proc.stdout)
            LabRPS.Console.PrintWarning(proc.stderr)
            result = proc.stdout
            LabRPS.Console.PrintMessage(result.decode())
            vendor_path = os.path.join(
                LabRPS.getUserAppDataDir(), "AdditionalPythonPackages"
            )
        ...
        for pymod in self.python_required:
            ...
            LabRPS.Console.PrintMessage(proc.stdout.decode())
            if proc.returncode != 0:
                self.failure.emit(
                    translate(
                        "AddonsInstaller",
...
class UpdateMetadataCacheWorker(QtCore.QThread):
    ...
    def process_metadata_txt(self, repo: Addon, data: QtCore.QByteArray):
        ...
        try:
            ...
            LabRPS.Console.PrintWarning(str(e))
            LabRPS.Console.PrintWarning(f" in package {repo.name}\n")
            return
        while True:
            line = f.readline()
            if not line:
            ...
            if line.startswith("workbenches="):
                ...
                for wb in depswb:
                    ...
                    if wb_name:
                        ...
                        LabRPS.Console.PrintLog(
                            f"{repo.display_name} requires LabRPS Addon '{wb_name}'\n"
                        )
...
class UpdateAllWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        for repo in self.repos:
            self.repo_queue.put(repo)
            LabRPS.Console.PrintLog(f"  UPDATER: Adding '{repo.name}' to update queue\n")
        ...
        workers = []
        for _ in range(1):
            LabRPS.Console.PrintLog(f"  UPDATER: Starting worker\n")
    ...
    def on_success(self, repo: Addon) -> None:
        LabRPS.Console.PrintLog(f"  UPDATER: Main thread received notice that worker successfully updated {repo.name}\n")
        self.progress_made.emit(
            len(self.repos) - self.repo_queue.qsize(), len(self.repos)
        )
    ...
    def on_failure(self, repo: Addon) -> None:
        LabRPS.Console.PrintLog(f"  UPDATER:  Main thread received notice that worker failed to update {repo.name}\n")
        self.progress_made.emit(
            len(self.repos) - self.repo_queue.qsize(), len(self.repos)
        )
...
class UpdateSingleWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        while True:
            if current_thread.isInterruptionRequested():
                LabRPS.Console.PrintLog(f"  UPDATER: Interruption requested, stopping all updates\n")
                return
            try:
                repo = self.repo_queue.get_nowait()
                LabRPS.Console.PrintLog(f"  UPDATER: Pulling {repo.name} from the update queue\n")
            except queue.Empty:
                LabRPS.Console.PrintLog(f"  UPDATER: Worker thread queue is empty, exiting thread\n")
                return
            if repo.repo_type == Addon.Kind.MACRO:
                LabRPS.Console.PrintLog(f"  UPDATER: Updating macro '{repo.name}'\n")
                self.update_macro(repo)
            if repo.repo_type == Addon.Kind.PLUGIN:
                LabRPS.Console.PrintLog(f"  UPDATER: Updating plugin '{repo.name}'\n")
                self.update_plugin(repo)
            else:
                LabRPS.Console.PrintLog(f"  UPDATER: Updating addon '{repo.name}'\n")
                self.update_package(repo)
            self.repo_queue.task_done()
            LabRPS.Console.PrintLog(f"  UPDATER: Worker thread completed action for '{repo.name}' and reported result to main thread\n")
    ...
    def update_macro(self, repo: Addon):
        ...
        cache_path = os.path.join(
            LabRPS.getUserCachePath(), "AddonManager", "MacroCache"
        )
        os.makedirs(cache_path, exist_ok=True)
        install_succeeded, _ = repo.macro.install(cache_path)

        if install_succeeded:
            install_succeeded, _ = repo.macro.install(LabRPS.getUserMacroDir(True))
            utils.update_macro_installation_details(repo)
    ...
    def update_plugin(self, repo: Addon):
        """Updating a plugin happens in this function, in the current thread"""

        cache_path = os.path.join(
            LabRPS.getUserCachePath(), "AddonManager", "PluginCache"
        )
        os.makedirs(cache_path, exist_ok=True)
        install_succeeded, _ = repo.plugin.install(cache_path)

        if install_succeeded:
            install_succeeded, _ = repo.plugin.install(LabRPS.getUserPluginDir(True))
            utils.update_plugin_installation_details(repo)
