...
#  \brief Unified handler for LabRPS macros that can be obtained from
#  different sources
#  @{
...
class Macro(object):
    ...
    def filename(self):
        ...
        return (self.name + ".RPSMacro").replace(" ", "_")

    def is_installed(self):
        if self.on_git and not self.src_filename:
            return False
        return os.path.exists(
            os.path.join(LabRPS.getUserMacroDir(True), self.filename)
        ) or os.path.exists(
            os.path.join(LabRPS.getUserMacroDir(True), "Macro_" + self.filename)
        )
    ...
    def install(self, macro_dir: str) -> Tuple[bool, List[str]]:
        ...
        LabRPS.Console.PrintLog(f"Macro {self.name} was installed successfully.\n")
        return True, []
