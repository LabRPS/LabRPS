...
try:
    ...
    # For standalone testing support working without the LabRPS import
    HAVE_LABRPS = False
...
try:
    ...
    if HAVE_LABRPS:
        LabRPS.Console.PrintError(
            translate(
                "AddonsInstaller",
                "Could not import QtNetwork -- it does not appear to be installed on your system. Please install the package 'python3-pyside2.qtnetwork' on your system and if possible contact your LabRPS package maintainer to alert them to the missing dependency. The Addon Manager will not be available.",
            )
            + "\n"
        )
    else:
...
if HAVE_QTNETWORK:
    ...
    class NetworkManager(QtCore.QObject):
        ...
        def __init__(self):
            ...
            if noProxyCheck:
                ...
                query = QtNetwork.QNetworkProxyQuery(
                    QtCore.QUrl("https://github.com/LabRPS/LabRPS")
                )
        ...
        def __authenticate_proxy(
            ...
            if HAVE_LABRPS and LabRPS.GuiUp:
                proxy_authentication = LabRPSGui.PySideUic.loadUi(
        ...
        def __on_ssl_error(self, reply: str, errors: List[str]):
            if HAVE_LABRPS:
                ...
                LabRPS.Console.PrintWarning(reply)
                for error in errors:
                    LabRPS.Console.PrintWarning(error)
            else:
