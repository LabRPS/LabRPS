...
def update_plugin_details(old_plugin, new_plugin):
    """Update a plugin with information from another one

    Update a plugin with information from another one, supposedly the same but
    from a different source. The first source is supposed to be git, the second
    one the wiki.
    """

    if old_plugin.on_git and new_plugin.on_git:
        LabRPS.Console.PrintLog(
            'The plugin "{}" is present twice in github, please report'.format(
                old_plugin.name
            )
        )
    # We don't report plugins present twice on the wiki because a link to a
    # plugin is considered as a plugin. 
    old_plugin.on_wiki = new_plugin.on_wiki
    for attr in ["desc", "url", "code"]:
        if not hasattr(old_plugin, attr):
            setattr(old_plugin, attr, getattr(new_plugin, attr))


def remove_directory_if_empty(dir):
    """Remove the directory if it is empty

    Directory LabRPS.getUserMacroDir(True) will not be removed even if empty.
    Directory LabRPS.getUserPluginDir(True) will not be removed even if empty.

    """

    if dir == LabRPS.getUserMacroDir(True) or dir == LabRPS.getUserPluginDir(True):
        return
...
def restart_labrps():
    """Shuts down and restarts LabRPS"""

    args = QtWidgets.QApplication.arguments()[1:]
    if LabRPSGui.getMainWindow().close():
...
def is_darkmode() -> bool:
    ...
    pl = LabRPSGui.getMainWindow().palette()
...
def get_plugin_version_from_file(filename: str) -> str:
    """Get the version of the plugiin from a local plugin file. Supports strings, ints, and floats, as
    well as a reference to __date__"""
...
def update_macro_installation_details(repo) -> None:
    if repo is None or not hasattr(repo, "macro") or repo.macro is None:
        LabRPS.Console.PrintLog(f"Requested macro details for non-macro object\n")
        return
    test_file_one = os.path.join(LabRPS.getUserMacroDir(True), repo.macro.filename)
    test_file_two = os.path.join(
        LabRPS.getUserMacroDir(True), "Macro_" + repo.macro.filename
    )
...
def update_plugin_installation_details(repo) -> None:
    if repo is None or not hasattr(repo, "plugin") or repo.plugin is None:
        LabRPS.Console.PrintLog(f"Requested plugin details for non-plugin object\n")
        return
    test_file_one = os.path.join(LabRPS.getUserPluginDir(True), repo.plugin.filename)
    test_file_two = os.path.join(
        LabRPS.getUserPluginDir(True), "Plugin_" + repo.plugin.filename
    )
    if os.path.exists(test_file_one):
        repo.updated_timestamp = os.path.getmtime(test_file_one)
        repo.installed_version = get_plugin_version_from_file(test_file_one)
    elif os.path.exists(test_file_two):
        repo.updated_timestamp = os.path.getmtime(test_file_two)
        repo.installed_version = get_plugin_version_from_file(test_file_two)
    else:
        return
