...
class PackageList(QWidget):
    ...
    def setModel(self, model):
        ...
        self.item_filter.setHideNewerLabRPSRequired(
            pref.GetBool("HideNewerLabRPSRequired", True)
        )
    ...
    def update_type_filter(self, type_filter: int) -> None:
        ...
        type_filter is an integer: 0 for all, 1 for workbenches, 2 for macros, and 3 for preference packs and 4 for plugins
...
class PackageListItemModel(QAbstractListModel):
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole):
        ...
        if role == Qt.ToolTipRole:
            ...
            elif self.repos[row].repo_type == Addon.Kind.PLUGIN:
                tooltip = translate(
                    "AddonsInstaller", "Click for details about plugin {}"
                ).format(self.repos[row].display_name)
            return tooltip
        ...
        if repo.metadata:
            ...
            if self.displayStyle == ListDisplayStyle.EXPANDED:
                if repo.macro.author:
                    ...
                    self.widget.ui.labelMaintainer.setText("")
        elif repo.plugin and repo.plugin.parsed:
            self.widget.ui.labelDescription.setText(repo.plugin.comment)
            version_string = ""
            if repo.plugin.version:
                version_string = repo.plugin.version + " "
            if repo.plugin.on_wiki:
                version_string += "(wiki)"
            elif repo.plugin.on_git:
                version_string += "(git)"
            else:
                version_string += "(unknown source)"
            if repo.plugin.date:
            ...
            self.widget.ui.labelVersion.setText("<i>" + version_string + "</i>")
            if self.displayStyle == ListDisplayStyle.EXPANDED:
                if repo.plugin.author:
                    caption = translate("AddonsInstaller", "Author")
                    self.widget.ui.labelMaintainer.setText(
                        caption + ": " + repo.plugin.author
                    )
                else:
                    self.widget.ui.labelMaintainer.setText("")
        else:
...
class PackageListFilter(QSortFilterProxyModel):
    ...
    def __init__(self):
        ...
        self.hide_py2 = False
        self.hide_newer_labrps_required = False

    def setPackageFilter(
        self, type: int
    ) -> None:  # 0=All, 1=Workbenches, 2=Macros, 3=Preference Packs, 4=Plugins
    ...
    def setHideNewerLabRPSRequired(self, hide_nfr: bool) -> None:
        self.hide_newer_labrps_required = hide_nfr
        self.invalidateFilter()
    ...
    def filterAcceptsRow(self, row, parent=QModelIndex()):
        ...
        if (
            data.status() == Addon.Status.NOT_INSTALLED
            and self.hide_newer_labrps_required
            and data.metadata
        ):
        ...
        if hasattr(self, "filterRegularExpression"):  # Added in Qt 5.12
            ...
            if re.isValid():
                ...
                if (
                    data.plugin
                    and data.plugin.comment
                    and re.match(data.plugin.comment).hasMatch()
                ):
                    return True
            ...
            if re.isValid():
                ...
                if (
                    data.plugin
                    and data.plugin.comment
                    and re.indexIn(data.plugin.comment) != -1
                ):
                    return True
                for tag in data.tags:
                    if re.indexIn(tag) != -1:
