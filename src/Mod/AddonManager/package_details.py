...
from addonmanager_workers import GetMacroDetailsWorker, GetPluginDetailsWorker, CheckSingleUpdateWorker
...
class PackageDetails(QWidget):
    ...
    def display_repo_status(self, status):
        ...
        if status != Addon.Status.NOT_INSTALLED:
            ...
            if status == Addon.Status.UPDATE_AVAILABLE:
                if repo.metadata:
                    ...
                    installed_version_string += ".</b>"
                elif repo.plugin and repo.plugin.version:
                    installed_version_string += (
                        "<b>"
                        + translate("AddonsInstaller", "Update available to version")
                        + " "
                    )
                    installed_version_string += repo.plugin.version
                    installed_version_string += ".</b>"
                else:
                    installed_version_string += (
                        "<b>"
                ...
                if have_git and repo.repo_type != Addon.Kind.PLUGIN:
                    basedir = LabRPS.getUserAppDataDir()
                    moddir = os.path.join(basedir, "Plugin", repo.name)
                    if os.path.exists(os.path.join(moddir, ".git")):
                        gitrepo = git.Repo(moddir)
                        branch = gitrepo.head.ref.name
                        detached_head = gitrepo.head.is_detached
                ...
                installed_version_string += (
                    translate(
                        "AddonsInstaller", "Updated, please restart LabRPS to use"
                    )
                    + "."
                )
            ...
            if repo.macro is not None:
                moddir = LabRPS.getUserMacroDir(True)
            if repo.plugin is not None:
                moddir = LabRPS.getUserPluginDir(True)
            else:
                basedir = LabRPS.getUserAppDataDir()
                moddir = os.path.join(basedir, "Mod", repo.name)
        ...
        required_version = self.requires_newer_labrps()
        if repo.obsolete:
            ...
            self.ui.labelWarningInfo.setText(
                "<h1>"
                + translate("AddonsInstaller", "WARNING: This addon requires LabRPS ")
                + required_version
                + "</h1>"
            )
            self.ui.labelWarningInfo.setStyleSheet(
    ...
    def requires_newer_labrps(self) -> Optional[str]:
        # If it's not installed, check to see if it's for a newer version of LabRPS
        if self.repo.status() == Addon.Status.NOT_INSTALLED and self.repo.metadata:
            ...
            first_supported_version = (
                self.repo.metadata.getFirstSupportedLabRPSVersion()
            )
    ...
    def set_change_branch_button_state(self):
        ...
        if self.repo.repo_type == Addon.Kind.MACRO:
            return
                ...
                # Is it a Plugin? If so, return:
        if self.repo.repo_type == Addon.Kind.PLUGIN:
            return
    ...
    def plugin_readme_updated(self):
        url = self.repo.plugin.wiki
        print(url)
        if not url:
            url = self.repo.plugin.url
        ...
        if HAS_QTWEBENGINE:
            if url:
                ...
                self.ui.urlBar.setText(
                    "("
                    + translate(
                        "AddonsInstaller", "No URL or wiki page provided by this plugin"
                    )
                    + ")"
                )
        else:
...
"""
    ...
    def enable_clicked(self) -> None:
        ...
        self.ui.labelWarningInfo.setText(
            ...
            + translate(
                "AddonsInstaller",
                "This Addon will be enabled next time you restart LabRPS.",
            )
            + "</h3>"
        )
    ...
    def disable_clicked(self) -> None:
        ...
        self.ui.labelWarningInfo.setText(
            ...
            + translate(
                "AddonsInstaller",
                "This Addon will be disabled next time you restart LabRPS.",
            )
            + "</h3>"
        )
        self.ui.labelWarningInfo.setStyleSheet(
...
if HAS_QTWEBENGINE:
    ...
    class RestrictedWebPage(QWebEnginePage):
        """A class that follows links to LabRPS wiki pages, but opens all other clicked links in the system web browser"""
        ...
        def javaScriptConsoleMessage(self, level, message, lineNumber, sourceID):
            ...
            if show_javascript_console_output:
                ...
                if level == QWebEnginePage.InfoMessageLevel:
                    LabRPS.Console.PrintMessage(f"{tag} {lineNumber}: {message}\n")
                elif level == QWebEnginePage.WarningMessageLevel:
                    LabRPS.Console.PrintWarning(f"{tag} {lineNumber}: {message}\n")
                elif level == QWebEnginePage.ErrorMessageLevel:
                    LabRPS.Console.PrintError(f"{tag} {lineNumber}: {message}\n")
