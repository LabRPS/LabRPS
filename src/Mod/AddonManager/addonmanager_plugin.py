...
#  @package AddonManager_plugin
#  \ingroup ADDONMANAGER
#  \brief Unified handler for LabRPS plugins that can be obtained from
#  different sources
#  @{


class Plugin(object):
    """This class provides a unified way to handle plugins coming from different sources"""
    ...
    def from_cache(self, cache_dict: Dict):
        instance = Plugin(cache_dict["name"])
    ...
    def filename(self):
        ...
        return (self.name + ".txt").replace(" ", "_")

    def is_installed(self):
        if self.on_git and not self.src_filename:
            return False
        print(os.path.join(LabRPS.getUserPluginDir(True), self.filename))
        return os.path.exists(
            os.path.join(LabRPS.getUserPluginDir(True), self.filename)
        ) or os.path.exists(
            os.path.join(LabRPS.getUserPluginDir(True), "Plugin_" + self.filename)
        )
    ...
    def fill_details_from_code(self, code: str) -> None:
        ...
        string_search_mapping = {
            ...
            "__icon__": "icon",
            "__xpm__": "xpm",
            "__phenomenon__": "phenomenon",
            "__rpsversion__": "rpsversion",
            ...
            for key, value in string_search_mapping.items():
                if lowercase_line.startswith(key):
                    ...
                    if match and '"""' not in after_equals:
                        ...
                        # Plugin authors are supposed to be providing strings here, but in some
                        ...
                        if key == "__version__":
                            if "__date__" in after_equals.lower():
                                LabRPS.Console.PrintLog(
                                    translate(
                                        "AddonsInstaller",
                                        "In plugin {}, string literal not found for {} element. Guessing at intent and using string from date element.",
                                ...
                                LabRPS.Console.PrintLog(
                                    translate(
                                        "AddonsInstaller",
                                        "In plugin {}, string literal not found for {} element. Guessing at intent and using string representation of contents.",
                                    ).format(self.name, key)
                                    + "\n"
                                )
                                ...
                                while True:
                                    ...
                                    if not line:
                                        LabRPS.Console.PrintError(
                                            translate(
                                                "AddonsInstaller",
                                                "Syntax error while reading {} from plugin {}",
                        ...
                        LabRPS.Console.PrintError(line + "\n")
                        continue
    ...
    def fill_details_from_wiki(self, url):
        ...
        if not p:
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Unable to open plugin wiki page at {}",
                ).format(url)
                + "\n"
            )
            return
        p = p.data().decode("utf8")
        # check if the plugin page has its code hosted elsewhere, download if
        # needed
        if "rawcodeurl" in p:
            ...
            if self.raw_code_url:
                ...
                if not u2:
                    LabRPS.Console.PrintWarning(
                        translate(
                            "AddonsInstaller",
                            "Unable to open plugin code URL {}",
        ...
        if not code:
            ...
            if code:
                ...
                LabRPS.Console.PrintWarning(
                    translate(
                        "AddonsInstaller", "Unable to fetch the code of this plugin."
                    )
                    + "\n"
                )

        desc = re.findall(
            r"<td class=\"ctEven left plugin-description\">(.*?)</td>",
            p.replace("\n", " "),
        )
        if desc:
            desc = desc[0]
            ...
            LabRPS.Console.PrintWarning(
                translate(
                    "AddonsInstaller",
                    "Unable to retrieve a description from the wiki for plugin {}",
                ).format(self.name)
                + "\n"
            )
    ...
    def clean_icon(self):
        if self.icon.startswith("http://") or self.icon.startswith("https://"):
            LabRPS.Console.PrintLog(
                f"Attempting to fetch plugin icon from {self.icon}\n"
            )
            p = NetworkManager.AM_NETWORK_MANAGER.blocking_get(self.icon)
            if p:
                cache_path = LabRPS.getUserCachePath()
                am_path = os.path.join(cache_path, "AddonManager", "PluginIcons")
                ...
                LabRPS.Console.PrintLog(
                    f"PLUGIN DEVELOPER WARNING: failed to download icon from {self.icon} for plugin {self.name}\n"
                )
                self.icon = ""
    ...
    def install(self, plugin_dir: str) -> Tuple[bool, List[str]]:
        """Install a plugin and all its related files
        Returns True if the plugin was installed correctly.
        Parameters
        ----------
        - plugin_dir: the directory to install into
        """
        ...
        if not os.path.isdir(plugin_dir):
            try:
                ...
                return False, [f"Failed to create {plugin_dir}"]
        plugin_path = os.path.join(plugin_dir, self.filename)
        try:
            with codecs.open(plugin_path, "w", "utf-8") as pluginfile:
                pluginfile.write(self.code)
        except IOError:
            return False, [f"Failed to write {plugin_path}"]
        ...
        if self.icon:
            if os.path.isabs(self.icon):
                dst_file = os.path.normpath(
                    os.path.join(plugin_dir, os.path.basename(self.icon))
                )
                try:
        ...
        for other_file in self.other_files:
            ...
            if os.path.isabs(other_file):
                ...
                dst_dir = os.path.join(plugin_dir, os.path.dirname(other_file))
            ...
            if os.path.isabs(other_file):
                src_file = other_file
                dst_file = os.path.normpath(
                    os.path.join(plugin_dir, os.path.basename(other_file))
                )
            else:
                src_file = os.path.normpath(os.path.join(base_dir, other_file))
                dst_file = os.path.normpath(os.path.join(plugin_dir, other_file))
            if not os.path.isfile(src_file):
                ...
                if self.raw_code_url:
                    ...
                    LabRPS.Console.PrintLog(f"Attempting to fetch {fetch_url}...\n")
                    ...
                    if p:
                        ...
                        LabRPS.Console.PrintWarning(
                            translate(
                                "AddonsInstaller",
                                "Unable to fetch plugin-specified file {} from {}",
                    ...
                    warnings.append(
                        translate(
                            "AddonsInstaller",
                            "Could not locate plugin-specified file {} (should have been at {})",
        ...
        LabRPS.Console.PrintLog(f"Plugin {self.name} was installed successfully.\n")
        return True, []

    def remove(self) -> bool:
        """Remove a plugin and all its related files

        Returns True if the plugin was removed correctly.
        """

        if not self.is_installed():
            # Plugin not installed, nothing to do.
            return True
        plugin_dir = LabRPS.getUserPluginDir(True)
        plugin_path = os.path.join(plugin_dir, self.filename)
        plugin_path_with_plugin_prefix = os.path.join(plugin_dir, "Plugin_" + self.filename)
        if os.path.exists(plugin_path):
            os.remove(plugin_path)
        elif os.path.exists(plugin_path_with_plugin_prefix):
            os.remove(plugin_path_with_plugin_prefix)
        # # Remove related files, which are supposed to be given relative to
        # # self.src_filename.
        # if self.xpm:
        #     xpm_file = os.path.join(plugin_dir, self.name + "_icon.xpm")
        #     if os.path.exists(xpm_file):
        #         os.remove(xpm_file)
        for other_file in self.other_files:
            if not other_file:
                continue
            LabRPS.Console.PrintMessage(f"{other_file}...")
            dst_file = os.path.join(plugin_dir, other_file)
            if not dst_file or not os.path.exists(dst_file):
                LabRPS.Console.PrintMessage(f"X\n")
                continue
            try:
                os.remove(dst_file)
        ...
        if os.path.isabs(self.icon):
            ...
            if os.path.exists(dst_file):
                try:
                    LabRPS.Console.PrintMessage(f"{os.path.basename(self.icon)}...")
                    os.remove(dst_file)
                    LabRPS.Console.PrintMessage("✓\n")
                except Exception:
                    LabRPS.Console.PrintMessage(f"?\n")
    ...
    def parse_wiki_page_for_icon(self, page_data: str) -> None:
        ...
        # <a rel="nofollow" class="external text" href="https://www.labrps.com/wiki/images/f/f5/Plugin_3D_Parametric_Curve.png">ToolBar Icon</a>
        ...
        if wiki_icon.startswith("http"):
            ...
            LabRPS.Console.PrintLog(
                f"Found a File: link for plugin {self.name} -- {wiki_icon}\n"
            )
            ...
            if p:
                ...
                for line in lines:
                    if trigger:
                        ...
                        if match:
                            wiki_icon = match.group(1)
                            self.icon = "https://www.wiki.labrps.com/" + wiki_icon
                            return
