...
#include <Mod/SeismicLabPlugins/SeismicLabPlugin/ui_DlgVerticalLocationDistribution.h>
#include <Mod/SeismicLabPlugins/SeismicLabPlugin/WLVerticalUniformDistribution.h>
...
/* TRANSLATOR SeismicLabGui::DlgVerticalLocationDistribution */
...
void DlgVerticalLocationDistribution::accept()
{
    ...
    if(!doc)
	    return;
    SeismicLab::CWLVerticalUniformDistribution* activefeature = static_cast<SeismicLab::CWLVerticalUniformDistribution*>(doc->getObject(_featureName.c_str()));
    if (!activefeature)
        return;
