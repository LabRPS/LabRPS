...
DabaghiDerKiureghian::DabaghiDerKiureghian() {
    ...
    this->LinkToWiki.setValue("https://wiki.labrps.com/Plugin_SeismicLab_SeismicLabSmelt#Dabaghi_and_Der_Kiureghian");
...
bool DabaghiDerKiureghian::Simulate(const SeismicLabAPI::SeismicLabSimulationData& Data, cube& dPhenomenon) {
...
for (int ss = 1; ss <= sampleN && false == Data.isInterruptionRequested.getValue(); ss++) {
      auto test_model_factory =
          Factory<stochastic::StochasticModel, stochastic::FaultType, stochastic::SimulationType,
                  double, double, double, double, double, double, unsigned int, unsigned int, bool,
                  int>::instance()
              ->create("DabaghiDerKiureghianNFGM", std::move(faulting), std::move(simulation_type),
                       ...
                       std::move(theta_or_phi), std::move(num_sims), std::move(num_realizations),
