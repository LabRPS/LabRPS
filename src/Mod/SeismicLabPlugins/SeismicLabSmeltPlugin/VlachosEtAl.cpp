...
VlachosEtAl::VlachosEtAl() {
    ...
    this->LinkToWiki.setValue("https://wiki.labrps.com/Plugin_SeismicLab_SeismicLabSmelt#Vlachos_et_al_2018");
...
bool VlachosEtAl::Simulate(const SeismicLabAPI::SeismicLabSimulationData& Data, cube& dPhenomenon)
{
...
for (int ss = 1; ss <= sampleN && false == Data.isInterruptionRequested.getValue(); ss++) {
      auto test_model_factory =
          Factory<stochastic::StochasticModel, double, double, double, double, unsigned int,
                  unsigned int, int>::instance()
              ->create("VlachosSiteSpecificEQ", std::move(moment_magnitude),
                       std::move(rupture_dist), std::move(vs30), std::move(orientation),
                       std::move(num_spectra), std::move(num_sims), std::move(seed));
      ...
      int cols = min(Data.numberOfSpatialPosition.getValue(), num_sims);
      ...
      for (int j = 0; j < cols && false == Data.isInterruptionRequested.getValue(); j++) {
          ...
          int rows = min(Data.numberOfTimeIncrements.getValue(), num_time_steps);
          for (int i = 0; i < rows && false == Data.isInterruptionRequested.getValue();
               i++) {
              dPhenomenon(i, j + 1, ss-1) = run_data[i];
              dPhenomenon(i, 0, ss - 1) = dt * i;
          }
      }

      if (Data.numberOfSpatialPosition.getValue() < num_sims || Data.numberOfTimeIncrements.getValue() < num_time_steps) {
          Base::Console().Warning(
              "The entire result data couldn't be reported. Please make sure the number of spatial "
              "location is %d and the number of time increments is %d.\n",
