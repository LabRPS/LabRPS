...
#include "ui_DlgSeismicLabFeaturesCreation.h"
...
/* TRANSLATOR SeismicLabGui::DlgSeismicLabFeaturesCreation */

DlgSeismicLabFeaturesCreation::DlgSeismicLabFeaturesCreation(QWidget* parent, SeismicLabAPI::SeismicLabFeature* feature)
    : QWidget(parent), ui(new Ui_DlgSeismicLabFeaturesCreation)
{
	ui->setupUi(this);
	...
	std::list<std::string> simList = SeismicLabGui::SeismicLabSimulationObserver::instance()->simulations();
	...
	SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(simeName.toUtf8().constData()));
 ...
 */
DlgSeismicLabFeaturesCreation::~DlgSeismicLabFeaturesCreation()
{
}

void DlgSeismicLabFeaturesCreation::fillFeatureComboBox(SeismicLab::SeismicLabSimulation* sim, QComboBox* comboBox, int selectedFeatureType)
{
	comboBox->clear();
	if (sim)
	{
        QString featureGroup = ui->featureTypeComboBox->currentText();
        QStringList plugginGroupObject = sim->findAllPluggedSeismicLabFeatures(featureGroup);
		ui->featureComboBox->addItems(plugginGroupObject);
	}
}


void DlgSeismicLabFeaturesCreation::createDlgSeismicLabFeature(const QString& simulation)
{
    auto docApp = App::GetApplication().getActiveDocument();
    if (!docApp)
        return;

    docApp->openTransaction("Create SeismicLab Feature");

	//get the simulation for which this feature will created
	SeismicLab::SeismicLabSimulation* simParent = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(ui->SimulationComboBox->currentText().toUtf8().constData()));
	if (!simParent)
		return;
	...
	//create the feature
	SeismicLabAPI::SeismicLabFeature* newFeature = simParent->createFeature(type, ui->featureComboBox->currentText().toUtf8().constData(), ui->nameLineEdit->text().toUtf8().constData());

	if (!newFeature)
		return;

	//set the feature properties
	auto featureProperties = simParent->GetSeismicLabPluggedSeismicLabFeatureDescription(ui->featureTypeComboBox->currentText(), ui->featureComboBox->currentText());
...
void DlgSeismicLabFeaturesCreation::featureTypeComboBoxIndexChanged(int index)
{
	SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(ui->SimulationComboBox->currentText().toUtf8().constData()));
	if (!sim) {
		QMessageBox::warning(this, tr("Feature Creation"), tr("No active simulation"));
		return;
	}
...
void DlgSeismicLabFeaturesCreation::accept(const QString& placement)
{

}

void DlgSeismicLabFeaturesCreation::reject()
{
...
/* TRANSLATOR PartGui::TaskSeismicLabFeatures */

TaskSeismicLabFeatures::TaskSeismicLabFeatures()
{
	Gui::TaskView::TaskBox* taskbox;
	widget = new DlgSeismicLabFeaturesCreation();
...
QDialogButtonBox::StandardButtons TaskSeismicLabFeatures::getStandardButtons() const
{
	return QDialogButtonBox::Close |
		QDialogButtonBox::Ok;
}

void TaskSeismicLabFeatures::modifyStandardButtons(QDialogButtonBox* box)
{
	QPushButton* btn = box->button(QDialogButtonBox::Ok);
	btn->setText(QApplication::translate("SeismicLabGui::DlgSeismicLabFeaturesCreation", "&Create"));
}

bool TaskSeismicLabFeatures::accept()
{
	widget->createDlgSeismicLabFeature(QString::fromLatin1(""));
	return true;
}

bool TaskSeismicLabFeatures::reject()
{
	return true;
}
...
/* TRANSLATOR PartGui::TaskSeismicLabFeaturesEdit */

TaskSeismicLabFeaturesEdit::TaskSeismicLabFeaturesEdit(SeismicLabAPI::SeismicLabFeature* feature)
{
	// create and show dialog for the SeismicLabFeatures
	Gui::TaskView::TaskBox* taskbox;
	widget = new DlgSeismicLabFeaturesCreation(nullptr, feature);
	taskbox = new Gui::TaskView::TaskBox(QPixmap(), widget->windowTitle(), true, nullptr);
	taskbox->groupLayout()->addWidget(widget);
	Content.push_back(taskbox);
}

TaskSeismicLabFeaturesEdit::~TaskSeismicLabFeaturesEdit()
{
	// automatically deleted in the sub-class
}

QDialogButtonBox::StandardButtons TaskSeismicLabFeaturesEdit::getStandardButtons() const
{
	return QDialogButtonBox::Cancel /*|
		QDialogButtonBox::Ok*/;
}

bool TaskSeismicLabFeaturesEdit::accept()
{
	//cannot modify featues
	return true;
}

bool TaskSeismicLabFeaturesEdit::reject()
{
	widget->reject();
	return true;
}

#include "moc_DlgSeismicLabFeaturesCreation.cpp"
