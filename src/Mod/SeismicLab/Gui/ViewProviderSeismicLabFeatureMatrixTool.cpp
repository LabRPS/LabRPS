...
#include "ViewProviderSeismicLabFeatureMatrixToolPy.h"
...
#include <Mod/SeismicLabAPI/App/IrpsSLMatrixTool.h>
...
{
  sPixmap = ":/icons/SeismicLabFeatures/SeismicLab_Feature_MatrixToolObj.svg";
}
...
bool ViewProviderSeismicLabFeatureMatrixTool::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeismicLabFeatureMatrixTool::ActivateFeature()
{
...
bool ViewProviderSeismicLabFeatureMatrixTool::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeismicLabFeatureMatrixTool::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeismicLabFeatureMatrixTool::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    ...
    if (Gui::getMainWindow()->getAlphaPlot()->isActiveSubWindowAMatrix())
    {
        ...
        func->trigger(cvector, boost::bind(&ViewProviderSeismicLabFeatureMatrixTool::MatrixToolCompute, this));
    }

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeismicLabFeatureMatrixTool::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderSeismicLabFeatureMatrixTool::goToWiki, this));
}


bool ViewProviderSeismicLabFeatureMatrixTool::MatrixToolCompute()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::MatrixToolCompute);
    
}

bool ViewProviderSeismicLabFeatureMatrixTool::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seismicLabFeatureInitalSetting(SeismicLab::SeismicLabUtils::objGroupMatrixTool, QString::fromUtf8(sim->getSimulationData()->matrixTool.getValue()));
     
    return true;
}

PyObject* ViewProviderSeismicLabFeatureMatrixTool::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeismicLabFeatureMatrixToolPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeismicLabFeatureMatrixTool::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
