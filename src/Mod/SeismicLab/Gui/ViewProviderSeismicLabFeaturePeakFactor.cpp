...
#include "ViewProviderSeismicLabFeaturePeakFactorPy.h"
...
#include <Mod/SeismicLabAPI/App/IrpsSLPeakFactor.h>
...
{
  sPixmap = ":/icons/SeismicLabFeatures/SeismicLab_Feature_PeakFactorObj.svg";
}
...
bool ViewProviderSeismicLabFeaturePeakFactor::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeismicLabFeaturePeakFactor::ActivateFeature()
{
auto doc = App::GetApplication().getActiveDocument();
    ...
    parentSim->PeakFactor.setValue(obj->getNameInDocument());
    return true;
}

bool ViewProviderSeismicLabFeaturePeakFactor::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeismicLabFeaturePeakFactor::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeismicLabFeaturePeakFactor::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
  
    QAction* cvalue = menu->addAction(QObject::tr("ComputePeakFactorValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeismicLabFeaturePeakFactor::computePeakFactorValue, this));

     QAction* cvector = menu->addAction(QObject::tr("ComputePeakFactorVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderSeismicLabFeaturePeakFactor::computePeakFactorVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputePeakFactorVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderSeismicLabFeaturePeakFactor::computePeakFactorVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeismicLabFeaturePeakFactor::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderSeismicLabFeaturePeakFactor::goToWiki, this));
}


bool ViewProviderSeismicLabFeaturePeakFactor::computePeakFactorValue()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputePeakFactorValue);
    
}

bool ViewProviderSeismicLabFeaturePeakFactor::computePeakFactorVectorP()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputePeakFactorVectorP);
    
}

bool ViewProviderSeismicLabFeaturePeakFactor::computePeakFactorVectorT()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputePeakFactorVectorT);
    
}

bool ViewProviderSeismicLabFeaturePeakFactor::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seismicLabFeatureInitalSetting(SeismicLab::SeismicLabUtils::objGroupPeakFactor, QString::fromUtf8(sim->getSimulationData()->peakFactor.getValue()));
     
    return true;
}

PyObject* ViewProviderSeismicLabFeaturePeakFactor::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeismicLabFeaturePeakFactorPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeismicLabFeaturePeakFactor::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
