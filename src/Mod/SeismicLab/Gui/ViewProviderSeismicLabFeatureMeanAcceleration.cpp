...
#include <Mod/SeismicLabAPI/App/IrpsSLMeanAcceleration.h>
...
{
  sPixmap = ":/icons/SeismicLabFeatures/SeismicLab_Feature_MeanObj.svg";
}
...
bool ViewProviderSeismicLabFeatureMeanAcceleration::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeismicLabFeatureMeanAcceleration::ActivateFeature()
{
...
bool ViewProviderSeismicLabFeatureMeanAcceleration::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeismicLabFeatureMeanAcceleration::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeismicLabFeatureMeanAcceleration::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
    QAction* cvalue = menu->addAction(QObject::tr("ComputeMeanAccelerationValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeismicLabFeatureMeanAcceleration::computeMeanAccelerationValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeMeanAccelerationVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderSeismicLabFeatureMeanAcceleration::computeMeanAccelerationVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputeMeanAccelerationVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderSeismicLabFeatureMeanAcceleration::computeMeanAccelerationVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeismicLabFeatureMeanAcceleration::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderSeismicLabFeatureMeanAcceleration::goToWiki, this));
}

bool ViewProviderSeismicLabFeatureMeanAcceleration::computeMeanAccelerationValue()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputeMeanAccelerationValue);
        
}

bool ViewProviderSeismicLabFeatureMeanAcceleration::computeMeanAccelerationVectorT()
{
     
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputeMeanAccelerationVectorT);
    
}

bool ViewProviderSeismicLabFeatureMeanAcceleration::computeMeanAccelerationVectorP()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputeMeanAccelerationVectorP);
    
}

bool ViewProviderSeismicLabFeatureMeanAcceleration::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seismicLabFeatureInitalSetting(SeismicLab::SeismicLabUtils::objGroupMeanAccelerationProfile, QString::fromUtf8(sim->getSimulationData()->meanFunction.getValue()));
     
    return true;
}

void ViewProviderSeismicLabFeatureMeanAcceleration::onChanged(const App::Property* prop)
{
    ViewProviderDocumentObject::onChanged(prop);
}

PyObject* ViewProviderSeismicLabFeatureMeanAcceleration::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeismicLabFeatureMeanAccelerationPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeismicLabFeatureMeanAcceleration::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
