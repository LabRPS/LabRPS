...
#include "ViewProviderSeismicLabFeatureProbabilityDensityFunctionPy.h"
...
#include <Mod/SeismicLabAPI/App/IrpsSLProbabilityDensityFunction.h>
...
bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::ActivateFeature()
{
...
bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeismicLabFeatureProbabilityDensityFunction::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeismicLabFeatureProbabilityDensityFunction::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
   
    QAction* cvalue = menu->addAction(QObject::tr("ComputePDFValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeismicLabFeatureProbabilityDensityFunction::computePDFValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputePDFVectorX"));
    func->trigger(cvector, boost::bind(&ViewProviderSeismicLabFeatureProbabilityDensityFunction::computePDFVectorX, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeismicLabFeatureProbabilityDensityFunction::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderSeismicLabFeatureProbabilityDensityFunction::goToWiki, this));
}

bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::computePDFVectorX()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputePDFVectorX);
    
}

bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::computePDFValue()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputePDFValue);
    
}

bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seismicLabFeatureInitalSetting(SeismicLab::SeismicLabUtils::objGroupProbabilityDensityFunction, QString::fromUtf8(sim->getSimulationData()->probabilityDensityFunction.getValue()));
     
    return true;
}

PyObject* ViewProviderSeismicLabFeatureProbabilityDensityFunction::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeismicLabFeatureProbabilityDensityFunctionPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeismicLabFeatureProbabilityDensityFunction::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
