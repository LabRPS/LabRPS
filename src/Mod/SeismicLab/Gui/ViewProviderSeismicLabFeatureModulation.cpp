...
#include "ViewProviderSeismicLabFeatureModulationPy.h"
...
PROPERTY_SOURCE(SeismicLabGui::ViewProviderSeismicLabFeatureModulation, Gui::ViewProviderRPSFeature)


ViewProviderSeismicLabFeatureModulation::ViewProviderSeismicLabFeatureModulation()
{
  sPixmap = ":/icons/SeismicLabFeatures/SeismicLab_Feature_ModulationObj.svg";
}
...
bool ViewProviderSeismicLabFeatureModulation::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeismicLabFeatureModulation::ActivateFeature()
{
...
bool ViewProviderSeismicLabFeatureModulation::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeismicLabFeatureModulation::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeismicLabFeatureModulation::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
 
    QAction* cvalue = menu->addAction(QObject::tr("ComputeModulationValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeismicLabFeatureModulation::computeModulationValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeModulationVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderSeismicLabFeatureModulation::computeModulationVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputeModulationVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderSeismicLabFeatureModulation::computeModulationVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeismicLabFeatureModulation::OnInitialSetting, this));

    QAction* act = menu->addAction(QObject::tr("Activate Feature"));
    func->trigger(act, boost::bind(&ViewProviderSeismicLabFeatureModulation::ActivateFeature, this));

    QAction* gtp = menu->addAction(QObject::tr("Go to Publication"));
    func->trigger(gtp, boost::bind(&ViewProviderSeismicLabFeatureModulation::goToPublication, this));

    QAction* gtw = menu->addAction(QObject::tr("Go to Wiki"));
    func->trigger(gtw, boost::bind(&ViewProviderSeismicLabFeatureModulation::goToWiki, this));
}


bool ViewProviderSeismicLabFeatureModulation::computeModulationValue()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputeModulationValue);

}

bool ViewProviderSeismicLabFeatureModulation::computeModulationVectorT()
{
    
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputeModulationVectorT);

}

bool ViewProviderSeismicLabFeatureModulation::computeModulationVectorP()
{   
    return runFeatureMethod(SeismicLab::SeismicLabUtils::ComputeModulationVectorP);
    
}

bool ViewProviderSeismicLabFeatureModulation::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seismicLabFeatureInitalSetting(SeismicLab::SeismicLabUtils::objGroupModulationFunction, QString::fromUtf8(sim->getSimulationData()->modulationFunction.getValue()));
     
    return true;
}

PyObject* ViewProviderSeismicLabFeatureModulation::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeismicLabFeatureModulationPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeismicLabFeatureModulation::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
