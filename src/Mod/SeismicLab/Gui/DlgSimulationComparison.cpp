...
/* TRANSLATOR SeismicLabGui::DlgSimulationComparison */

DlgSimulationComparison::DlgSimulationComparison(SeismicLab::SeismicLabSimulationComparison* comparison, QWidget* parent)
    : QWidget(parent), ui(new Ui_DlgSimulationComparison), _comparison(comparison)
{
	...
	SeismicLab::SeismicLabSimulation* sim1 = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(simeName1.toUtf8().constData()));

	QString simeName2 = ui->comboBox_Sim_Candidate2->currentText();
	SeismicLab::SeismicLabSimulation* sim2 = static_cast<SeismicLab::SeismicLabSimulation*>(SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(simeName2.toUtf8().constData()));
    
	std::vector<QString> groups = sim1->getSeismicLabPluggableFeatures();
...
{
	SeismicLab::SeismicLabSimulation* sim1 = static_cast<SeismicLab::SeismicLabSimulation*> (SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(ui->comboBox_Sim_Candidate1->currentText().toUtf8().constData()));
	SeismicLab::SeismicLabSimulation* sim2 = static_cast<SeismicLab::SeismicLabSimulation*> (SeismicLabGui::SeismicLabSimulationObserver::instance()->getSimulation(ui->comboBox_Sim_Candidate2->currentText().toUtf8().constData()));
...
void DlgSimulationComparison::fillPluggedFeatureComboBox(SeismicLab::SeismicLabSimulation* sim, QComboBox* comboBox)
{
	comboBox->clear();
	if (sim)
	{
		QString seismicLabGroup = ui->comboBox_FeatureGroup->currentText();
		QStringList plugginGroupObject = sim->findAllPluggedSeismicLabFeatures(seismicLabGroup);
        comboBox->addItems(plugginGroupObject);

	}
}

void DlgSimulationComparison::fillFeatureMethodComboBoxCandidate(SeismicLab::SeismicLabSimulation* sim, QComboBox* comboBox)
{
	comboBox->clear();
	if (sim)
	{
		QString seismicLabGroup = ui->comboBox_FeatureGroup->currentText();
		QStringList plugginGroupObject = sim->findAllFeatureMethods(seismicLabGroup);
        comboBox->addItems(plugginGroupObject);

	}
}

void DlgSimulationComparison::fillFeatureComboBox(SeismicLab::SeismicLabSimulation* sim, QComboBox* comboBox, QString group, QString type)
{
	comboBox->clear();
	if (sim)
	{

		QStringList plugginGroupObject = sim->findAllSeismicLabFeaturesByGoupAndType(group, type);
        comboBox->addItems(plugginGroupObject);
...
void DlgSimulationComparison::modifyStandardButtons(QDialogButtonBox* box)
{
    ...
    btn->setText(QApplication::translate("SeismicLabGui::DlgSimulationComparison", "&Create"));
}
...
void DlgSimulationComparison::accept()
{
    ...
    auto obj = doc->addObject("SeismicLab::SeismicLabSimulationComparison", uniqueName.c_str());
	doc->commitTransaction();
	SeismicLab::SeismicLabSimulationComparison* comparison = static_cast<SeismicLab::SeismicLabSimulationComparison*>(obj);
...
DlgSimulationComparisonEdit::DlgSimulationComparisonEdit(SeismicLab::SeismicLabSimulationComparison* comparison)
{
...
void DlgSimulationComparisonEdit::modifyStandardButtons(QDialogButtonBox* box)
{
	...
	{
    QPushButton* btn = box->button(QDialogButtonBox::Ok);
    btn->setText(QApplication::translate("SeismicLabGui::DlgSimulationComparisonEdit", "&Save"));
	}
	else
	{
    QPushButton* btn = box->button(QDialogButtonBox::Ok);
    btn->setText(QApplication::translate("SeismicLabGui::DlgSimulationComparisonEdit", "&Create"));
	}
