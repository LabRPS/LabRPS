...
public:
    ...
    Module() : Py::ExtensionModule<Module>("SeismicLab")
    {
        ...
        initialize("This module is the SeismicLab module."); // register with Python
    }
...
private:
    ...
    Py::Object getSimulation(const Py::Tuple& args)
    {
        ...
        try {
            ...
            SeismicLab::SeismicLabSimulation* sim = static_cast<SeismicLab::SeismicLabSimulation*>(doc->getObject(psName));
            if (!sim)
                throw Py::RuntimeError("Simulation not found.");

            if (!sim->getTypeId().isDerivedFrom(SeismicLab::SeismicLabSimulation::getClassTypeId()))
                throw Py::RuntimeError("Simulation not found.");
    ...
    try {
        ...
        SeismicLab::SeismicLabSimulation* parentSimulation = static_cast<SeismicLab::SeismicLabSimulation*>(doc->getObject(simulationName));
        if (!parentSimulation)
           throw Py::RuntimeError("Parent simulation not found.");

        if (!parentSimulation->getTypeId().isDerivedFrom(SeismicLab::SeismicLabSimulation::getClassTypeId())) 
              throw Py::RuntimeError("Parent simulation not found.");
    ...
    QString phenomenon = SeismicLabUtils::rpsPhenomenonWindVelocity;
