...
class SeismicLabExamples(QtGui.QWidget):
    def __init__(self):
        super(SeismicLabExamples, self).__init__()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        files_info = {}
        self.files_name = {}
        seismiclabfeatures = set()

        for f in files:
            module = import_module("seismiclabexamples." + f)
            if hasattr(module, "get_information"):
                info = getattr(module, "get_information")()
                files_info[f] = info
                self.files_name[info["name"]] = f
                seismiclabfeatures.add(info["seismiclabfeature"])
                                
        seismiclabfeatures = sorted(seismiclabfeatures)
        ...
        all_seismiclabfeatures = QtGui.QTreeWidgetItem(self.view, ["SeismicLabFeatures"])
        for feat in seismiclabfeatures:
            feat_item = QtGui.QTreeWidgetItem(all_seismiclabfeatures, [feat])
            for example, info in files_info.items():
                if info["seismiclabfeature"] == feat:
                    QtGui.QTreeWidgetItem(feat_item, [info["name"]])
        self.view.addTopLevelItem(all_seismiclabfeatures)
    ...
    def accept(self):
        ...
        LabRPSGui.doCommand("from seismiclabexamples.{}  import setup".format(str(example)))
    ...
    #    # if done this way the Python commands are printed in Python console
    #    LabRPSGui.doCommand("from seismiclabexamples.manager import run_example")
...
def show_examplegui():
    ...
    example_widget = QtGui.QDockWidget("SEISMICLAB Examples", mw)
    example_widget.setWidget(SeismicLabExamples())
