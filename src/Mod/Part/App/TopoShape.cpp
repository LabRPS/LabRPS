...
void TopoShape::exportStep(const char *filename) const
{
    try {
        ...
        // https://forum.labrpsweb.org/viewtopic.php?f=8&t=52967
        //makeHeader.SetName(new TCollection_HAsciiString((Standard_CString)(encodeFilename(filename).c_str())));
        makeHeader.SetAuthorValue (1, new TCollection_HAsciiString("LabRPS"));
        makeHeader.SetOrganizationValue (1, new TCollection_HAsciiString("LabRPS"));
        makeHeader.SetOriginatingSystem(new TCollection_HAsciiString("LabRPS"));
        makeHeader.SetDescriptionValue(1, new TCollection_HAsciiString("LabRPS Model"));
...
{
    // https://forum.labrpsweb.org/viewtopic.php?f=3&t=1445
    // https://forum.labrpsweb.org/viewtopic.php?f=3&t=52719
#if 1
    // Build a torus
    gp_Circ circle;
...
TopoDS_Shape TopoShape::makeOffset2D(double offset, short joinType, bool fill, bool allowOpenResult, bool intersection) const
{
    ...
    if(shapesToProcess.size() > 0) {
        ...
        if (fabs(offset) > Precision::Confusion()) {
            ...
            // http://www.labrpsweb.org/tracker/view.php?id=2699
