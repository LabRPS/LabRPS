...
class ViewProviderTube:
    ...
    def setupContextMenu(self, viewObject, menu):
        action = menu.addAction(LabRPS.Qt.translate("QObject", "Edit %1").replace("%1", viewObject.Object.Label))
    ...
    def startDefaultEditMode(self, viewObject):
        ...
        if not document.HasPendingTransaction:
            text = LabRPS.Qt.translate("QObject", "Edit %1").replace("%1", viewObject.Object.Label)
            document.openTransaction(text)
        viewObject.Document.setEdit(viewObject.Object, 0)

    def setEdit(self, viewObject, mode):
        if mode == 0:
            LabRPSGui.Control.showDialog(TaskTubeUI(viewObject))
            return True

    def unsetEdit(self, viewObject, mode):
        if mode == 0:
            LabRPSGui.Control.closeDialog()
            return True
...
class TaskTubeUI:
    ...
    def __init__(self, viewObject):
        ...
        LabRPSGui.ExpressionBinding(self.form.tubeOuterRadius).bind(object,"OuterRadius")
        LabRPSGui.ExpressionBinding(self.form.tubeInnerRadius).bind(object,"InnerRadius")
        LabRPSGui.ExpressionBinding(self.form.tubeHeight).bind(object,"Height")
