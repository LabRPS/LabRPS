...
void CmdPartPointsFromMesh::activated(int iMsg)
{
    ...
    auto found = std::find_if(geoms.begin(), geoms.end(), [](App::DocumentObject* obj) {
        return Base::labrps_dynamic_cast<Part::Feature>(obj);
    });

    if (found != geoms.end()) {

        double defaultDistance = getDefaultDistance(Base::labrps_dynamic_cast<Part::Feature>(*found));
    ...
    for (std::vector<Gui::SelectionObject>::iterator it = objs.begin(); it != objs.end(); ++it) {
        try {
            ...
            doCommand(Doc,"\nsh = App.getDocument('%s').%s.Shape\n"
                          ...
                          "\t\tGui.ActiveDocument.%s.hide()\n"
                          "else:\n"
                          "\t\tLabRPS.Console.PrintError('Defeaturing failed\\n')",
                          it->getDocName(),
                          it->getFeatName(),
                          faces.c_str(),
                          it->getFeatName());
