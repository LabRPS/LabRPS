...
def getParamRefine():
    return LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Part/Boolean").GetBool("RefineModel")
...
def cmdCreateJoinFeature(name, mode):
    ...
    LabRPS.ActiveDocument.openTransaction("Create "+mode)
    LabRPSGui.addModule("BOPTools.JoinFeatures")
    LabRPSGui.doCommand("j = BOPTools.JoinFeatures.make{mode}(name='{name}')".format(mode=mode, name=name))
    if mode == "Embed" or mode == "Cutout":
        LabRPSGui.doCommand("j.Base = App.ActiveDocument."+sel[0].Object.Name)
        LabRPSGui.doCommand("j.Tool = App.ActiveDocument."+sel[1].Object.Name)
    elif mode == "Connect":
...
class CommandEmbed:
    ...
    def Activated(self):
        if len(LabRPSGui.Selection.getSelectionEx()) == 2:
...
def addCommands():
    LabRPSGui.addCommand('Part_JoinCutout', CommandCutout())
    LabRPSGui.addCommand('Part_JoinEmbed', CommandEmbed())
    LabRPSGui.addCommand('Part_JoinConnect', CommandConnect())
