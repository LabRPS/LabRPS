...
def makeBooleanFragments(name):
    ...
    obj = LabRPS.ActiveDocument.addObject("Part::FeaturePython", name)
...
class ViewProviderBooleanFragments:
    ...
    def onDelete(self, feature, subelements):
        try:
            ...
            LabRPS.Console.PrintError("Error in onDelete: " + str(err))
        return True
...
def cmdCreateBooleanFragmentsFeature(name, mode):
    ...
    LabRPS.ActiveDocument.openTransaction("Create Boolean Fragments")
    LabRPSGui.addModule("BOPTools.SplitFeatures")
    LabRPSGui.doCommand("j = BOPTools.SplitFeatures.makeBooleanFragments(name='{name}')".format(name=name))
    LabRPSGui.doCommand("j.Objects = {sel}".format(
       sel= "["  +  ", ".join(["App.ActiveDocument."+so.Object.Name for so in sel])  +  "]"
       ))
    LabRPSGui.doCommand("j.Mode = {mode}".format(mode= repr(mode)))

    try:
        LabRPSGui.doCommand("j.Proxy.execute(j)")
        LabRPSGui.doCommand("j.purgeTouched()")
    except Exception as err:
    ...
    LabRPSGui.doCommand("for obj in j.ViewObject.Proxy.claimChildren():\n"
                         "    obj.ViewObject.hide()")
...
class CommandBooleanFragments:
    ...
    def Activated(self):
        if len(LabRPSGui.Selection.getSelectionEx()) >= 1:
...
def cmdCreateSliceFeature(name, mode, transaction=True):
    ...
    sel = LabRPSGui.Selection.getSelectionEx()
    if transaction: LabRPS.ActiveDocument.openTransaction("Create Slice")
    LabRPSGui.addModule("BOPTools.SplitFeatures")
    LabRPSGui.doCommand("f = BOPTools.SplitFeatures.makeSlice(name='{name}')".format(name=name))
    LabRPSGui.doCommand("f.Base = {sel}[0]\n"
                         "f.Tools = {sel}[1:]".format(
       sel= "["  +  ", ".join(["App.ActiveDocument."+so.Object.Name for so in sel])  +  "]"
       ))
    LabRPSGui.doCommand("f.Mode = {mode}".format(mode= repr(mode)))
    ...
    try:
        ...
        if mb.clickedButton() is btnAbort:
            if transaction: LabRPS.ActiveDocument.abortTransaction()
            return False

    LabRPSGui.doCommand("for obj in f.ViewObject.Proxy.claimChildren():\n"
                         "    obj.ViewObject.hide()")

    if transaction: LabRPS.ActiveDocument.commitTransaction()
    return True

def cmdSliceApart():
    LabRPS.ActiveDocument.openTransaction("Slice apart")
    made = cmdCreateSliceFeature(name="Slice", mode="Split", transaction=False)
    
    if made:
        LabRPSGui.addModule("CompoundTools.Explode")
        LabRPSGui.doCommand("CompoundTools.Explode.explodeCompound(f)")
        LabRPSGui.doCommand("f.ViewObject.hide()")
        LabRPS.ActiveDocument.commitTransaction()
...
def cmdCreateXORFeature(name):
    ...
    sel = LabRPSGui.Selection.getSelectionEx()
    LabRPS.ActiveDocument.openTransaction("Create Boolean XOR")
    LabRPSGui.addModule("BOPTools.SplitFeatures")
    LabRPSGui.doCommand("j = BOPTools.SplitFeatures.makeXOR(name='{name}')".format(name=name))
    LabRPSGui.doCommand("j.Objects = {sel}".format(
       sel= "["  +  ", ".join(["App.ActiveDocument."+so.Object.Name for so in sel])  +  "]"
       ))
...
def addCommands():
    LabRPSGui.addCommand('Part_BooleanFragments',CommandBooleanFragments())
    LabRPSGui.addCommand('Part_Slice',CommandSlice())
    LabRPSGui.addCommand('Part_SliceApart',CommandSliceApart())
    LabRPSGui.addCommand('Part_XOR',CommandXOR())
