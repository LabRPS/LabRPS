...
CRPSMitsuyasuDirectionalSpreadingFunction::CRPSMitsuyasuDirectionalSpreadingFunction()
{
   ...
   this->LinkToWiki.setValue("https://wiki.labrps.com/Plugin_SeaLab#Mitsuyasu_Directional_Spreading_Function");
...
bool CRPSMitsuyasuDirectionalSpreadingFunction::ComputeDirectionalSpreadingFunctionValue(const SeaLabAPI::SeaLabSimulationData& Data, const Base::Vector3d& location, const double& dfrequency, const double& dDirection, double& dValue)
{
  ...
  dValue = directionalSpreadingFunction.computeSpreadingFunction(dDirection, dfrequency, ModalFrequency.getQuantityValue().getValueAs(Base::Quantity::Hertz), TenMeterHeightmeanWindSpeed.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond)) * ScaleCoefficient.getValue();
  return true;
}
