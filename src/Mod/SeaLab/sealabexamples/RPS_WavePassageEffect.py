...
def get_information():
    return {
        ...
        "sealabfeature": "WavePassageEffect",
    }
...
from sealabexamples.RPS_WavePassageEffect import setup
setup()
...
# init LabRPS document. New document will be created if the passed document is null.
    ...
    loc = SeaLabObjects.makeFeature("SimulationPoints", "Simulation", "Horizontal Distribution", "Location Distribution")
    ...
    # create new mean acceleration feature in the simulation called "Simulation"
    mean = SeaLabObjects.makeFeature("MeanAcceleration", "Simulation",  "Logarithmic Law Profile", "Mean Acceleration")
    
    # check if the created mean acceleration feature
    if not mean:
        LabRPS.Console.PrintError("The creation of the mean acceleratin was not successuful.\n")
        return None
    ...
    frequency = SeaLabObjects.makeFeature("Frequencies", "Simulation",  "Double Index Frequency Discretization", "Frequency Distribution")
    ...
    wavepassageEff = SeaLabObjects.makeFeature("WavePassageEffect", "Simulation", "Exponential Wave Passage effect", "Wave Passage Effect")
    
    # check if the created mean acceleration feature is ok
    ...
    if LabRPS.GuiUp and showGui:
        ...
        SeaLabGui.SeaLabPyTool.showArray(sim.getSimulationData().numberOfFrequency, 3, wpe12)
        SeaLabGui.SeaLabPyTool.showArray(sim.getSimulationData().numberOfFrequency, 3, wpe13)
        SeaLabGui.SeaLabPyTool.showArray(sim.getSimulationData().numberOfFrequency, 3, wpe23)
