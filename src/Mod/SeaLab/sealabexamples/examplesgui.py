...
class SeaLabExamples(QtGui.QWidget):
    def __init__(self):
        super(SeaLabExamples, self).__init__()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        for f in files:
            module = import_module("sealabexamples." + f)
            if hasattr(module, "get_information"):
                info = getattr(module, "get_information")()
                files_info[f] = info
                self.files_name[info["name"]] = f
                sealabfeatures.add(info["sealabfeature"])
                                
        sealabfeatures = sorted(sealabfeatures)
        ...
        all_sealabfeatures = QtGui.QTreeWidgetItem(self.view, ["SeaLabFeatures"])
        for feat in sealabfeatures:
            feat_item = QtGui.QTreeWidgetItem(all_sealabfeatures, [feat])
            for example, info in files_info.items():
                if info["sealabfeature"] == feat:
                    QtGui.QTreeWidgetItem(feat_item, [info["name"]])
        self.view.addTopLevelItem(all_sealabfeatures)
    ...
    def accept(self):
        ...
        LabRPSGui.doCommand("from sealabexamples.{}  import setup".format(str(example)))
    ...
    #    # if done this way the Python commands are printed in Python console
    #    LabRPSGui.doCommand("from sealabexamples.manager import run_example")
...
def show_examplegui():
    ...
    example_widget = QtGui.QDockWidget("SEALAB Examples", mw)
    example_widget.setWidget(SeaLabExamples())
