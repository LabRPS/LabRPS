...
public:
    ...
    Module() : Py::ExtensionModule<Module>("SeaLab")
    {
        ...
        initialize("This module is the SeaLab module."); // register with Python
    }
...
private:
    ...
    Py::Object getSimulation(const Py::Tuple& args)
    {
        ...
        try {
            ...
            SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(doc->getObject(psName));
            if (!sim)
                throw Py::RuntimeError("Simulation not found.");

            if (!sim->getTypeId().isDerivedFrom(SeaLab::SeaLabSimulation::getClassTypeId()))
                throw Py::RuntimeError("Simulation not found.");
    ...
    try {
        ...
        SeaLab::SeaLabSimulation* parentSimulation = static_cast<SeaLab::SeaLabSimulation*>(doc->getObject(simulationName));
        if (!parentSimulation)
           throw Py::RuntimeError("Parent simulation not found.");

        if (!parentSimulation->getTypeId().isDerivedFrom(SeaLab::SeaLabSimulation::getClassTypeId())) 
              throw Py::RuntimeError("Parent simulation not found.");
    ...
    QString phenomenon = SeaLabUtils::rpsPhenomenonWindVelocity;
