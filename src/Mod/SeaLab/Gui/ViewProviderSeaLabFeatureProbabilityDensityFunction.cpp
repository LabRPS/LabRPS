...
#include "ViewProviderSeaLabFeatureProbabilityDensityFunctionPy.h"
...
#include <Mod/SeaLabAPI/App/IrpsSeLProbabilityDensityFunction.h>
...
bool ViewProviderSeaLabFeatureProbabilityDensityFunction::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeaLabFeatureProbabilityDensityFunction::ActivateFeature()
{
...
bool ViewProviderSeaLabFeatureProbabilityDensityFunction::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeaLabFeatureProbabilityDensityFunction::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeaLabFeatureProbabilityDensityFunction::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
   
    QAction* cvalue = menu->addAction(QObject::tr("ComputePDFValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeaLabFeatureProbabilityDensityFunction::computePDFValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputePDFVectorX"));
    func->trigger(cvector, boost::bind(&ViewProviderSeaLabFeatureProbabilityDensityFunction::computePDFVectorX, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeaLabFeatureProbabilityDensityFunction::OnInitialSetting, this));
...
bool ViewProviderSeaLabFeatureProbabilityDensityFunction::computePDFVectorX()
{
    ...
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputePDFVectorX);
    
}

bool ViewProviderSeaLabFeatureProbabilityDensityFunction::computePDFValue()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputePDFValue);
    
}

bool ViewProviderSeaLabFeatureProbabilityDensityFunction::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seaLabFeatureInitalSetting(SeaLab::SeaLabUtils::objGroupProbabilityDensityFunction, QString::fromUtf8(sim->getSimulationData()->probabilityDensityFunction.getValue()));
     
    return true;
}

PyObject* ViewProviderSeaLabFeatureProbabilityDensityFunction::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeaLabFeatureProbabilityDensityFunctionPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeaLabFeatureProbabilityDensityFunction::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
