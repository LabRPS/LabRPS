...
#include "ViewProviderSeaLabFeatureMatrixToolPy.h"
...
#include <Mod/SeaLabAPI/App/IrpsSeLMatrixTool.h>
...
{
  sPixmap = ":/icons/SeaLabFeatures/SeaLab_Feature_MatrixToolObj.svg";
}
...
bool ViewProviderSeaLabFeatureMatrixTool::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeaLabFeatureMatrixTool::ActivateFeature()
{
...
bool ViewProviderSeaLabFeatureMatrixTool::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeaLabFeatureMatrixTool::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeaLabFeatureMatrixTool::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    ...
    if (Gui::getMainWindow()->getAlphaPlot()->isActiveSubWindowAMatrix())
    {
        ...
        func->trigger(cvector, boost::bind(&ViewProviderSeaLabFeatureMatrixTool::MatrixToolCompute, this));
    }

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeaLabFeatureMatrixTool::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderSeaLabFeatureMatrixTool::goToWiki, this));
}


bool ViewProviderSeaLabFeatureMatrixTool::MatrixToolCompute()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::MatrixToolCompute);
    
}

bool ViewProviderSeaLabFeatureMatrixTool::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seaLabFeatureInitalSetting(SeaLab::SeaLabUtils::objGroupMatrixTool, QString::fromUtf8(sim->getSimulationData()->matrixTool.getValue()));
     
    return true;
}

PyObject* ViewProviderSeaLabFeatureMatrixTool::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeaLabFeatureMatrixToolPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeaLabFeatureMatrixTool::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
