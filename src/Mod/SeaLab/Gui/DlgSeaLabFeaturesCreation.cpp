...
#include "ui_DlgSeaLabFeaturesCreation.h"
...
/* TRANSLATOR SeaLabGui::DlgSeaLabFeaturesCreation */

DlgSeaLabFeaturesCreation::DlgSeaLabFeaturesCreation(QWidget* parent, SeaLabAPI::SeaLabFeature* feature)
    : QWidget(parent), ui(new Ui_DlgSeaLabFeaturesCreation)
{
	ui->setupUi(this);
	...
	SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->getSimulation(simeName.toUtf8().constData()));
 ...
 */
DlgSeaLabFeaturesCreation::~DlgSeaLabFeaturesCreation()
{
}

void DlgSeaLabFeaturesCreation::fillFeatureComboBox(SeaLab::SeaLabSimulation* sim, QComboBox* comboBox, int selectedFeatureType)
{
	comboBox->clear();
	if (sim)
	{
        QString featureGroup = ui->featureTypeComboBox->currentText();
        QStringList plugginGroupObject = sim->findAllPluggedSeaLabFeatures(featureGroup);
		ui->featureComboBox->addItems(plugginGroupObject);
	}
}


void DlgSeaLabFeaturesCreation::createDlgSeaLabFeature(const QString& simulation)
{
    auto docApp = App::GetApplication().getActiveDocument();
    if (!docApp)
        return;

    docApp->openTransaction("Create SeaLab Feature");

	//get the simulation for which this feature will created
	SeaLab::SeaLabSimulation* simParent = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->getSimulation(ui->SimulationComboBox->currentText().toUtf8().constData()));
	if (!simParent)
		return;
	...
	//create the feature
	SeaLabAPI::SeaLabFeature* newFeature = simParent->createFeature(type, ui->featureComboBox->currentText().toUtf8().constData(), ui->nameLineEdit->text().toUtf8().constData());

	if (!newFeature)
		return;

	//set the feature properties
	auto featureProperties = simParent->GetSeaLabPluggedSeaLabFeatureDescription(ui->featureTypeComboBox->currentText(), ui->featureComboBox->currentText());
...
void DlgSeaLabFeaturesCreation::featureTypeComboBoxIndexChanged(int index)
{
	SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->getSimulation(ui->SimulationComboBox->currentText().toUtf8().constData()));
	if (!sim) {
		QMessageBox::warning(this, tr("Feature Creation"), tr("No active simulation"));
		return;
	}
...
void DlgSeaLabFeaturesCreation::accept(const QString& placement)
{

}

void DlgSeaLabFeaturesCreation::reject()
{
...
/* TRANSLATOR PartGui::TaskSeaLabFeatures */

TaskSeaLabFeatures::TaskSeaLabFeatures()
{
	Gui::TaskView::TaskBox* taskbox;
	widget = new DlgSeaLabFeaturesCreation();
...
QDialogButtonBox::StandardButtons TaskSeaLabFeatures::getStandardButtons() const
{
	return QDialogButtonBox::Close |
		QDialogButtonBox::Ok;
}

void TaskSeaLabFeatures::modifyStandardButtons(QDialogButtonBox* box)
{
	QPushButton* btn = box->button(QDialogButtonBox::Ok);
	btn->setText(QApplication::translate("SeaLabGui::DlgSeaLabFeaturesCreation", "&Create"));
}

bool TaskSeaLabFeatures::accept()
{
	widget->createDlgSeaLabFeature(QString::fromLatin1(""));
	return true;
}

bool TaskSeaLabFeatures::reject()
{
	return true;
}
...
/* TRANSLATOR PartGui::TaskSeaLabFeaturesEdit */

TaskSeaLabFeaturesEdit::TaskSeaLabFeaturesEdit(SeaLabAPI::SeaLabFeature* feature)
{
	// create and show dialog for the SeaLabFeatures
	Gui::TaskView::TaskBox* taskbox;
	widget = new DlgSeaLabFeaturesCreation(nullptr, feature);
	taskbox = new Gui::TaskView::TaskBox(QPixmap(), widget->windowTitle(), true, nullptr);
	taskbox->groupLayout()->addWidget(widget);
	Content.push_back(taskbox);
}

TaskSeaLabFeaturesEdit::~TaskSeaLabFeaturesEdit()
{
	// automatically deleted in the sub-class
}

QDialogButtonBox::StandardButtons TaskSeaLabFeaturesEdit::getStandardButtons() const
{
	return QDialogButtonBox::Cancel /*|
		QDialogButtonBox::Ok*/;
}

bool TaskSeaLabFeaturesEdit::accept()
{
	//cannot modify featues
	return true;
}

bool TaskSeaLabFeaturesEdit::reject()
{
	widget->reject();
	return true;
}

#include "moc_DlgSeaLabFeaturesCreation.cpp"
