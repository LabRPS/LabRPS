...
PROPERTY_SOURCE(SeaLabGui::ViewProviderSeaLabFeatureTableTool, Gui::ViewProviderRPSFeature)


ViewProviderSeaLabFeatureTableTool::ViewProviderSeaLabFeatureTableTool()
{
  sPixmap = ":/icons/SeaLabFeatures/SeaLab_Feature_TableToolObj.svg";
}
...
bool ViewProviderSeaLabFeatureTableTool::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeaLabFeatureTableTool::ActivateFeature()
{
...
bool ViewProviderSeaLabFeatureTableTool::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeaLabFeatureTableTool::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeaLabFeatureTableTool::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    ...
    if (Gui::getMainWindow()->getAlphaPlot()->isActiveSubWindowATable())
    {
        ...
        func->trigger(cvector, boost::bind(&ViewProviderSeaLabFeatureTableTool::tableToolCompute, this));
    }

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeaLabFeatureTableTool::OnInitialSetting, this));

    QAction* act = menu->addAction(QObject::tr("Activate Feature"));
    func->trigger(act, boost::bind(&ViewProviderSeaLabFeatureTableTool::ActivateFeature, this));

    QAction* gtp = menu->addAction(QObject::tr("Go to Publication"));
    func->trigger(gtp, boost::bind(&ViewProviderSeaLabFeatureTableTool::goToPublication, this));

    QAction* gtw = menu->addAction(QObject::tr("Go to Wiki"));
    func->trigger(gtw, boost::bind(&ViewProviderSeaLabFeatureTableTool::goToWiki, this));
}


bool ViewProviderSeaLabFeatureTableTool::tableToolCompute()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::TableToolCompute);
    
}

bool ViewProviderSeaLabFeatureTableTool::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seaLabFeatureInitalSetting(SeaLab::SeaLabUtils::objGroupTableTool, QString::fromUtf8(sim->getSimulationData()->tableTool.getValue()));

    return true;
}

PyObject* ViewProviderSeaLabFeatureTableTool::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeaLabFeatureTableToolPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeaLabFeatureTableTool::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
