...
#include <Mod/SeaLabAPI/App/IrpsSeLPeakFactor.h>
...
{
  sPixmap = ":/icons/SeaLabFeatures/SeaLab_Feature_PeakFactorObj.svg";
}
...
bool ViewProviderSeaLabFeaturePeakFactor::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeaLabFeaturePeakFactor::ActivateFeature()
{
...
bool ViewProviderSeaLabFeaturePeakFactor::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeaLabFeaturePeakFactor::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeaLabFeaturePeakFactor::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
  
    QAction* cvalue = menu->addAction(QObject::tr("ComputePeakFactorValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeaLabFeaturePeakFactor::computePeakFactorValue, this));

     QAction* cvector = menu->addAction(QObject::tr("ComputePeakFactorVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderSeaLabFeaturePeakFactor::computePeakFactorVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputePeakFactorVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderSeaLabFeaturePeakFactor::computePeakFactorVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeaLabFeaturePeakFactor::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderSeaLabFeaturePeakFactor::goToWiki, this));
}


bool ViewProviderSeaLabFeaturePeakFactor::computePeakFactorValue()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputePeakFactorValue);
    
}

bool ViewProviderSeaLabFeaturePeakFactor::computePeakFactorVectorP()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputePeakFactorVectorP);
    
}

bool ViewProviderSeaLabFeaturePeakFactor::computePeakFactorVectorT()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputePeakFactorVectorT);
    
}

bool ViewProviderSeaLabFeaturePeakFactor::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seaLabFeatureInitalSetting(SeaLab::SeaLabUtils::objGroupPeakFactor, QString::fromUtf8(sim->getSimulationData()->peakFactor.getValue()));
     
    return true;
}

PyObject* ViewProviderSeaLabFeaturePeakFactor::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeaLabFeaturePeakFactorPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeaLabFeaturePeakFactor::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
