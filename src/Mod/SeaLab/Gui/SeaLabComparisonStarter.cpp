...
#include "SeaLabComparisonStarter.h"
...
SeaLabComparisonStarter::SeaLabComparisonStarter(SeaLab::SeaLabSimulation* sim1,SeaLab::SeaLabSimulation* sim2, std::string comparisonName, QObject* parent)
    : m_sim1(sim1), m_sim2(sim2), m_comparisonName(comparisonName), QObject(parent)
{ 
}

SeaLabComparisonStarter::~SeaLabComparisonStarter()
{
    
}

RPSSeaLabSimulationWorker *SeaLabComparisonStarter::GetSeaLabSimulationWorker1()
{
    return simulationWorker1;
}

RPSSeaLabSimulationWorker* SeaLabComparisonStarter::GetSeaLabSimulationWorker2()
{
    return simulationWorker2;
}

QThread* SeaLabComparisonStarter::getSeaLabSimulationThread1()
{ 
    return simulationThread1; 
}

QThread* SeaLabComparisonStarter::getSeaLabSimulationThread2() 
{ 
    return simulationThread2; 
}

std::string SeaLabComparisonStarter::getComparisonName()
{ return m_comparisonName; }

QString SeaLabComparisonStarter::logSimulationInfo(bool status, const QString &name) {
    ...
    QString table;
    table = SeaLab::SeaLabUtils::makeHtmlTable(37, 3, false, profile);

    table = table.arg(tr("Computation Time"), QString::number(GetSeaLabSimulationWorker1()->getSimulationTime()/1000) + QString::fromLatin1(" seconds"), QString::number(GetSeaLabSimulationWorker2()->getSimulationTime()/1000) + QString::fromLatin1(" seconds"));
...
void SeaLabComparisonStarter::startSimulationWorker(QString function, const char* complexNumberDisplay)
{
    // create a worker
    simulationWorker1 = new RPSSeaLabSimulationWorker(m_sim1);
    simulationWorker2 = new RPSSeaLabSimulationWorker(m_sim2);
    ...
    connect(this, SIGNAL(stopped()), simulationWorker1, SLOT(RPSSeaLabSimulationWorker::stop()), Qt::DirectConnection);
    connect(this, SIGNAL(stopped()), simulationWorker2, SLOT(RPSSeaLabSimulationWorker::stop()), Qt::DirectConnection);
...
void SeaLabComparisonStarter::slotDisplayResultInTable(QString str, int what)
{
    if (what == 1)
    {
...
void SeaLabComparisonStarter::slotDisplayResultInMatrix(QString str, int what)
{
    if (what == 1)
    {
...
void SeaLabComparisonStarter::slotDisplayResultInTable1(QString str, int what)
{
    slotDisplayResultInTable(str, what);
}
void SeaLabComparisonStarter::slotDisplayResultInMatrix1(QString str, int what)
{
    slotDisplayResultInMatrix(str, what);
}
void SeaLabComparisonStarter::slotDisplayResultInTable2(QString str, int what)
{
    slotDisplayResultInTable(str, what);
}
void SeaLabComparisonStarter::slotDisplayResultInMatrix2(QString str, int what)
{
    slotDisplayResultInMatrix(str, what);
}

void SeaLabComparisonStarter::setComplexNumberDisplay(const char* displayType)
{
