...
#include "ViewProviderSeaLabFeatureSkewnessPy.h"
...
#include <Mod/SeaLabAPI/App/IrpsSeLSkewness.h>
...
PROPERTY_SOURCE(SeaLabGui::ViewProviderSeaLabFeatureSkewness, Gui::ViewProviderRPSFeature)


ViewProviderSeaLabFeatureSkewness::ViewProviderSeaLabFeatureSkewness()
{
  sPixmap = ":/icons/SeaLabFeatures/SeaLab_Feature_SkewnessObj.svg";
}
...
bool ViewProviderSeaLabFeatureSkewness::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderSeaLabFeatureSkewness::ActivateFeature()
{
...
bool ViewProviderSeaLabFeatureSkewness::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderSeaLabFeatureSkewness::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderSeaLabFeatureSkewness::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);

    QAction* cvalue = menu->addAction(QObject::tr("ComputeSkewnessValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderSeaLabFeatureSkewness::computeSkewnessValue, this));

    QAction* cvectorP = menu->addAction(QObject::tr("ComputeSkewnessVectorP"));
    func->trigger(cvectorP, boost::bind(&ViewProviderSeaLabFeatureSkewness::computeSkewnessVectorP, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeSkewnessVectorT"));
    func->trigger(cvector, boost::bind(&ViewProviderSeaLabFeatureSkewness::computeSkewnessVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderSeaLabFeatureSkewness::OnInitialSetting, this));

    QAction* act = menu->addAction(QObject::tr("Activate Feature"));
    func->trigger(act, boost::bind(&ViewProviderSeaLabFeatureSkewness::ActivateFeature, this));

    QAction* gtp = menu->addAction(QObject::tr("Go to Publication"));
    func->trigger(gtp, boost::bind(&ViewProviderSeaLabFeatureSkewness::goToPublication, this));

    QAction* gtw = menu->addAction(QObject::tr("Go to Wiki"));
    func->trigger(gtw, boost::bind(&ViewProviderSeaLabFeatureSkewness::goToWiki, this));
}


bool ViewProviderSeaLabFeatureSkewness::computeSkewnessValue()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputeSkewnessValue);
    
}

bool ViewProviderSeaLabFeatureSkewness::computeSkewnessVectorP()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputeSkewnessVectorP);
    
}

bool ViewProviderSeaLabFeatureSkewness::computeSkewnessVectorT()
{
    
    return runFeatureMethod(SeaLab::SeaLabUtils::ComputeSkewnessVectorT);
    
}

bool ViewProviderSeaLabFeatureSkewness::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->seaLabFeatureInitalSetting(SeaLab::SeaLabUtils::objGroupSkewness, QString::fromUtf8(sim->getSimulationData()->skewness.getValue()));
     
    return true;
}

PyObject* ViewProviderSeaLabFeatureSkewness::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderSeaLabFeatureSkewnessPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderSeaLabFeatureSkewness::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    SeaLab::SeaLabSimulation* sim = static_cast<SeaLab::SeaLabSimulation*>(SeaLabGui::SeaLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
