...
__author__ = "JAndersM"
__url__ = "http://www.labrpsweb.org/index-fr.html"
__version__ = "00.02"
__date__ = "03/05/2019"
...
if LabRPS.GuiUp:
    LabRPSGui.addCommand('Image_Scaling', _CommandImageScaling())
...
def cmdCreateImageScaling(name, trackers):
    ...
    class Ui_Dialog(object):
        def setupUi(self, Dialog):
            self.view = LabRPSGui.ActiveDocument.ActiveView
            self.stack = []
        ...
        def accept(self):
            ...
            try:
                try:
                    q = LabRPS.Units.parseQuantity(self.lineEdit.text())
                    d = q.Value
                    if q.Unit == LabRPS.Units.Unit(): # plain number
                        ok = True
                    elif q.Unit == LabRPS.Units.Length:
                        ok = True
                except Exception:
                    ok = False
                if not ok:
                ...
                LabRPS.Console.PrintMessage("Image: Scale="+str(s)+"\n")
                self.tracker.off()
                self.tracker.finalize()
                self.dialog.hide()
                LabRPSGui.SendMsgToActiveView("ViewFit")
    ...
    #Init        
    if LabRPSGui.ActiveDocument is not None:
        d = QtGui.QWidget()
        ui = Ui_Dialog()
        ui.setupUi(d)
        centerOnScreen (d)
    else:
        LabRPS.Console.PrintWarning("no document to work with\n")
