...
try:
    BaseError = LabRPS.Base.LabRPSError
...
def callopenscad(inputfilename,outputfilename=None, outputext='csg', keepname=False):
    ...
    def check_output2(*args, **kwargs):
        ...
        if stderrd.strip():
            LabRPS.Console.PrintWarning(stderrd+u'\n')
        if stdoutd.strip():
            LabRPS.Console.PrintMessage(stdoutd+u'\n')
            return stdoutd

    preferences = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
...
def call_openscad_with_pipes(input_filename, output_filename, output_extension, keep_name):
    ...
    # read or write access to the files, only the LabRPS process does. In the future
...
def reverseimporttypes():
    ...
    importtypes = {}
    import LabRPS
    for key,value in LabRPS.getImportType().items():
...
def fcsubmatrix(m):
    """Extracts the 3x3 Submatrix from a labrps Matrix Object
...
def decomposerotoinversion(m, precision=4):
    ...
    if rmat == [[-1,0,0],[0,1,0],[0,0,1]]:
        cmat.scale(-1,1,1)
        return m*cmat,LabRPS.Vector(1)
    elif rmat == [[1,0,0],[0,-1,0],[0,0,1]]:
        cmat.scale(1,-1,1)
        return m*cmat, LabRPS.Vector(0,1)
    elif rmat == [[1,0,0],[0,1,0],[0,0,-1]]:
        cmat.scale(1,1,-1)
        return m*cmat, LabRPS.Vector(0,0,1)
    else:
        cmat.scale(1,1,-1)
        return m*cmat, LabRPS.Vector(0,0,1)
...
-2.0, -1.0)
    ...
    bestrot = LabRPS.Rotation()
    dangle  = comparerotations(r1, bestrot)
    for axis in wkaxes():
        for angle in vangl:
            for axissign in (1.0,-1.0):
                r2=LabRPS.Rotation(axis*axissign, angle)
...
def process2D_ObjectsViaOpenSCAD(ObjList, Operation, doc=None):
    import LabRPS
    doc = doc or LabRPS.activeDocument()
...
def process_ObjectsViaOpenSCADShape(doc, children, name, maxmeshpoints=None):
    if all((not obj.Shape.isNull() and obj.Shape.Volume == 0) \
        ...
        import LabRPS
        LabRPS.Console.PrintError( translate('OpenSCAD',\
