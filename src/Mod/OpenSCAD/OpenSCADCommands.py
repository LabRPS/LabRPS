...
class ExplodeGroup:
    "Ungroup Objects"
    def IsActive(self):
        return LabRPSGui.Selection.countObjectsOfType('Part::Feature') > 0
    ...
    def Activated(self):
        def isdefault(shapecolor):
            ...
            scol=LabRPS.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned('DefaultShapeColor',0xccccccff)
        ...
        def explode(obj,color=True):
            if obj.isDerivedFrom('Part::Fuse') or \
                ...
                if plm.isNull() or all((len(oo.InList)==1 and \
                    ...
                    LabRPS.Console.PrintError(translate('OpenSCAD', 'Unable to explode %s') % obj.Name +u'\n')

        for obj in LabRPSGui.Selection.getSelection():
...
class ColorCodeShape:
    ...
    def Activated(self):
        ...
        selection=LabRPSGui.Selection.getSelectionEx()
        if len(selection) > 0:
            objs=[selobj.Object for selobj in selection]

        else:
            objs=LabRPS.ActiveDocument.Objects
...
class Edgestofaces:
    ...
    def Activated(self):
        ...
        Overlappingfaces(edgestofaces(edges,None)).makefeatures(LabRPS.ActiveDocument)
        for selobj in selection:
...
class MirrorMeshFeature:
    def IsActive(self):
        return LabRPSGui.Selection.countObjectsOfType('Mesh::Feature') > 0
...
class ExpandPlacements:
    ...
    def Activated(self):
        ...
        for selobj in LabRPSGui.Selection.getSelectionEx():
            expandplacements.expandplacements(selobj.Object,LabRPS.Placement())
        LabRPS.ActiveDocument.recompute()
    def GetResources(self):
...
class ReplaceObject:
    def IsActive(self):
        nobj = LabRPSGui.Selection.countObjectsOfType('Part::Feature')
        if nobj == 3: return True
        elif nobj == 2: return tuple((len(obj.InList)) for obj in \
                LabRPSGui.Selection.getSelection()) in ((0,1),(1,0))
        #else: return False

    def Activated(self):
        import replaceobj
        objs=LabRPSGui.Selection.getSelection()
        if len(objs)==3 or \
                tuple((len(obj.InList)) for obj in objs) in ((0,1),(1,0)):
            replaceobj.replaceobjfromselection(objs)
        else:
            LabRPS.Console.PrintError(translate('OpenSCAD', 'Please select 3 objects first')+u'\n')
    def GetResources(self):
...
class RemoveSubtree:
    def IsActive(self):
        return LabRPSGui.Selection.countObjectsOfType('Part::Feature') > 0
    def Activated(self):
        import OpenSCADUtils,LabRPSGui
        OpenSCADUtils.removesubtree(LabRPSGui.Selection.getSelection())
...
class AddSCADTask:
    ...
    def addelement(self):
        ...
        try:
            ...
            doc=LabRPS.activeDocument() or LabRPS.newDocument()
            if asmesh:
                import Mesh
            ...
            LabRPS.Console.PrintError(e.value)
...
class OpenSCADMeshBooleanTask:
    ...
    def doboolean(self):
        ...
        newmesh,objsused = meshoponobjs(opname,LabRPSGui.Selection.getSelection())
        if len(objsused) > 0:
            newmeshobj = LabRPS.activeDocument().addObject('Mesh::Feature',opname) #create a Feature for the result
...
class AddOpenSCADElement:
    def IsActive(self):
        return not LabRPSGui.Control.activeDialog()

    def Activated(self):
        panel = AddSCADTask()
        LabRPSGui.Control.showDialog(panel)
...
class OpenSCADMeshBoolean:
    def IsActive(self):
        return not LabRPSGui.Control.activeDialog() and \
            len(LabRPSGui.Selection.getSelection()) >= 1
...
class Hull:
    def IsActive(self):
        return len(LabRPSGui.Selection.getSelection()) >= 2
    ...
    def Activated(self):
        ...
        for selobj in selection:
            ...
            selobj.Object.ViewObject.hide()
        importCSG.process_ObjectsViaOpenSCAD(LabRPS.activeDocument(),objList,"hull")
        LabRPS.ActiveDocument.recompute()
    def GetResources(self):
...
class Minkowski:
    ...
    def Activated(self):
        ...
        for selobj in selection:
            objList.append(selobj.Object)
            selobj.Object.ViewObject.hide()
        importCSG.process_ObjectsViaOpenSCAD(LabRPS.activeDocument(),objList,"minkowski")
        LabRPS.ActiveDocument.recompute()
    def GetResources(self):
...
LabRPSGui.addCommand('OpenSCAD_ColorCodeShape',ColorCodeShape())
LabRPSGui.addCommand('OpenSCAD_ExplodeGroup',ExplodeGroup())
LabRPSGui.addCommand('OpenSCAD_Edgestofaces',Edgestofaces())
LabRPSGui.addCommand('OpenSCAD_RefineShapeFeature',RefineShapeFeature())
LabRPSGui.addCommand('OpenSCAD_MirrorMeshFeature',MirrorMeshFeature())
LabRPSGui.addCommand('OpenSCAD_ScaleMeshFeature',ScaleMeshFeature())
LabRPSGui.addCommand('OpenSCAD_ResizeMeshFeature',ResizeMeshFeature())
LabRPSGui.addCommand('OpenSCAD_IncreaseToleranceFeature',IncreaseToleranceFeature())
LabRPSGui.addCommand('OpenSCAD_ExpandPlacements',ExpandPlacements())
LabRPSGui.addCommand('OpenSCAD_ReplaceObject',ReplaceObject())
LabRPSGui.addCommand('OpenSCAD_RemoveSubtree',RemoveSubtree())
LabRPSGui.addCommand('OpenSCAD_AddOpenSCADElement',AddOpenSCADElement())
LabRPSGui.addCommand('OpenSCAD_MeshBoolean',OpenSCADMeshBoolean())
LabRPSGui.addCommand('OpenSCAD_Hull',Hull())
LabRPSGui.addCommand('OpenSCAD_Minkowski',Minkowski())
