...
class UserLabExamples(QtGui.QWidget):
    def __init__(self):
        super(UserLabExamples, self).__init__()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        for f in files:
            module = import_module("userlabexamples." + f)
            if hasattr(module, "get_information"):
                info = getattr(module, "get_information")()
                files_info[f] = info
                self.files_name[info["name"]] = f
                userlabfeatures.add(info["userlabfeature"])
                                
        userlabfeatures = sorted(userlabfeatures)
        ...
        all_userlabfeatures = QtGui.QTreeWidgetItem(self.view, ["UserLabFeatures"])
        for feat in userlabfeatures:
            feat_item = QtGui.QTreeWidgetItem(all_userlabfeatures, [feat])
            for example, info in files_info.items():
                if info["userlabfeature"] == feat:
                    QtGui.QTreeWidgetItem(feat_item, [info["name"]])
        self.view.addTopLevelItem(all_userlabfeatures)
    ...
    def accept(self):
        ...
        LabRPSGui.doCommand("from userlabexamples.{}  import setup".format(str(example)))
    ...
    #    # if done this way the Python commands are printed in Python console
    #    LabRPSGui.doCommand("from userlabexamples.manager import run_example")
...
def show_examplegui():
    ...
    example_widget = QtGui.QDockWidget("USERLAB Examples", mw)
    example_widget.setWidget(UserLabExamples())
