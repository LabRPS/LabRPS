...
/* TRANSLATOR UserLabGui::DlgSimulationComparison */

DlgSimulationComparison::DlgSimulationComparison(UserLab::UserLabSimulationComparison* comparison, QWidget* parent)
    : QWidget(parent), ui(new Ui_DlgSimulationComparison), _comparison(comparison)
{
	...
	UserLab::UserLabSimulation* sim1 = static_cast<UserLab::UserLabSimulation*>(UserLabGui::UserLabSimulationObserver::instance()->getSimulation(simeName1.toUtf8().constData()));

	QString simeName2 = ui->comboBox_Sim_Candidate2->currentText();
	UserLab::UserLabSimulation* sim2 = static_cast<UserLab::UserLabSimulation*>(UserLabGui::UserLabSimulationObserver::instance()->getSimulation(simeName2.toUtf8().constData()));
    
	std::vector<QString> groups = sim1->getUserLabPluggableFeatures();
...
{
	UserLab::UserLabSimulation* sim1 = static_cast<UserLab::UserLabSimulation*> (UserLabGui::UserLabSimulationObserver::instance()->getSimulation(ui->comboBox_Sim_Candidate1->currentText().toUtf8().constData()));
	UserLab::UserLabSimulation* sim2 = static_cast<UserLab::UserLabSimulation*> (UserLabGui::UserLabSimulationObserver::instance()->getSimulation(ui->comboBox_Sim_Candidate2->currentText().toUtf8().constData()));
...
void DlgSimulationComparison::fillPluggedFeatureComboBox(UserLab::UserLabSimulation* sim, QComboBox* comboBox)
{
	comboBox->clear();
	if (sim)
	{
		QString userGroup = ui->comboBox_FeatureGroup->currentText();
		QStringList plugginGroupObject = sim->findAllPluggedUserLabFeatures(userGroup);
        comboBox->addItems(plugginGroupObject);

	}
}

void DlgSimulationComparison::fillFeatureMethodComboBoxCandidate(UserLab::UserLabSimulation* sim, QComboBox* comboBox)
{
	comboBox->clear();
	if (sim)
	{
		QString userGroup = ui->comboBox_FeatureGroup->currentText();
		QStringList plugginGroupObject = sim->findAllFeatureMethods(userGroup);
        comboBox->addItems(plugginGroupObject);

	}
}

void DlgSimulationComparison::fillFeatureComboBox(UserLab::UserLabSimulation* sim, QComboBox* comboBox, QString group, QString type)
{
	comboBox->clear();
	if (sim)
	{

		QStringList plugginGroupObject = sim->findAllUserLabFeaturesByGoupAndType(group, type);
        comboBox->addItems(plugginGroupObject);
...
void DlgSimulationComparison::modifyStandardButtons(QDialogButtonBox* box)
{
    ...
    btn->setText(QApplication::translate("UserLabGui::DlgSimulationComparison", "&Create"));
}
...
void DlgSimulationComparison::accept()
{
    ...
    auto obj = doc->addObject("UserLab::UserLabSimulationComparison", uniqueName.c_str());
	doc->commitTransaction();
	UserLab::UserLabSimulationComparison* comparison = static_cast<UserLab::UserLabSimulationComparison*>(obj);
...
DlgSimulationComparisonEdit::DlgSimulationComparisonEdit(UserLab::UserLabSimulationComparison* comparison)
{
...
void DlgSimulationComparisonEdit::modifyStandardButtons(QDialogButtonBox* box)
{
	...
	{
    QPushButton* btn = box->button(QDialogButtonBox::Ok);
    btn->setText(QApplication::translate("UserLabGui::DlgSimulationComparisonEdit", "&Save"));
	}
	else
	{
    QPushButton* btn = box->button(QDialogButtonBox::Ok);
    btn->setText(QApplication::translate("UserLabGui::DlgSimulationComparisonEdit", "&Create"));
	}
