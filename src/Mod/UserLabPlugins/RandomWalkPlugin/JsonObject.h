...
public:
   ...
   */
    inline bool operator!=(const JsonObject& json_value) const { return !(*this == json_value); };
   ...
   */
    template<typename Tparam> bool add_value(const std::string& key, const Tparam& value);
   ...
   */
    template<typename Tparam> Tparam get_value(const std::string& key) const;
   ...
   */
    json get_library_json() const { return json_object_; };
