...
//phenomenon
QString UserDefinedPhenomenonLabUtils::rpsPhenomenonWindVelocity = QString::fromLatin1("Wind Velocity");

QString UserDefinedPhenomenonLabUtils::objGroupSimulationMethod =  QString::fromLatin1("Simulation Method");
QString UserDefinedPhenomenonLabUtils::docRPSFeatures =  QString::fromLatin1("RPS Features");
QString UserDefinedPhenomenonLabUtils::docSimulations =  QString::fromLatin1("Simulations");

QString UserDefinedPhenomenonLabUtils::Simulate = QString::fromLatin1("Simulate");
QString UserDefinedPhenomenonLabUtils::SimulateInLargeScaleMode = QString::fromLatin1("SimulateInLargeScaleMode");

bool UserDefinedPhenomenonLabUtils::getObjects(std::map<const std::string, std::string>& map, std::map<const std::string, std::string>& lstObject, const std::string& pluginName, const std::string& objectGroup)
{
    if (!map.empty())
    {
...
bool UserDefinedPhenomenonLabUtils::updateObjectsMap(std::map<const std::string, std::string>& map, const QString& lstObject)
{
    if (!map.empty())
    {
...
bool UserDefinedPhenomenonLabUtils::updateObjectToSkipMap(std::map<const std::string, std::string>& map, const QString name, const QString pluginName)
{
    map[name.toUtf8().constData()] = pluginName.toUtf8().constData();
    return true;
}

bool UserDefinedPhenomenonLabUtils::isThisObjectGroupPluggedInThisPlugin(const std::map<const std::string, std::string>& map, const QString& pluginName)
{
    if (!map.empty())
    {
        //go through the map
...
bool UserDefinedPhenomenonLabUtils::isThisObjectInstalled(std::map<const std::string, std::string>& map, QString& object)
{
    if (!map.empty())
    {
...
double UserDefinedPhenomenonLabUtils::rgbRandomSeed_ = static_cast<double>(rand()) / RAND_MAX;
int UserDefinedPhenomenonLabUtils::rgbCounter_ = 0;

int UserDefinedPhenomenonLabUtils::getWordSizeApp() {
...
QString UserDefinedPhenomenonLabUtils::getOperatingSystem() {
...
int UserDefinedPhenomenonLabUtils::getWordSizeOfOS() {
...
QString UserDefinedPhenomenonLabUtils::splitstring(const QString& string) {
...
QString UserDefinedPhenomenonLabUtils::joinstring(const QString& string) {
  if (string.isEmpty()) return string;
...
QString UserDefinedPhenomenonLabUtils::makeHtmlTable(const int row, const int column,
