...
public:
    ...
    Module() : Py::ExtensionModule<Module>("UserDefinedPhenomenonLab")
    {
        ...
        initialize("This module is the UserDefinedPhenomenonLab module."); // register with Python
    }
...
private:
    ...
    Py::Object getSimulation(const Py::Tuple& args)
    {
        ...
        try {
            ...
            UserDefinedPhenomenonLab::UserDefinedPhenomenonLabSimulation* sim = static_cast<UserDefinedPhenomenonLab::UserDefinedPhenomenonLabSimulation*>(doc->getObject(psName));
            if (!sim)
                throw Py::RuntimeError("Simulation not found.");

            if (!sim->getTypeId().isDerivedFrom(UserDefinedPhenomenonLab::UserDefinedPhenomenonLabSimulation::getClassTypeId()))
                throw Py::RuntimeError("Simulation not found.");
    ...
    try {
        ...
        UserDefinedPhenomenonLab::UserDefinedPhenomenonLabSimulation* parentSimulation = static_cast<UserDefinedPhenomenonLab::UserDefinedPhenomenonLabSimulation*>(doc->getObject(simulationName));
        if (!parentSimulation)
           throw Py::RuntimeError("Parent simulation not found.");

        if (!parentSimulation->getTypeId().isDerivedFrom(UserDefinedPhenomenonLab::UserDefinedPhenomenonLabSimulation::getClassTypeId())) 
              throw Py::RuntimeError("Parent simulation not found.");
    ...
    QString phenomenon = UserDefinedPhenomenonLabUtils::rpsPhenomenonWindVelocity;
