...
import LabRPS, LabRPSGui,inspect
...
def addCommand(name,cmdObject):
	...
	for i in range(len(list)-1):
		source += list[i+1][pos:]
	LabRPSGui.addCommand(name,cmdObject,source)
...
class TemplatePyMod_Cmd2:
    "Example command class"
    def Activated(self):
        d = LabRPS.ActiveDocument
        v = LabRPSGui.ActiveDocument.ActiveView
    ...
    def IsActive(self):
        if LabRPS.ActiveDocument is None:
            return False
        else:
            return True
...
class TemplatePyMod_Cmd3:
    ...
    def Activated(self):
        ...
        LabRPSGui.activateWorkbench("PythonQtWorkbench")
...
class SphereCreator:
	def __init__(self):
		...
		self.pt = Part
		self.mode = False
		LabRPS.Console.PrintMessage("Create instance of SphereCreator\n")

	def __del__(self):
		LabRPS.Console.PrintMessage("Delete instance of SphereCreator\n")

	def enter(self):
		if (self.mode):
			return
		LabRPS.Console.PrintMessage("Enter sphere creation mode\n")
		self.av = LabRPSGui.ActiveDocument.ActiveView
	...
	def leave(self):
		if (not self.mode):
			return
		LabRPS.Console.PrintMessage("Leave sphere creation mode\n")
	...
	def create(self, info):
		...
		if (down):
			...
			LabRPS.Console.PrintMessage("Clicked on position: ("+str(pos[0])+", "+str(pos[0])+")")
			msg = " -> (%f,%f,%f)\n" % (pnt.x, pnt.y, pnt.z)
			LabRPS.Console.PrintMessage(msg)
...
class TemplatePyMod_Cmd4:
	...
	def __del__(self):
		LabRPS.Console.PrintError('TemplatePyMod_Cmd4 was destroyed\n')

	def Activated(self):
		if LabRPSGui.ActiveDocument != None:
			self.sc.enter()
		else:
			LabRPS.Console.PrintWarning('A 3d view must be created\n')
...
class TemplatePyMod_Cmd5:
	...
	def Activated(self):
		...
		if myRenderArea is None:
			...
			myRenderArea = sogui.SoGuiRenderArea(LabRPSGui.getMainWindow())
...
class TemplatePyGrp_1:
    def Activated(self):
        import LabRPS
        LabRPS.Console.PrintMessage("TemplatePyGrp_1\n")
...
class TemplatePyGrp_2:
    def Activated(self):
        import LabRPS
        LabRPS.Console.PrintMessage("TemplatePyGrp_2\n")
...
class TemplatePyGrp_3:
    def Activated(self):
        import LabRPS
        LabRPS.Console.PrintMessage("TemplatePyGrp_3\n")
...
LabRPSGui.addCommand('TemplatePyMod_Cmd4',TemplatePyMod_Cmd4())
LabRPSGui.addCommand('TemplatePyMod_Cmd5',TemplatePyMod_Cmd5())
LabRPSGui.addCommand('TemplatePyMod_Cmd6',TemplatePyMod_Cmd6())
LabRPSGui.addCommand('TemplatePyGrp_1',TemplatePyGrp_1())
LabRPSGui.addCommand('TemplatePyGrp_2',TemplatePyGrp_2())
LabRPSGui.addCommand('TemplatePyGrp_3',TemplatePyGrp_3())
LabRPSGui.addCommand('TemplatePyGroup',TemplatePyGroup())
LabRPSGui.addCommand('TemplatePyCheckable',TemplatePyCheckable())
