...
PROPERTY_SOURCE(WindLab::CDeodatisAndShinozuka1991, WindLabAPI::WindLabFeatureSimulationMethod)

CDeodatisAndShinozuka1991::CDeodatisAndShinozuka1991()
{ 
    
}

bool CDeodatisAndShinozuka1991::OnInitialSetting(const WindLabAPI::WindLabSimulationData& Data)
{
    QMessageBox::warning(0,QString::fromLatin1("Deodatis method"), QString::fromLatin1("No additional data required."));
    return true;
}

bool CDeodatisAndShinozuka1991::Simulate(const WindLabAPI::WindLabSimulationData& Data, cube &dPhenomenon)
{
    ...
    for (int ss = 1; ss <= sampleN && false == Data.isInterruptionRequested.getValue(); ss++) {
        ...
        if (!returnResult) {
            Base::Console().Error(
                "The computation of the auto spectrum vector has failed.\n");
            return false;
        } 
            //compute matrix G
