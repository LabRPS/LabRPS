...
CRPSSimuAcrossWindSpectrum::CRPSSimuAcrossWindSpectrum()
{
  ...
  this->LinkToWiki.setValue("https://wiki.labrps.com/Plugin_WindLab#Simiu_Across_Wind_Spectrum");
...
bool CRPSSimuAcrossWindSpectrum::ComputeYAutoSpectrumValue(const WindLabAPI::WindLabSimulationData &Data, const Base::Vector3d &location, const double &dFrequency, const double &dTime, double &dValue)
{
    ...
    if (!returnResult) {
  	...
  	WindLabTools::SimuSpectrum simuPSD;
    dValue = simuPSD.computeAcrossWindAutoSpectrum(dFrequency, location.z, MEAN, ShearVelocity.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond), Constant1.getValue(), Constant2.getValue()) * ScaleCoefficient.getValue();
	return true;
} 
