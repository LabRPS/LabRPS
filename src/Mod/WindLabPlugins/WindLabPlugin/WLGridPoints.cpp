...
WLGridPoints::WLGridPoints()
{
    ...
    ADD_PROPERTY_TYPE(StartPoint, (Base::Vector3d(0,0,0)), locationgroup, App::Prop_None, "the center point of the grid.");
...
{
    int pointNumber1 = Length1.getValue() / Spacing1.getValue() + 1;
    int pointNumber2 = Length2.getValue() / Spacing2.getValue() + 1;
    ...
    if (QString::fromLatin1(LocationPlan.getValueAsString()) == QString::fromLatin1("XY Plane")) {
       for (int loop1 = 0; loop1 < pointNumber1; loop1++){
          for (int loop2 = 0; loop2 < pointNumber2; loop2++){
            Base::Vector3d position = Base::Vector3d(StartPoint.getValue().x + loop1 * Spacing1.getQuantityValue().getValueAs(Base::Quantity::Metre), StartPoint.getValue().y + loop2 * Spacing2.getQuantityValue().getValueAs(Base::Quantity::Metre),StartPoint.getValue().z);
        ...
        for (int loop1 = 0; loop1 < pointNumber1; loop1++){
          for (int loop2 = 0; loop2 < pointNumber2; loop2++){
            Base::Vector3d position = Base::Vector3d(StartPoint.getValue().x, StartPoint.getValue().y + loop1 * Spacing1.getQuantityValue().getValueAs(Base::Quantity::Metre),StartPoint.getValue().z + loop2 * Spacing2.getQuantityValue().getValueAs(Base::Quantity::Metre));
            points.push_back(position);
          }
        }
    }
         ...
         for (int loop1 = 0; loop1 < pointNumber1; loop1++){
           for (int loop2 = 0; loop2 < pointNumber2; loop2++){
            Base::Vector3d position = Base::Vector3d(StartPoint.getValue().x + loop1 * Spacing1.getQuantityValue().getValueAs(Base::Quantity::Metre), StartPoint.getValue().y, StartPoint.getValue().z + loop2 * Spacing2.getQuantityValue().getValueAs(Base::Quantity::Metre));
            points.push_back(position);
           }
       }
    }
...
{
	WindLabGui::GridPointsDialogEdit* dlg = new WindLabGui::GridPointsDialogEdit(StartPoint, Spacing1, Spacing2, Length1, Length2, LocationPlan,Data.spatialDistribution);
