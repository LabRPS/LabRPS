...
CRPSvonKarmanAlongWindSpectrum::CRPSvonKarmanAlongWindSpectrum()
{
  ...
  this->LinkToWiki.setValue("https://wiki.labrps.com/Plugin_WindLab#von_Karman_Along_Wind_Spectrum");
...
bool CRPSvonKarmanAlongWindSpectrum::ComputeXAutoSpectrumValue(const WindLabAPI::WindLabSimulationData &Data, const Base::Vector3d &location, const double &dFrequency, const double &dTime, double &dValue)
{
    ...
    dValue = vonKarmanPSD.computeAlongWindAutoSpectrum(dFrequency, MEAN, IntegralLengthScale.getQuantityValue().getValueAs(Base::Quantity::Metre), StandardDeviation.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond), Constant1.getValue(), Constant2.getValue()) * ScaleCoefficient.getValue();
