...
{
	// the input diolag
    WindLabGui::DlgDavenportSpectrumEdit* dlg = new WindLabGui::DlgDavenportSpectrumEdit(MeanWindSpeed10, ShearVelocity, Data.alongWindSpectrumModel);
...
bool CRPSDavenportSpectrum::ComputeXAutoSpectrumValue(const WindLabAPI::WindLabSimulationData &Data, const Base::Vector3d &location, const double &dFrequency, const double &dTime, double &dValue)
{
   bool returnResult = true;
    ...
    dValue = davenportPSD.computeAlongWindAutoSpectrum(dFrequency, ShearVelocity.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond), MeanWindSpeed10.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond)) * ScaleCoefficient.getValue();
