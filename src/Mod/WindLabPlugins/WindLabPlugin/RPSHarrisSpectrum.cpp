...
bool CRPSHarrisSpectrum::ComputeXCrossSpectrumValue(const WindLabAPI::WindLabSimulationData &Data, const Base::Vector3d &locationJ, const Base::Vector3d &locationK, const double &dFrequency, const double &dTime, std::complex<double> &dValue)
{
	...
	else if (!Data.stationarity.getValue() && Data.uniformModulation.getValue() && this->IsUniformlyModulated.getValue())
	{
        ...
        PSDj = harrisPSD.computeAlongWindAutoSpectrum(dFrequency, ShearVelocity.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond), dModValueJ * MeanWindSpeed10.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond));
        PSDk = harrisPSD.computeAlongWindAutoSpectrum(dFrequency, ShearVelocity.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond), dModValueK * MeanWindSpeed10.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond));
        dValue = std::sqrt(PSDj * PSDk) * COHjk;
	}
...
bool CRPSHarrisSpectrum::ComputeXAutoSpectrumValue(const WindLabAPI::WindLabSimulationData &Data, const Base::Vector3d &location, const double &dFrequency, const double &dTime, double &dValue)
{
   ...
   WindLabTools::HarrisSpectrum harrisPSD;
	...
	else if (!Data.stationarity.getValue() && Data.uniformModulation.getValue() && this->IsUniformlyModulated.getValue())
	{
        ...
        //we compute the time dependent mean wind speed first before introducing it in the psd formula
        dValue = harrisPSD.computeAlongWindAutoSpectrum(dFrequency, ShearVelocity.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond), dModValue * MeanWindSpeed10.getQuantityValue().getValueAs(Base::Quantity::MetrePerSecond));
	}
	else//this includes cases where the user chooses non-stationary wind with non-uniforme modulation. This feature cannot be used in this case.
	{
        Base::Console().Error("The computation of the power spectral density value has failed. The active feature does not allow non-stationarity with non-uniform modulation.\n");
