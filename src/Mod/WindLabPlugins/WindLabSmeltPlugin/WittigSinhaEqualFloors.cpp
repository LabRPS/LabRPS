...
WittigSinhaEqualFloors::WittigSinhaEqualFloors() {
    ...
    this->LinkToWiki.setValue("https://wiki.labrps.com/Plugin_WindLab_WindLabSmelt#Wittig_Sinha_Equal_Floors");
}
...
bool WittigSinhaEqualFloors::Simulate(const WindLabAPI::WindLabSimulationData& Data, cube& dPhenomenon) {
...
for (int ss = 1; ss <= sampleN && false == Data.isInterruptionRequested.getValue(); ss++) {
      ...
      auto run = Factory<stochastic::StochasticModel, std::string, double, double, unsigned int,
                         double, int>::instance()
                     ->create("WittigSinhaDiscreteFreqWind", std::move(exposure_category),
                              std::move(gust_speed), std::move(height), std::move(num_floors),
                              std::move(total_time), std::move(seed));
      auto time_histories_json = run->generate(eventName);


      for (int j = 0; j < num_floors && false == Data.isInterruptionRequested.getValue(); j++) {
          auto run_data =
              time_histories_json.get_library_json()["Events"][eventIndex]["timeSeries"][j]["data"]
