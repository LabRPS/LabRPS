#include "RPSRandomDistributionDialog.h"
...
#include "ui_RPSRandomDistributionDialog.h"
...
void RandomDistributionDialog::distributionChanged(int index) {
  ...
  switch (dist) {
    ...
    case Weibull:{
       ui_->distFormulaLabel->setPixmap(
           QPixmap(distFormulaPath + "dist-weibull.png"));
       ui_->parameter1DbleSpinBox->show();
       ui_->parameter1Label->show();
       ui_->parameter2DbleSpinBox->show();
       ui_->parameter2Label->show();
...
QVector<double> RandomDistributionDialog::getParametersVector()
{
    QSettings settings;
    settings.beginGroup("General");

    parameters.clear();
    parameters << settings.value("RandomDistributionParam-1").toDouble()
               << settings.value("RandomDistributionParam-2").toDouble()
               << settings.value("RandomDistributionParam-3").toDouble();
    settings.endGroup();
    return parameters;
