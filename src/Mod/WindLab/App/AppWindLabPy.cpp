...
public:
    ...
    Module() : Py::ExtensionModule<Module>("WindLab")
    {
...
private:
    ...
    Py::Object getSimulation(const Py::Tuple& args)
    {
        ...
        try {
            ...
            WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(doc->getObject(psName));
            if (!sim)
                throw Py::RuntimeError("Simulation not found.");

            if (!sim->getTypeId().isDerivedFrom(WindLab::WindLabSimulation::getClassTypeId()))
                throw Py::RuntimeError("Simulation not found.");
    ...
    try {
        ...
        WindLab::WindLabSimulation* parentSimulation = static_cast<WindLab::WindLabSimulation*>(doc->getObject(simulationName));
        if (!parentSimulation)
           throw Py::RuntimeError("Parent simulation not found.");

        if (!parentSimulation->getTypeId().isDerivedFrom(WindLab::WindLabSimulation::getClassTypeId())) 
              throw Py::RuntimeError("Parent simulation not found.");
    ...
    QString phenomenon = WindLabUtils::rpsPhenomenonWindVelocity;
