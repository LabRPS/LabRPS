...
PROPERTY_SOURCE(WindLabGui::ViewProviderWindLabFeatureModulation, Gui::ViewProviderRPSFeature)


ViewProviderWindLabFeatureModulation::ViewProviderWindLabFeatureModulation()
{
  sPixmap = ":/icons/WindLabFeatures/WindLab_Feature_ModulationObj.svg";
}
...
bool ViewProviderWindLabFeatureModulation::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeatureModulation::ActivateFeature()
{
...
bool ViewProviderWindLabFeatureModulation::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeatureModulation::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeatureModulation::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
 
    QAction* cvalue = menu->addAction(QObject::tr("ComputeModulationValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderWindLabFeatureModulation::computeModulationValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeModulationVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeatureModulation::computeModulationVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputeModulationVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderWindLabFeatureModulation::computeModulationVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeatureModulation::OnInitialSetting, this));

    QAction* act = menu->addAction(QObject::tr("Activate Feature"));
    func->trigger(act, boost::bind(&ViewProviderWindLabFeatureModulation::ActivateFeature, this));

    QAction* gtp = menu->addAction(QObject::tr("Go to Publication"));
    func->trigger(gtp, boost::bind(&ViewProviderWindLabFeatureModulation::goToPublication, this));

    QAction* gtw = menu->addAction(QObject::tr("Go to Wiki"));
    func->trigger(gtw, boost::bind(&ViewProviderWindLabFeatureModulation::goToWiki, this));
}


bool ViewProviderWindLabFeatureModulation::computeModulationValue()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeModulationValue);

}

bool ViewProviderWindLabFeatureModulation::computeModulationVectorT()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeModulationVectorT);

}

bool ViewProviderWindLabFeatureModulation::computeModulationVectorP()
{   
    return runFeatureMethod(WindLab::WindLabUtils::ComputeModulationVectorP);
    
}

bool ViewProviderWindLabFeatureModulation::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupModulationFunction, QString::fromUtf8(sim->getSimulationData()->modulationFunction.getValue()));
     
    return true;
}

PyObject* ViewProviderWindLabFeatureModulation::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeatureModulationPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeatureModulation::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(WindLabGui::WindLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
