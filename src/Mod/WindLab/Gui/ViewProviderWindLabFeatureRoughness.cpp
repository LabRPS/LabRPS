...
#include <Mod/WindLabAPI/App/IrpsWLRoughness.h>
...
{
  sPixmap = ":/icons/WindLabFeatures/WindLab_Feature_RoughnessObj.svg";
}
...
bool ViewProviderWindLabFeatureRoughness::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeatureRoughness::ActivateFeature()
{
auto doc = App::GetApplication().getActiveDocument();
    ...
    parentSim->Roughness.setValue(obj->getNameInDocument());
    return true;
}

bool ViewProviderWindLabFeatureRoughness::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeatureRoughness::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeatureRoughness::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
   
    QAction* cvalue = menu->addAction(QObject::tr("ComputeRoughnessValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderWindLabFeatureRoughness::computeRoughnessValue, this));

    QAction* cvectorP = menu->addAction(QObject::tr("ComputeRoughnessVectorP"));
    func->trigger(cvectorP, boost::bind(&ViewProviderWindLabFeatureRoughness::computeRoughnessVectorP, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeRoughnessVectorT"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeatureRoughness::computeRoughnessVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeatureRoughness::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderWindLabFeatureRoughness::goToWiki, this));
}


bool ViewProviderWindLabFeatureRoughness::computeRoughnessValue()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeRoughnessValue);
    
}


bool ViewProviderWindLabFeatureRoughness::computeRoughnessVectorP()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeRoughnessVectorP);
    
}



bool ViewProviderWindLabFeatureRoughness::computeRoughnessVectorT()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeRoughnessVectorT);
    
}

bool ViewProviderWindLabFeatureRoughness::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupRoughness, QString::fromUtf8(sim->getSimulationData()->roughness.getValue()));
     
    return true;
}

PyObject* ViewProviderWindLabFeatureRoughness::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeatureRoughnessPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeatureRoughness::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(WindLabGui::WindLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
