...
#include <Mod/WindLabAPI/App/IrpsWLPeakFactor.h>
...
bool ViewProviderWindLabFeaturePeakFactor::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeaturePeakFactor::ActivateFeature()
{
...
bool ViewProviderWindLabFeaturePeakFactor::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeaturePeakFactor::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeaturePeakFactor::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
  
    QAction* cvalue = menu->addAction(QObject::tr("ComputePeakFactorValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderWindLabFeaturePeakFactor::computePeakFactorValue, this));

     QAction* cvector = menu->addAction(QObject::tr("ComputePeakFactorVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeaturePeakFactor::computePeakFactorVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputePeakFactorVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderWindLabFeaturePeakFactor::computePeakFactorVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeaturePeakFactor::OnInitialSetting, this));
...
bool ViewProviderWindLabFeaturePeakFactor::computePeakFactorValue()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputePeakFactorValue);
    
}

bool ViewProviderWindLabFeaturePeakFactor::computePeakFactorVectorP()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputePeakFactorVectorP);
    
}

bool ViewProviderWindLabFeaturePeakFactor::computePeakFactorVectorT()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputePeakFactorVectorT);
    
}

bool ViewProviderWindLabFeaturePeakFactor::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupPeakFactor, QString::fromUtf8(sim->getSimulationData()->peakFactor.getValue()));
     
    return true;
}

PyObject* ViewProviderWindLabFeaturePeakFactor::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeaturePeakFactorPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeaturePeakFactor::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(WindLabGui::WindLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
