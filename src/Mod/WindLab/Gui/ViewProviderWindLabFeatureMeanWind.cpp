...
#include <Mod/WindLabAPI/App/IrpsWLMean.h>
...
{
  sPixmap = ":/icons/WindLabFeatures/WindLab_Feature_MeanObj.svg";
}
...
bool ViewProviderWindLabFeatureMeanWind::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeatureMeanWind::ActivateFeature()
{
...
bool ViewProviderWindLabFeatureMeanWind::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeatureMeanWind::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeatureMeanWind::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
    QAction* cvalue = menu->addAction(QObject::tr("ComputeMeanWindSpeedValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderWindLabFeatureMeanWind::computeMeanWindSpeedValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeMeanWindSpeedVectorP"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeatureMeanWind::computeMeanWindSpeedVectorP, this));

    QAction* carray = menu->addAction(QObject::tr("ComputeMeanWindSpeedVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderWindLabFeatureMeanWind::computeMeanWindSpeedVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeatureMeanWind::OnInitialSetting, this));
...
bool ViewProviderWindLabFeatureMeanWind::computeMeanWindSpeedValue()
{
    ...
    return runFeatureMethod(WindLab::WindLabUtils::ComputeMeanWindSpeedValue);
        
}

bool ViewProviderWindLabFeatureMeanWind::computeMeanWindSpeedVectorT()
{
     
    return runFeatureMethod(WindLab::WindLabUtils::ComputeMeanWindSpeedVectorT);
    
}

bool ViewProviderWindLabFeatureMeanWind::computeMeanWindSpeedVectorP()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeMeanWindSpeedVectorP);
    
}

bool ViewProviderWindLabFeatureMeanWind::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupMeanWindProfile, QString::fromUtf8(sim->getSimulationData()->meanFunction.getValue()));
     
    return true;
}

void ViewProviderWindLabFeatureMeanWind::onChanged(const App::Property* prop)
{
    ViewProviderDocumentObject::onChanged(prop);
}

PyObject* ViewProviderWindLabFeatureMeanWind::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeatureMeanWindPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeatureMeanWind::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(WindLabGui::WindLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
