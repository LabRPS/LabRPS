...
PROPERTY_SOURCE(WindLabGui::ViewProviderWindLabFeatureFrequencyDistribution, Gui::ViewProviderDocumentObject)


ViewProviderWindLabFeatureFrequencyDistribution::ViewProviderWindLabFeatureFrequencyDistribution()
{
  sPixmap = ":/icons/WindLabFeatures/WindLab_Feature_FrequencyObj.svg";
}
...
bool ViewProviderWindLabFeatureFrequencyDistribution::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeatureFrequencyDistribution::ActivateFeature()
{
...
bool ViewProviderWindLabFeatureFrequencyDistribution::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeatureFrequencyDistribution::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeatureFrequencyDistribution::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
  
    QAction* cvectorV = menu->addAction(QObject::tr("ComputeFrequenciesValue"));
    func->trigger(cvectorV, boost::bind(&ViewProviderWindLabFeatureFrequencyDistribution::computeFrequencyValue, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeFrequenciesVectorF"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeatureFrequencyDistribution::computeFrequenciesVectorF, this));

     QAction* cvectorFP = menu->addAction(QObject::tr("ComputeFrequenciesMatrixFP"));
    func->trigger(cvectorFP, boost::bind(&ViewProviderWindLabFeatureFrequencyDistribution::computeFrequenciesMatrixFP, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeatureFrequencyDistribution::OnInitialSetting, this));

    QAction* act = menu->addAction(QObject::tr("Activate Feature"));
    func->trigger(act, boost::bind(&ViewProviderWindLabFeatureFrequencyDistribution::ActivateFeature, this));
}

bool ViewProviderWindLabFeatureFrequencyDistribution::computeFrequencyValue()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeFrequencyValue);
    
}

bool ViewProviderWindLabFeatureFrequencyDistribution::computeFrequenciesVectorF()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeFrequenciesVectorF);
    
}

bool ViewProviderWindLabFeatureFrequencyDistribution::computeFrequenciesMatrixFP()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeFrequenciesMatrixFP);
    
}


bool ViewProviderWindLabFeatureFrequencyDistribution::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupFrequencyDistribution, QString::fromUtf8(sim->getSimulationData()->frequencyDistribution.getValue()));
     
    return true;
}

PyObject* ViewProviderWindLabFeatureFrequencyDistribution::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeatureFrequencyDistributionPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeatureFrequencyDistribution::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{
