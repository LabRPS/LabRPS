...
{
  sPixmap = ":/icons/WindLabFeatures/WindLab_Feature_DecomposedSpectrumObj.svg";
}
...
bool ViewProviderWindLabFeaturePSDDecompositionMethod::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeaturePSDDecompositionMethod::ActivateFeature()
{
...
bool ViewProviderWindLabFeaturePSDDecompositionMethod::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeaturePSDDecompositionMethod::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeaturePSDDecompositionMethod::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
  
    QAction* cvector = menu->addAction(QObject::tr("ComputeDecomposedCrossSpectrumVectorF"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeaturePSDDecompositionMethod::computeDecomposedCrossSpectrumVectorF, this));

    QAction* carray = menu->addAction(QObject::tr("ComputeDecomposedCrossSpectrumVectorT"));
    func->trigger(carray, boost::bind(&ViewProviderWindLabFeaturePSDDecompositionMethod::computeDecomposedCrossSpectrumVectorT, this));

      QAction* cvalue = menu->addAction(QObject::tr("ComputeDecomposedCrossSpectrumMatrixPP"));
    func->trigger(cvalue, boost::bind(&ViewProviderWindLabFeaturePSDDecompositionMethod::computeDecomposedCrossSpectrumMatrixPP, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeaturePSDDecompositionMethod::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderWindLabFeaturePSDDecompositionMethod::goToWiki, this));
}

bool ViewProviderWindLabFeaturePSDDecompositionMethod::computeDecomposedCrossSpectrumMatrixPP()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeDecomposedCrossSpectrumMatrixPP);
    
}

bool ViewProviderWindLabFeaturePSDDecompositionMethod::computeDecomposedCrossSpectrumVectorF()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeDecomposedCrossSpectrumVectorF);
    
}

bool ViewProviderWindLabFeaturePSDDecompositionMethod::computeDecomposedCrossSpectrumVectorT()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeDecomposedCrossSpectrumVectorT);
    
}

bool ViewProviderWindLabFeaturePSDDecompositionMethod::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupSpectrumDecompositionMethod, QString::fromUtf8(sim->getSimulationData()->cpsdDecompositionMethod.getValue()));
     
    return true;
}

PyObject* ViewProviderWindLabFeaturePSDDecompositionMethod::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeaturePSDDecompositionMethodPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeaturePSDDecompositionMethod::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(WindLabGui::WindLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
