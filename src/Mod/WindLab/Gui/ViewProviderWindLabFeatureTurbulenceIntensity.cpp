...
{
  sPixmap = ":/icons/WindLabFeatures/WindLab_Feature_TurbulenceIntensityObj.svg";
}
...
bool ViewProviderWindLabFeatureTurbulenceIntensity::doubleClicked(void)
{
    ActivateFeature();
...
bool ViewProviderWindLabFeatureTurbulenceIntensity::ActivateFeature()
{
auto doc = App::GetApplication().getActiveDocument();
    ...
    parentSim->TurbulenceIntensity.setValue(obj->getNameInDocument());
    return true;
}
bool ViewProviderWindLabFeatureTurbulenceIntensity::setEdit(int ModNum)
{
    Q_UNUSED(ModNum);
...
void ViewProviderWindLabFeatureTurbulenceIntensity::unsetEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
...
void ViewProviderWindLabFeatureTurbulenceIntensity::setupContextMenu(QMenu* menu, QObject*, const char*)
{
    // toggle command to display components
    Gui::ActionFunction* func = new Gui::ActionFunction(menu);
    
    QAction* cvalue = menu->addAction(QObject::tr("ComputeTurbulenceIntensityValue"));
    func->trigger(cvalue, boost::bind(&ViewProviderWindLabFeatureTurbulenceIntensity::computeTurbulenceIntensityValue, this));

    QAction* cvectorP = menu->addAction(QObject::tr("ComputeTurbulenceIntensityVectorP"));
    func->trigger(cvectorP, boost::bind(&ViewProviderWindLabFeatureTurbulenceIntensity::computeTurbulenceIntensityVectorP, this));

    QAction* cvector = menu->addAction(QObject::tr("ComputeTurbulenceIntensityVectorT"));
    func->trigger(cvector, boost::bind(&ViewProviderWindLabFeatureTurbulenceIntensity::computeTurbulenceIntensityVectorT, this));

    QAction* init = menu->addAction(QObject::tr("Setup Feature"));
    func->trigger(init, boost::bind(&ViewProviderWindLabFeatureTurbulenceIntensity::OnInitialSetting, this));
    ...
    func->trigger(gtw, boost::bind(&ViewProviderWindLabFeatureTurbulenceIntensity::goToWiki, this));
}


bool ViewProviderWindLabFeatureTurbulenceIntensity::computeTurbulenceIntensityValue()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeTurbulenceIntensityValue);
    
}

bool ViewProviderWindLabFeatureTurbulenceIntensity::computeTurbulenceIntensityVectorP()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeTurbulenceIntensityVectorP);
    
}


bool ViewProviderWindLabFeatureTurbulenceIntensity::computeTurbulenceIntensityVectorT()
{
    
    return runFeatureMethod(WindLab::WindLabUtils::ComputeTurbulenceIntensityVectorT);
    
}
bool ViewProviderWindLabFeatureTurbulenceIntensity::OnInitialSetting()
{
ActivateFeature();
    ...
    sim->windLabFeatureInitalSetting(WindLab::WindLabUtils::objGroupTurbulenceIntensity, QString::fromUtf8(sim->getSimulationData()->turbulenceIntensity.getValue()));
     
    return true;
}

PyObject* ViewProviderWindLabFeatureTurbulenceIntensity::getPyObject(void)
{
    if (PythonObject.is(Py::_None())) {
        // ref counter is set to 1
        PythonObject = Py::Object(new ViewProviderWindLabFeatureTurbulenceIntensityPy(this), true);
    }
    return Py::new_reference_to(PythonObject);
}

bool ViewProviderWindLabFeatureTurbulenceIntensity::runFeatureMethod(const QString function, const char* complexNumberDisplay)
{  
ActivateFeature();
    WindLab::WindLabSimulation* sim = static_cast<WindLab::WindLabSimulation*>(WindLabGui::WindLabSimulationObserver::instance()->active());
    if (!sim) {Base::Console().Warning("No valide active simulation found.\n");return false;}
