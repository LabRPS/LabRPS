<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="el" sourcelanguage="en">
  <context>
    <name>TestGui::UnitTest</name>
    <message>
      <location filename="../../UnitTest.ui" line="17"/>
      <source>LabRPS UnitTest</source>
      <translation>Έλεγχος Μονάδων του LabRPS</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="32"/>
      <source>Test</source>
      <translation>Έλεγχος</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="44"/>
      <source>Select test name:</source>
      <translation>Επιλέξτε όνομα ελέγχου:</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="96"/>
      <source>&amp;Start</source>
      <translation>Έναρξη</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="99"/>
      <source>Alt+S</source>
      <translation>Alt+S</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="128"/>
      <source>&amp;Help</source>
      <translation>Βο&amp;ήθεια</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="131"/>
      <source>F1</source>
      <translation>F1</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="141"/>
      <source>&amp;About</source>
      <translation>Σχετικές Πληροφορίες</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="144"/>
      <source>Alt+A</source>
      <translation>Alt+A</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="154"/>
      <source>&amp;Close</source>
      <translation>Κλείσιμο</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="157"/>
      <source>Alt+C</source>
      <translation>Alt+C</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="169"/>
      <source>Progress</source>
      <translation>Πρόοδος</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="188"/>
      <source>Run:</source>
      <translation>Εκτέλεση:</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="205"/>
      <source>Failures:</source>
      <translation>Αποτυχίες:</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="222"/>
      <source>Errors:</source>
      <translation>Σφάλματα:</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="239"/>
      <source>Remaining:</source>
      <translation>Απομένουν:</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="259"/>
      <source>Failures and errors</source>
      <translation>Αποτυχίες και σφάλματα</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="275"/>
      <source>Description</source>
      <translation>Περιγραφή</translation>
    </message>
    <message>
      <location filename="../../UnitTest.ui" line="292"/>
      <source>Idle</source>
      <translation>Σε αδράνεια</translation>
    </message>
  </context>
  <context>
    <name>TestGui::UnitTestDialog</name>
    <message>
      <location filename="../../UnitTestImp.cpp" line="155"/>
      <source>Help</source>
      <translation>Βοήθεια</translation>
    </message>
    <message>
      <location filename="../../UnitTestImp.cpp" line="155"/>
      <source>Enter the name of a callable object which, when called, will return a TestCase.
Click 'start', and the test thus produced will be run.

Double click on an error in the tree view to see more information about it, including the stack trace.</source>
      <translation>Εισάγετε το όνομα ενός αντικειμένου που δύναται να κληθεί, το οποίο όταν κληθεί, επιστρέφει μια Περίπτωση Ελέγχου.
Κάντε κλικ στην επιλογή 'έναρξη' και ο έλεγχος που δημιουργήθηκε θα εκτελεστεί.

Κάντε διπλό κλικ σε οποιοδήποτε σφάλμα στο δενδροδιάγραμμα για να δείτε περισσότερες πληροφορίες, συμπεριλαμβανομένου του ίχνους της στοίβας.</translation>
    </message>
    <message>
      <location filename="../../UnitTestImp.cpp" line="167"/>
      <source>About LabRPS UnitTest</source>
      <translation>Σχετικά με τον Έλεγχο Μονάδων του Labrps</translation>
    </message>
    <message>
      <location filename="../../UnitTestImp.cpp" line="167"/>
      <source>Copyright (c) Werner Mayer

LabRPS UnitTest is part of LabRPS and supports writing Unit Tests for ones own modules.</source>
      <translation>Πνευματικά δικαιώματα (c) Werner Mayer

Ο Έλεγχος Μονάδων του LabRPS είναι μέρος του λογισμικού LabRPS και υποστηρίζει την εγγραφή Ελέγχων Μονάδων για τις λειτουργικές μονάδες του εκάστοτε χρήστη.</translation>
    </message>
  </context>
</TS>
