...
const char * RPSWindLabAPIInfo::getWindLabAPIVersion()
{
    ...
    CrpsCoherenceFactory::~CrpsCoherenceFactory()
    {
 	...
 	IrpsWLCoherence *CrpsCoherenceFactory::BuildObject(const std::string& name)
 	{
...
CrpsLocationDistributionFactory::~CrpsLocationDistributionFactory()
{
  ...
  for (std::map<const std::string, PyObject*>::const_iterator it = mProducerMap.begin(); it != mProducerMap.end(); ++it) {
    ...
    IrpsWLLocationDistribution *CrpsLocationDistributionFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsMeanFactory::~CrpsMeanFactory()
    {
 	...
 	IrpsWLMean *CrpsMeanFactory::BuildObject(const std::string& name)
 	{
    ...
    std::map<const std::string, std::string> CrpsXSpectrumFactory::mOjectsSkipDuringUnintallationMap;
 	...
 	std::map<const std::string, std::string> CrpsXSpectrumFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsXSpectrumFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsXSpectrumFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsXSpectrumFactory::mProducerMap;
    std::map<const std::string, bool> CrpsXSpectrumFactory::mStationarityMap;

    CrpsXSpectrumFactory::CrpsXSpectrumFactory() {}
    CrpsXSpectrumFactory::~CrpsXSpectrumFactory()
    {
        Base::PyGILStateLocker lock;
        for (std::map<const std::string, PyObject*>::const_iterator it = mProducerMap.begin();
 	...
 	void CrpsXSpectrumFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsXSpectrumFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateXSpectrumCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsXSpectrumFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
 	...
 	IrpsWLXSpectrum *CrpsXSpectrumFactory::BuildObject(const std::string& name)
 	{
     ...
     PyObject* CrpsXSpectrumFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
    {
        PyObject* producer = mProducerMap[pluggedFeatureName];
 	...
 	std::map<const std::string, CreateXSpectrumCallback>& CrpsXSpectrumFactory::GetObjectNamesMap()
 	{
 		return mXSpectrums;
 	}

 	std::map<const std::string, std::string> & CrpsXSpectrumFactory::GetTobeInstalledObjectsMap()
 	{
 		return mTobeInstalledObjectsMap;
 	}

 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetOjectDescriptionMap()
 	{
 		return mOjectDescriptionMap;
 	}

 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetOjectAndPluginMap()
 	{
 		return mOjectAndPluginMap;
 	}

 	///
 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetTitleMap()
 	{
 		return mTitleMap;
 	}

 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetLinkMap()
 	{
 		return mLinkMap;
 	}

 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetAuthorMap()
 	{
 		return mAuthorMap;
 	}

 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetDateMap()
 	{
 		return mDateMap;
 	}

 	std::map<const std::string, std::string>& CrpsXSpectrumFactory::GetVersionMap()
 	{
 		return mVersionMap;
 	}

 	std::map<const std::string, std::string> &  CrpsXSpectrumFactory::GetOjectsSkipDuringUnintallationMap()
     {
 	    return mOjectsSkipDuringUnintallationMap;
     }

    std::map<const std::string, PyObject*>& CrpsXSpectrumFactory::GetProducerMap()
     {
        return mProducerMap;
     }

              std::map<const std::string, bool>& CrpsXSpectrumFactory::GetStationarityMap()
     {
        return mStationarityMap;
     }
    ...
    std::map<const std::string, std::string> CrpsZSpectrumFactory::mOjectsSkipDuringUnintallationMap;
 	...
 	std::map<const std::string, std::string> CrpsZSpectrumFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsZSpectrumFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsZSpectrumFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsZSpectrumFactory::mProducerMap;
    std::map<const std::string, bool> CrpsZSpectrumFactory::mStationarityMap;

    CrpsZSpectrumFactory::CrpsZSpectrumFactory() {}
    CrpsZSpectrumFactory::~CrpsZSpectrumFactory()
    {
        Base::PyGILStateLocker lock;
        for (std::map<const std::string, PyObject*>::const_iterator it = mProducerMap.begin();
 	...
 	void CrpsZSpectrumFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsZSpectrumFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateZSpectrumCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsZSpectrumFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
 	...
 	IrpsWLZSpectrum *CrpsZSpectrumFactory::BuildObject(const std::string& name)
 	{
     ...
     PyObject* CrpsZSpectrumFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
    {
        PyObject* producer = mProducerMap[pluggedFeatureName];
 	...
 	std::map<const std::string, CreateZSpectrumCallback>& CrpsZSpectrumFactory::GetObjectNamesMap()
 	{
 		return mZSpectrums;
 	}

 	std::map<const std::string, std::string> & CrpsZSpectrumFactory::GetTobeInstalledObjectsMap()
 	{
 		return mTobeInstalledObjectsMap;
 	}

 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetOjectDescriptionMap()
 	{
 		return mOjectDescriptionMap;
 	}

 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetOjectAndPluginMap()
 	{
 		return mOjectAndPluginMap;
 	}

 	///
 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetTitleMap()
 	{
 		return mTitleMap;
 	}

 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetLinkMap()
 	{
 		return mLinkMap;
 	}

 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetAuthorMap()
 	{
 		return mAuthorMap;
 	}

 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetDateMap()
 	{
 		return mDateMap;
 	}

 	std::map<const std::string, std::string>& CrpsZSpectrumFactory::GetVersionMap()
 	{
 		return mVersionMap;
 	}

 	std::map<const std::string, std::string> &  CrpsZSpectrumFactory::GetOjectsSkipDuringUnintallationMap()
     {
 	    return mOjectsSkipDuringUnintallationMap;
     }

    std::map<const std::string, PyObject*>& CrpsZSpectrumFactory::GetProducerMap()
     {
        return mProducerMap;
     }

      std::map<const std::string, bool>& CrpsZSpectrumFactory::GetStationarityMap()
     {
        return mStationarityMap;
     }
 	...
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mTobeInstalledObjectsMap;
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mOjectDescriptionMap;
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mOjectAndPluginMap;
     std::map<const std::string, std::string> CrpsYSpectrumFactory::mOjectsSkipDuringUnintallationMap;

 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mTitleMap;
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mLinkMap;
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsYSpectrumFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsYSpectrumFactory::mProducerMap;
    std::map<const std::string, bool> CrpsYSpectrumFactory::mStationarityMap;

    CrpsYSpectrumFactory::CrpsYSpectrumFactory() {}
    CrpsYSpectrumFactory::~CrpsYSpectrumFactory()
    {
        Base::PyGILStateLocker lock;
        for (std::map<const std::string, PyObject*>::const_iterator it = mProducerMap.begin();
 	...
 	void CrpsYSpectrumFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsYSpectrumFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateYSpectrumCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsYSpectrumFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
 	...
 	IrpsWLYSpectrum *CrpsYSpectrumFactory::BuildObject(const std::string& name)
 	{
         ...
         PyObject* CrpsYSpectrumFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
    {
        PyObject* producer = mProducerMap[pluggedFeatureName];
 	...
 	std::map<const std::string, CreateYSpectrumCallback>& CrpsYSpectrumFactory::GetObjectNamesMap()
 	{
 		return mYSpectrums;
 	}

 	std::map<const std::string, std::string> & CrpsYSpectrumFactory::GetTobeInstalledObjectsMap()
 	{
 		return mTobeInstalledObjectsMap;
 	}

 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetOjectDescriptionMap()
 	{
 		return mOjectDescriptionMap;
 	}

 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetOjectAndPluginMap()
 	{
 		return mOjectAndPluginMap;
 	}

 	///
 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetTitleMap()
 	{
 		return mTitleMap;
 	}

 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetLinkMap()
 	{
 		return mLinkMap;
 	}

 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetAuthorMap()
 	{
 		return mAuthorMap;
 	}

 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetDateMap()
 	{
 		return mDateMap;
 	}

 	std::map<const std::string, std::string>& CrpsYSpectrumFactory::GetVersionMap()
 	{
 		return mVersionMap;
 	}

 	std::map<const std::string, std::string> &  CrpsYSpectrumFactory::GetOjectsSkipDuringUnintallationMap()
     {
 	    return mOjectsSkipDuringUnintallationMap;
     }

        std::map<const std::string, PyObject*>& CrpsYSpectrumFactory::GetProducerMap()
     {
        return mProducerMap;
     }
     std::map<const std::string, bool>& CrpsYSpectrumFactory::GetStationarityMap()
     {
        return mStationarityMap;
     }
    ...
    CrpsSimuMethodFactory::~CrpsSimuMethodFactory()
    {
 	...
 	IrpsWLSimuMethod *CrpsSimuMethodFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsFrequencyDistributionFactory::~CrpsFrequencyDistributionFactory()
    {
 	...
 	IrpsWLFrequencyDistribution *CrpsFrequencyDistributionFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsRandomnessFactory::~CrpsRandomnessFactory()
    {
 	...
 	IrpsWLRandomness *CrpsRandomnessFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsPSDdecomMethodFactory::~CrpsPSDdecomMethodFactory()
    {
 	...
 	IrpsWLPSDdecompositionMethod *CrpsPSDdecomMethodFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsCorrelationFactory::~CrpsCorrelationFactory()
    {
 	...
 	IrpsWLCorrelation *CrpsCorrelationFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsModulationFactory::~CrpsModulationFactory()
    {
 	...
 	IrpsWLModulation *CrpsModulationFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsTableToolFactory::~CrpsTableToolFactory()
    {
 	...
 	IrpsWLTableTool *CrpsTableToolFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsMatrixToolFactory::~CrpsMatrixToolFactory()
    {
 	...
 	IrpsWLMatrixTool *CrpsMatrixToolFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsUserDefinedRPSObjectFactory::~CrpsUserDefinedRPSObjectFactory()
    {
 	...
 	IrpsWLUserDefinedRPSObject *CrpsUserDefinedRPSObjectFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsCumulativeProbabilityDistributionFactory::~CrpsCumulativeProbabilityDistributionFactory()
    {
 	...
 	IrpsWLCumulativeProbabilityDistribution *CrpsCumulativeProbabilityDistributionFactory::BuildObject(const std::string& name)
 	{
    ...
    PyObject* CrpsCumulativeProbabilityDistributionFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
 	...
 	std::vector<std::string> CrpsGustFactorFactory::mGustFactorNames;
 	QString CrpsGustFactorFactory::mOwnerPlugin;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mTobeInstalledObjectsMap;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mOjectDescriptionMap;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mOjectAndPluginMap;
     std::map<const std::string, std::string> CrpsGustFactorFactory::mOjectsSkipDuringUnintallationMap;

 	std::map<const std::string, std::string> CrpsGustFactorFactory::mTitleMap;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mLinkMap;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsGustFactorFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsGustFactorFactory::mProducerMap;
    std::map<const std::string, bool> CrpsGustFactorFactory::mStationarityMap;
    ...
    CrpsGustFactorFactory::~CrpsGustFactorFactory()
    {
 	...
 	void CrpsGustFactorFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{		
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsGustFactorFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateGustFactorCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsGustFactorFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
    ...
    PyObject* CrpsGustFactorFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
     {
        PyObject* producer = mProducerMap[pluggedFeatureName];
    ...
    CrpsKurtosisFactory::~CrpsKurtosisFactory()
    {
 	...
 	IrpsWLKurtosis *CrpsKurtosisFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsPeakFactorFactory::~CrpsPeakFactorFactory()
    {
 	...
 	IrpsWLPeakFactor *CrpsPeakFactorFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsProbabilityDensityFunctionFactory::~CrpsProbabilityDensityFunctionFactory()
    {
 	...
 	IrpsWLProbabilityDensityFunction *CrpsProbabilityDensityFunctionFactory::BuildObject(const std::string& name)
 	{
    ...
    PyObject* CrpsProbabilityDensityFunctionFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
 	...
 	std::vector<std::string> CrpsRoughnessFactory::mRoughnessNames;
 	QString CrpsRoughnessFactory::mOwnerPlugin;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mTobeInstalledObjectsMap;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mOjectDescriptionMap;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mOjectAndPluginMap;
     std::map<const std::string, std::string> CrpsRoughnessFactory::mOjectsSkipDuringUnintallationMap;

 	std::map<const std::string, std::string> CrpsRoughnessFactory::mTitleMap;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mLinkMap;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsRoughnessFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsRoughnessFactory::mProducerMap;
    std::map<const std::string, bool> CrpsRoughnessFactory::mStationarityMap;
    ...
    CrpsRoughnessFactory::~CrpsRoughnessFactory()
    {
 	...
 	void CrpsRoughnessFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{		
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsRoughnessFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateRoughnessCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsRoughnessFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
    ...
    PyObject* CrpsRoughnessFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
     {
        PyObject* producer = mProducerMap[pluggedFeatureName];
    ...
    CrpsShearVelocityOfFlowFactory::~CrpsShearVelocityOfFlowFactory()
    {
 	...
 	IrpsWLShearVelocityOfFlow *CrpsShearVelocityOfFlowFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsStandardDeviationFactory::~CrpsStandardDeviationFactory()
    {
 	...
 	IrpsWLStandardDeviation *CrpsStandardDeviationFactory::BuildObject(const std::string& name)
 	{
    ...
    PyObject* CrpsStandardDeviationFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
 	...
 	std::vector<std::string> CrpsTurbulenceIntensityFactory::mTurbulenceIntensityNames;
 	QString CrpsTurbulenceIntensityFactory::mOwnerPlugin;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mTobeInstalledObjectsMap;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mOjectDescriptionMap;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mOjectAndPluginMap;
     std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mOjectsSkipDuringUnintallationMap;

 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mTitleMap;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mLinkMap;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsTurbulenceIntensityFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsTurbulenceIntensityFactory::mProducerMap;
    std::map<const std::string, bool> CrpsTurbulenceIntensityFactory::mStationarityMap;

    CrpsTurbulenceIntensityFactory::CrpsTurbulenceIntensityFactory() {}
    CrpsTurbulenceIntensityFactory::~CrpsTurbulenceIntensityFactory()
    {
        Base::PyGILStateLocker lock;
        for (std::map<const std::string, PyObject*>::const_iterator it = mProducerMap.begin();
 	...
 	void CrpsTurbulenceIntensityFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{		
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsTurbulenceIntensityFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateTurbulenceIntensityCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsTurbulenceIntensityFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
 	...
 	IrpsWLTurbulenceIntensity *CrpsTurbulenceIntensityFactory::BuildObject(const std::string& name)
 	{
    ...
    PyObject* CrpsTurbulenceIntensityFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
     {
        PyObject* producer = mProducerMap[pluggedFeatureName];
     ...
     std::map<const std::string, CreateTurbulenceIntensityCallback>& CrpsTurbulenceIntensityFactory::GetObjectNamesMap()
 	{
 		return mTurbulenceIntensitys;
 	}

 	std::map<const std::string, std::string> & CrpsTurbulenceIntensityFactory::GetTobeInstalledObjectsMap()
 	{
 		return mTobeInstalledObjectsMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetOjectDescriptionMap()
 	{
 		return mOjectDescriptionMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetOjectAndPluginMap()
 	{
 		return mOjectAndPluginMap;
 	}

 	///
 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetTitleMap()
 	{
 		return mTitleMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetLinkMap()
 	{
 		return mLinkMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetAuthorMap()
 	{
 		return mAuthorMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetDateMap()
 	{
 		return mDateMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceIntensityFactory::GetVersionMap()
 	{
 		return mVersionMap;
 	}

     std::map<const std::string, std::string> &  CrpsTurbulenceIntensityFactory::GetOjectsSkipDuringUnintallationMap()
     {
 	    return mOjectsSkipDuringUnintallationMap;
     }

     std::map<const std::string, PyObject*>& CrpsTurbulenceIntensityFactory::GetProducerMap()
     {
        return mProducerMap;
     }
     std::map<const std::string, bool>& CrpsTurbulenceIntensityFactory::GetStationarityMap()
     {
        return mStationarityMap;
     }
	
 	CrpsTurbulenceScaleFactory::CallbackMap CrpsTurbulenceScaleFactory::mTurbulenceScales;
 	std::vector<std::string> CrpsTurbulenceScaleFactory::mTurbulenceScaleNames;
 	QString CrpsTurbulenceScaleFactory::mOwnerPlugin;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mTobeInstalledObjectsMap;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mOjectDescriptionMap;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mOjectAndPluginMap;
     std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mOjectsSkipDuringUnintallationMap;

 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mTitleMap;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mLinkMap;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mAuthorMap;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mDateMap;
 	std::map<const std::string, std::string> CrpsTurbulenceScaleFactory::mVersionMap;
    std::map<const std::string, PyObject*> CrpsTurbulenceScaleFactory::mProducerMap;
    std::map<const std::string, bool> CrpsTurbulenceScaleFactory::mStationarityMap;

    CrpsTurbulenceScaleFactory::CrpsTurbulenceScaleFactory() {}
    CrpsTurbulenceScaleFactory::~CrpsTurbulenceScaleFactory()
    {
        Base::PyGILStateLocker lock;
        for (std::map<const std::string, PyObject*>::const_iterator it = mProducerMap.begin();
 	...
 	void CrpsTurbulenceScaleFactory::InitializeObject(const std::string& name, const std::string& pluginName, const std::string& publicationTitle, const std::string& publicationLink, const std::string& publicationAuthor, const std::string& publicationDate, const std::string& version, const bool& stationarity)
 	{		
 		mTobeInstalledObjectsMap[name] = pluginName;
 		mTitleMap[name] = publicationTitle;
 		mLinkMap[name] = publicationLink;
 	...
 	void CrpsTurbulenceScaleFactory::RegisterObject(const std::string& name, const std::string& pluginName, const std::string& description, CreateTurbulenceScaleCallback cb)
 	{
 		if (mTobeInstalledObjectsMap[name] != pluginName)
 		{
 			return;
 	...
 	void CrpsTurbulenceScaleFactory::UnregisterObject(const std::string& name, const std::string& pluginName)
 	{
 		if (pluginName == mOjectAndPluginMap[name] && mOjectsSkipDuringUnintallationMap.find(name) == mOjectsSkipDuringUnintallationMap.end())
 		{
 	...
 	IrpsWLTurbulenceScale *CrpsTurbulenceScaleFactory::BuildObject(const std::string& name)
 	{
    ...
    PyObject* CrpsTurbulenceScaleFactory::produceFeature(const std::string& newFeatureName, const std::string& simulationName, const std::string& pluggedFeatureName)
     {
        PyObject* producer = mProducerMap[pluggedFeatureName];
     ...
     std::map<const std::string, CreateTurbulenceScaleCallback>& CrpsTurbulenceScaleFactory::GetObjectNamesMap()
 	{
 		return mTurbulenceScales;
 	}

 	std::map<const std::string, std::string> & CrpsTurbulenceScaleFactory::GetTobeInstalledObjectsMap()
 	{
 		return mTobeInstalledObjectsMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetOjectDescriptionMap()
 	{
 		return mOjectDescriptionMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetOjectAndPluginMap()
 	{
 		return mOjectAndPluginMap;
 	}

 	///
 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetTitleMap()
 	{
 		return mTitleMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetLinkMap()
 	{
 		return mLinkMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetAuthorMap()
 	{
 		return mAuthorMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetDateMap()
 	{
 		return mDateMap;
 	}

 	std::map<const std::string, std::string>& CrpsTurbulenceScaleFactory::GetVersionMap()
 	{
 		return mVersionMap;
 	}

     std::map<const std::string, std::string> &  CrpsTurbulenceScaleFactory::GetOjectsSkipDuringUnintallationMap()
     {
 	    return mOjectsSkipDuringUnintallationMap;
     }

     std::map<const std::string, PyObject*>& CrpsTurbulenceScaleFactory::GetProducerMap()
     {
        return mProducerMap;
     }
     std::map<const std::string, bool>& CrpsTurbulenceScaleFactory::GetStationarityMap()
     {
        return mStationarityMap;
     }
    ...
    CrpsVarianceFactory::~CrpsVarianceFactory()
    {
 	...
 	IrpsWLVariance *CrpsVarianceFactory::BuildObject(const std::string& name)
 	{
    ...
    CrpsWavePassageEffectFactory::~CrpsWavePassageEffectFactory()
    {
 	...
 	IrpsWLWavePassageEffect *CrpsWavePassageEffectFactory::BuildObject(const std::string& name)
 	{
