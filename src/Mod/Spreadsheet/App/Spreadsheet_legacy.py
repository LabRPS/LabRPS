...
in LabRPS, but is still there for archiving purposes."""


import re, math, LabRPS, LabRPSGui
...
class Spreadsheet:
    ...
    LabRPS object or as a standalone python object.
    ...
    def _updateControllers(self):
        ...
        if hasattr(self,"Object"):
            obj = LabRPS.ActiveDocument.getObject(self.Object)
            if obj:
                import Draft
...
class SpreadsheetController:
    ...
    def getDataSet(self,obj):
        ...
        if hasattr(obj,"FilterType"):
            import Draft
            baseset = LabRPS.ActiveDocument.Objects
...
class SpreadsheetView(QtGui.QWidget):
    "A spreadsheet viewer for LabRPS"
    ...
    def closeEvent(self, event):
        ...
        if LabRPSGui:
            if LabRPSGui.ActiveDocument:
                LabRPSGui.ActiveDocument.resetEdit()
...
class _Command_Spreadsheet_Create:
    "the Spreadsheet_Create LabRPS command"
    ...
    def Activated(self):
        from DraftTools import translate
        LabRPS.ActiveDocument.openTransaction(str(translate("Spreadsheet","Create Spreadsheet")))
        LabRPSGui.doCommand("import Spreadsheet")
        LabRPSGui.doCommand("s = Spreadsheet.makeSpreadsheet()")
        LabRPSGui.doCommand("LabRPS.ActiveDocument.recompute()")
        LabRPSGui.doCommand("LabRPSGui.ActiveDocument.setEdit(s.Name,0)")
        LabRPS.ActiveDocument.commitTransaction()


class _Command_Spreadsheet_Controller:
    "the Spreadsheet_Controller LabRPS command"
    def GetResources(self):
    ...
    def IsActive(self):
        if LabRPSGui.Selection.getSelection():
            return True
        else:
            return False
    ...
    def Activated(self):
        import Draft
        if Draft.getType(LabRPSGui.Selection.getSelection()[0]) == "Spreadsheet":
            from DraftTools import translate
            n = LabRPSGui.Selection.getSelection()[0].Name
            LabRPS.ActiveDocument.openTransaction(str(translate("Spreadsheet","Add controller")))
            LabRPSGui.doCommand("import Spreadsheet")
            LabRPSGui.doCommand("Spreadsheet.makeSpreadsheetController(LabRPS.ActiveDocument."+n+")")
            LabRPS.ActiveDocument.commitTransaction()
            LabRPS.ActiveDocument.recompute()
...
class _Command_Spreadsheet_PropertyController:
    ...
    def Activated(self):
        ...
        if (len(sel) == 1) and Draft.getType(sel[0]) == "Spreadsheet":
            n = LabRPSGui.Selection.getSelection()[0].Name
            LabRPS.ActiveDocument.openTransaction(str(translate("Spreadsheet","Add property controller")))
            LabRPSGui.doCommand("import Spreadsheet")
            LabRPSGui.doCommand("Spreadsheet.makeSpreadsheetPropertyController(LabRPS.ActiveDocument."+n+")")
            LabRPS.ActiveDocument.commitTransaction()
            LabRPS.ActiveDocument.recompute()
        elif (len(sel) == 2):
            ...
            LabRPS.ActiveDocument.openTransaction(str(translate("Spreadsheet","Add property controller")))
            LabRPSGui.doCommand("import Spreadsheet")
            LabRPSGui.doCommand("Spreadsheet.makeSpreadsheetPropertyController(LabRPS.ActiveDocument."+s+",LabRPS.ActiveDocument."+o+")")
            LabRPS.ActiveDocument.commitTransaction()
            LabRPS.ActiveDocument.recompute()
...
def makeSpreadsheet():
    ...
    obj = LabRPS.ActiveDocument.addObject("App::FeaturePython","Spreadsheet")
    Spreadsheet(obj)
    if LabRPS.GuiUp:
...
def makeSpreadsheetController(spreadsheet,cell=None,direction=None):
    ...
    obj = LabRPS.ActiveDocument.addObject("App::FeaturePython","CellController")
...
def makeSpreadsheetPropertyController(spreadsheet,object=None,prop=None,cell=None):
    ...
    (such as "B6")."""
    obj = LabRPS.ActiveDocument.addObject("App::FeaturePython","PropertyController")
...
def addSpreadsheetView(view):
    "addSpreadsheetView(view): adds the given spreadsheet view to the LabRPS MDI area"
    if LabRPS.GuiUp:
        import Spreadsheet_rc
...
def open(filename):
    "called when labrps opens a csv file"
    import os
    docname = os.path.splitext(os.path.basename(filename))[0]
    doc = LabRPS.newDocument(docname)
    LabRPS.ActiveDocument = doc
    read(filename)
    doc.recompute()
    return doc
...
def insert(filename,docname):
    "called when labrps wants to import a csv file"
    try:
...
def export(exportList,filename):
    "called when labrps exports a csv file"
    import csv, Draft
    if not exportList:
...
#LabRPSGui.addCommand('Spreadsheet_Create',_Command_Spreadsheet_Create())
#LabRPSGui.addCommand('Spreadsheet_Controller',_Command_Spreadsheet_Controller())
#LabRPSGui.addCommand('Spreadsheet_PropertyController',_Command_Spreadsheet_PropertyController())
