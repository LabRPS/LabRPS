...
bool Cell::getStringContent(std::string & s, bool persistent) const
{
    if (expression) {
        ...
        else if (labrps_dynamic_cast<App::StringExpression>(expression.get())) {
        ...
        else if (labrps_dynamic_cast<App::ConstantExpression>(expression.get()))
            s = "=" + expression->toString();
        else if (labrps_dynamic_cast<App::NumberExpression>(expression.get()))
            s = expression->toString();
        else
            s = "=" + expression->toString(persistent);
...
void Cell::afterRestore() {
    auto expr = labrps_dynamic_cast<StringExpression>(expression.get());
    if(expr) 
        ...
        if (*value == '=') {
            ...
            if (!newExpr && isStartingWithNumber) {
                try {
                    ...
                    if (const auto fraction = labrps_dynamic_cast<OperatorExpression>(parsedExpr.get())) {
                        if (fraction->getOperator() == OperatorExpression::UNIT) {
                            const auto left = labrps_dynamic_cast<NumberExpression>(fraction->getLeft());
                            const auto right = labrps_dynamic_cast<UnitExpression>(fraction->getRight());
                            if (left && right) {
                            ...
                            const bool isNumberNom = labrps_dynamic_cast<NumberExpression>(fraction->getLeft());
                            const bool isNumberDenom = labrps_dynamic_cast<NumberExpression>(fraction->getRight());

                            // check for numbers with units in (de)nominator
                            const auto opNom = labrps_dynamic_cast<OperatorExpression>(fraction->getLeft());
                            const auto opDenom = labrps_dynamic_cast<OperatorExpression>(fraction->getRight());
                            ...
                            const auto uDenom = labrps_dynamic_cast<UnitExpression>(fraction->getRight());
                    ...
                    else if (const auto number = labrps_dynamic_cast<NumberExpression>(parsedExpr.get())) {
