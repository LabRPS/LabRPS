...
Property * Sheet::setFloatProperty(CellAddress key, double value)
{
    ...
    if (!prop || prop->getTypeId() != PropertyFloat::getClassTypeId()) {
        ...
        floatProp = labrps_dynamic_cast<PropertyFloat>(addDynamicProperty("App::PropertyFloat", name.c_str(), nullptr, nullptr, Prop_ReadOnly | Prop_Hidden | Prop_NoPersist));
    }
    else
...
Property * Sheet::setIntegerProperty(CellAddress key, long value)
{
    ...
    if (!prop || prop->getTypeId() != PropertyInteger::getClassTypeId()) {
        ...
        intProp = labrps_dynamic_cast<PropertyInteger>(addDynamicProperty(
...
Property * Sheet::setQuantityProperty(CellAddress key, double value, const Base::Unit & unit)
{
    ...
    if (!prop || prop->getTypeId() != PropertySpreadsheetQuantity::getClassTypeId()) {
        ...
        quantityProp = labrps_dynamic_cast<PropertySpreadsheetQuantity>(p);
    }
    else
...
Property * Sheet::setStringProperty(CellAddress key, const std::string & value)
{
    std::string name = key.toString(CellAddress::Cell::ShowRowColumn);
    Property * prop = props.getDynamicPropertyByName(name.c_str());
    PropertyString * stringProp = labrps_dynamic_cast<PropertyString>(prop);
    ...
    if (!stringProp) {
        ...
        stringProp = labrps_dynamic_cast<PropertyString>(addDynamicProperty("App::PropertyString", name.c_str(), nullptr, nullptr, Prop_ReadOnly | Prop_Hidden | Prop_NoPersist));
    }
...
Property * Sheet::setObjectProperty(CellAddress key, Py::Object object)
{
    std::string name = key.toString(CellAddress::Cell::ShowRowColumn);
    Property * prop = props.getDynamicPropertyByName(name.c_str());
    PropertyPythonObject * pyProp = labrps_dynamic_cast<PropertyPythonObject>(prop);
    ...
    if (!pyProp) {
        ...
        pyProp = labrps_dynamic_cast<PropertyPythonObject>(addDynamicProperty("App::PropertyPythonObject", name.c_str(), nullptr, nullptr, Prop_ReadOnly | Prop_Hidden | Prop_NoPersist));
    }
...
void Sheet::updateProperty(CellAddress key)
{
    ...
    if (cell != nullptr) {
        ...
        auto number = labrps_dynamic_cast<NumberExpression>(output.get());
        if(number) {
            long l;
            auto constant = labrps_dynamic_cast<ConstantExpression>(output.get());
            ...
            auto str_expr = labrps_dynamic_cast<StringExpression>(output.get());
            if(str_expr) 
                setStringProperty(key, str_expr->getText().c_str());
            else {
                Base::PyGILStateLocker lock;
                auto py_expr = labrps_dynamic_cast<PyObjectExpression>(output.get());
                if(py_expr) 
