...
void CmdSpreadsheetMergeCells::activated(int iMsg)
{
    ...
    if (getActiveGuiDocument()) {
        ...
        SpreadsheetGui::SheetView * sheetView = labrps_dynamic_cast<SpreadsheetGui::SheetView>(activeWindow);
...
void CmdSpreadsheetImport::activated(int iMsg)
{
    ...
    if (!fileName.isEmpty()) {
        ...
        Sheet * sheet = labrps_dynamic_cast<Sheet>(App::GetApplication().getActiveDocument()->addObject("Spreadsheet::Sheet", FeatName.c_str()));
        if (sheet){
...
bool CmdSpreadsheetExport::isActive()
{
    if (getActiveGuiDocument()) {
        Gui::MDIView* activeWindow = Gui::getMainWindow()->activeWindow();
        if (activeWindow && labrps_dynamic_cast<SpreadsheetGui::SheetView>(activeWindow))
            return true;
