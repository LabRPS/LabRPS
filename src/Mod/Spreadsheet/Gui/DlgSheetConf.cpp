    ...
    std::string &rangeConf, ObjectIdentifier &path, bool init)
{
    ...
    if(cell && cell->getExpression()) {
        ...
        if(expr->isDerivedFrom(FunctionExpression::getClassTypeId())) {
            auto fexpr = Base::labrps_dynamic_cast<FunctionExpression>(cell->getExpression());
        ...
        auto vexpr = Base::labrps_dynamic_cast<VariableExpression>(expr);
        if(vexpr) {
            auto prop = Base::labrps_dynamic_cast<PropertyEnumeration>(
                                vexpr->getPath().getProperty());
            if(prop) {
                auto obj = Base::labrps_dynamic_cast<DocumentObject>(prop->getContainer());
                if (obj && prop->hasName()) {
