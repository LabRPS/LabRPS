...
import LabRPS, Part, os, LabRPSGui
...
model_tab_filename = LabRPS.getHomePath()+ "Mod/Idf/Idflibs/footprints_models.csv"

## path to directory containing step models

step_path=LabRPS.getHomePath()+ "Mod/Idf/Idflibs/"
...
def open(filename):
    """called when labrps opens an Emn file"""
...
def insert(filename,docname):
    """called when labrps imports an Emn file"""
...
def process_emn(doc,filename):
   ...
   for emnline in emnlines:
       ...
       if current_section==".PLACEMENT"  and section_counter>1 and fmod(section_counter,2)==1:
          ...
          LabRPS.Console.PrintMessage(str(place_item)+"\n")
          placement.append(place_item)
   LabRPS.Console.PrintMessage("\n".join(passed_sections)+"\n")
   LabRPS.Console.PrintMessage("Proceed "+str(Process_board_outline(doc,board_outline,drills,board_thickness))+" outlines\n")
...
def Process_board_outline(doc,board_outline,drills,board_thickness):
       ...
       if vertex_index==0:
          lines=point[0] 
           ...
           if point[3]!=0 and point[3]!=360:
              ...
              LabRPS.Console.PrintMessage("mid point "+str(mid_point)+"\n")
           elif point[3]==360:
    ...
    if lines!=-1:
      ...
      out_face.append(Part.Face(out_shape))
      outline=out_face[0]
      LabRPS.Console.PrintMessage("Added outline\n")
      if len(out_face)>1:
        for otl_cut in out_face[1: ]:
          outline=outline.cut(otl_cut)
          LabRPS.Console.PrintMessage("Cutting shape inside outline\n")
      for drill in drills:
        LabRPS.Console.PrintMessage("Cutting hole inside outline\n")
      ...
      #LabRPSGui.Selection.addSelection(doc_outline)
      #LabRPSGui.runCommand("Draft_Upgrade")
      #outline=LabRPS.ActiveDocument.getObject("Union").Shape
      #LabRPS.ActiveDocument.removeObject("Union")
      #doc_outline=doc.addObject("Part::Feature","Board_outline")
...
def process_emp(doc,filename,placement,board_thickness):
   ...
   for empline in emplines:
     ...
     if current_section==".HEADER"  and section_counter==2:
        ...
        LabRPS.Console.PrintMessage("Emp version: "+emprecords[1]+"\n")
   ...
   for place_item in placement:
     if place_item[1] in comps:
       ...
       LabRPS.Console.PrintMessage("Adding EMP model "+str(place_item[0])+"\n")
...
def place_steps(doc,placement,board_thickness):
    ...
    LabRPS.Console.PrintMessage("Step models to be loaded for footprints: "+str(validkeys)+"\n")
    grp=doc.addObject("App::DocumentObjectGroup", "Step Lib")
    for validkey in validkeys:
         ImportGui.insert(step_path+model_dict[validkey],LabRPS.ActiveDocument.Name)
         #partName=LabRPS.ActiveDocument.ActiveObject.Name
         impPart=LabRPS.ActiveDocument.ActiveObject
         #impPart.Shape=LabRPS.ActiveDocument.ActiveObject.Shape
         #impPart.ViewObject.DiffuseColor=LabRPS.ActiveDocument.ActiveObject.ViewObject.DiffuseColor
         impPart.ViewObject.Visibility=0
         impPart.Label=validkey
         grp.addObject(impPart)
         step_dict.append((validkey,impPart))
         LabRPS.Console.PrintMessage("Reading step file "+str(model_dict[validkey])+" for footprint "+str(validkey)+"\n")
    ...
    for place_item in placement:
      if place_item[2] in step_dict:
        ...
        LabRPS.Console.PrintMessage("Adding STEP model "+str(place_item[0])+"\n")
...
def toQuaternion(heading, attitude,bank): # rotation heading=around Y, attitude =around Z,  bank attitude =around X
    """toQuaternion(heading, attitude,bank)->LabRPS.Base.Rotation(Quternion)"""
    c1 = cos(heading/2)
    s1 = sin(heading/2)
    c2 = cos(attitude/2)
    s2 = sin(attitude/2)
    ...
    y = s1*c2*c3 + c1*s2*s3
    z = c1*s2*c3 - s1*c2*s3
    return  LabRPS.Base.Rotation(x,y,z,w)  
