...
PyObject* ISSCSpectrumPy::computeSpectrum(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ddd|dd", &frequency, &significantWaveHeight, &peakFrequency, &c1, &c2))
    return nullptr;
    return Py::new_reference_to(Py::Float(getISSCSpectrumPtr()->computeSpectrum(frequency, significantWaveHeight, peakFrequency, c1, c2)));
}
