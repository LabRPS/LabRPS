                ...
                for message in responseJSON["messages"]:
                    if "type" in message:
                        if message["type"] == "info":
                            ...
                            LabRPS.Console.PrintWarning("W3C validator returned a non-document error:\n {}".format(message))
                            return

        except urllib.error.HTTPError as e:
            LabRPS.Console.PrintWarning("W3C validator returned response code {}".format(e.code))

        except urllib.error.URLError:
            LabRPS.Console.PrintWarning("Could not communicate with W3C validator")

        if errorCount > 0 or warningCount > 0:
            StartPage.exportTestFile()
            LabRPS.Console.PrintWarning("HTML validation failed: Start page source written to your home directory for analysis.")
            ...
            LabRPS.Console.PrintWarning("The Start page is valid HTML, but the W3C sent back {} informative messages:\n{}.".format(infoCount,validationResultString))
