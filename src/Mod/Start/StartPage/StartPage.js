...
function load() {
    ...
    if (allowDownloads == 1) {
        ...
        var tobj=new JSONscriptRequest('https://api.github.com/repos/LabRPS/LabRPS/commits?callback=printCommits');
        ...
        ddiv.innerHTML = "Connecting...";
        var tobj=new JSONscriptRequest('https://api.github.com/repos/LabRPS/LabRPS-addons/contents?callback=printAddons');
        tobj.buildScriptTag(); // Build the script tag
        tobj.addScriptTag(); // Execute (add) the script tag
        ...
        if (showForum == 1) {
            ...
            ddiv.innerHTML = "Connecting...";
            var tobj=new JSONscriptRequest('https://www.freecadweb.org/xml-to-json.php?callback=printForum&url=https://forum.labrpsweb.org/feed.php');
            tobj.buildScriptTag(); // Build the script tag
            tobj.addScriptTag(); // Execute (add) the script tag
            ddiv.innerHTML = "Downloading addons list...";
        }
...
function printAddons(data) {
    ...
    var blacklist = ['addons_installer.RPSMacro','LabRPS-Addon-Details.md','README.md'];
