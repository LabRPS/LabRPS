...
# Note: It is built only once per LabRPS session for now...
...
def isOpenableByLabRPS(filename):

    "check if LabRPS can handle this file type"

    if os.path.isdir(filename):
        return False
    if os.path.basename(filename)[0] == ".":
        return False
    extensions = [key.lower() for key in LabRPS.getImportType().keys()]
...
def getInfo(filename):
    ...
    tformat = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetString("TimeFormat","%m/%d/%Y %H:%M:%S")
    ...
    if os.path.exists(filename):
        ...
        if filename.lower().endswith(".fcstd"):
            ...
            # check for meta-file if it's really a LabRPS document
...
def buildCard(filename,method,arg=None):
    ...
    result = ""
    if os.path.exists(filename) and isOpenableByLabRPS(filename):
...
def handle():
    ...
    if hasattr(Start,"tempfolder"):
        ...
        tempfolder = tempfile.mkdtemp(prefix="LabRPSStartThumbnails")

    # build the html page skeleton

    resources_dir = os.path.join(LabRPS.getResourceDir(), "Mod", "Start", "StartPage")
    p = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start")
    ...
    if LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("UseStyleSheet",False):
        qssfile = LabRPS.ParamGet("User parameter:BaseApp/Preferences/MainWindow").GetString("StyleSheet","")
        if qssfile:
            # Search for stylesheet in user, system and resources locations
            user = os.path.join(LabRPS.getUserAppDataDir(), "Gui", "Stylesheets")
            system = os.path.join(LabRPS.getResourceDir(), "Gui", "Stylesheets")
    ...
    if not LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("ShowTips",True):
    ...
    # build a "create new" icon with the LabRPS background color gradient

    if not "createimg" in iconbank:
        p = LabRPS.ParamGet("User parameter:BaseApp/Preferences/View")
    ...
    rf = LabRPS.ParamGet("User parameter:BaseApp/Preferences/RecentFiles")
    ...
    if LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("NewFileGradient",False):
    ...
    SECTION_EXAMPLES = ""
    if LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("ShowExamples",True):
        SECTION_EXAMPLES = "<h2>"+TranslationTexts.T_EXAMPLES+"</h2>"
        SECTION_EXAMPLES += "<ul>"
        examples_path = LabRPS.getResourceDir()+"examples"
        if os.path.exists(examples_path):
            examples = os.listdir(examples_path)
            for basename in examples:
                filename = LabRPS.getResourceDir()+"examples"+os.sep+basename
    ...
    SECTION_CUSTOM = ""
    cfolders = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetString("ShowCustomFolder","")
    if cfolders:
        dn = 0
        for cfolder in cfolders.split(";;"): # allow several paths separated by ;;
            ...
            if not os.path.exists(cfolder):
                LabRPS.Console.PrintWarning("Custom folder not found: %s" % cfolder)
            else:
    ...
    LabRPS.getResourceDir()
    for wb in sorted(LabRPSGui.listWorkbenches().keys()):
        ...
        if wb in iconbank:
            ...
            img = os.path.join(LabRPS.getResourceDir(),"Mod",wn,"Resources","icons",wn+"Workbench.svg")
            if not os.path.exists(img):
                w = LabRPSGui.listWorkbenches()[wb]
                if hasattr(w,"Icon") and w.Icon:
                    ...
                    img = os.path.join(resources_dir,"images/labrps.png")
            iconbank[wb] = img
        UL_WORKBENCHES += '<li>'
        UL_WORKBENCHES += '<img src="file:///'+img.replace('\\','/')+'" alt="'+wn+'">&nbsp;'
        UL_WORKBENCHES += '<a href="https://www.labrpsweb.org/wiki/'+wn+'_Workbench">'+wn.replace("Reverse_Engineering","ReverseEng")+'</a>'
        UL_WORKBENCHES += '</li>'
    UL_WORKBENCHES += '</ul>'
    ...
    if LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("AllowDownload",False):
        HTML = HTML.replace("var allowDownloads = 0;","var allowDownloads = 1;")

    # enable or disable forum

    if LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("ShowForum",False):
    ...
    if LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool("ShowNotes",False):
...
def exportTestFile():
    ...
    f = open(os.path.expanduser("~")+os.sep+"labrps-startpage.html","w")
    f.write(handle())
    f.close()
...
def postStart():
    ...
    param = LabRPS.ParamGet("User parameter:BaseApp/Preferences/Mod/Start")

    # switch workbench
    wb = param.GetString("AutoloadModule","")
    if "$LastModule" == wb:
        wb = LabRPS.ParamGet("User parameter:BaseApp/Preferences/General").GetString("LastModule","")
    if wb:
        # don't switch workbenches if we are not in Start anymore
        if LabRPSGui.activeWorkbench() and (LabRPSGui.activeWorkbench().name() == "StartWorkbench"):
            LabRPSGui.activateWorkbench(wb)
...
def checkPostOpenStartPage():
    ...
    import Start
    if LabRPS.ParamGet('User parameter:BaseApp/Preferences/Mod/Start').GetBool('DoNotShowOnOpen',False) and (not hasattr(Start,'CanOpenStartPage')):
        if len(sys.argv) > 1:
            postStart()
