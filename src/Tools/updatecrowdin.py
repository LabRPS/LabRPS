...
"""
This utility offers several commands to interact with the LabRPS project on
crowdin. For it to work, you need a ~/.crowdin-labrps-token file in your
user's folder, that contains the API access token that gives access to the
crowdin LabRPS project. The API token can also be specified in the
...
THRESHOLD = 25 # how many % must be translated for the translation to be included in LabRPS
...
def load_token():
    # load API token stored in ~/.crowdin-labrps-token
    config_file = os.path.expanduser('~')+os.sep+".crowdin-labrps-token"
...
def applyTranslations(languages):
    ...
    src = os.path.join(currentfolder,"labrps.zip")
    dst = os.path.join(tempfolder,"labrps.zip")
    if not os.path.exists(src):
        print("labrps.zip file not found! Aborting. Run \"download\" command before this one.")
        sys.exit()
    shutil.copyfile(src, dst)
    os.chdir(tempfolder)
    zfile=zipfile.ZipFile("labrps.zip")
    print("extracting labrps.zip...")
    zfile.extractall()
