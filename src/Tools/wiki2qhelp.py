...
DEFAULTURL = "www.labrpsweb.org/wiki" #default URL if no URL is passed
...
OUTPUTPATH = os.path.expanduser("~")+os.sep+'.LabRPS' # Where to store the qch file
...
def joinpdf(folder=TMPFOLDER,startpage=INDEX,outputname='labrps.pdf'):
...
def compile(qhpfile,outputname='labrps.qch'):
...
def generate(qhcpfile):
    ...
    txt="""
The help files for LabRPS.
    """
    about=open(TMPFOLDER + os.sep + "about.txt","w")
    about.write(txt)
    about.close()
    qhcfile = OUTPUTPATH + os.sep + "labrps.qhc"
...
<QHelpCollectionProject version="1.0">
    <assistant>
        <title>LabRPS User Manual</title>
        <applicationIcon>Crystal_Clear_app_tutorials.png</applicationIcon>
        <cacheDirectory>labrps/labrps</cacheDirectory>
        <startPage>qthelp://org.labrps.usermanual_0.9/doc/Online_Help_Startpage.html</startPage>
        <aboutMenuText>
            <text>About LabRPS</text>
        </aboutMenuText>
        <aboutDialog>
            <file>about.txt</file>
            <!--
    ...
    <docFiles>
        <generate>
            <file>
                <input>labrps.qhp</input>
                <output>labrps.qch</output>
                </file>
            </generate>
        <register>
            <file>labrps.qch</file>
        </register>
    </docFiles>
</QHelpCollectionProject>
'''
    if VERBOSE: print ("Building project file...")
    qfilename = folder + os.sep + "labrps.qhcp"
    f = open(qfilename,'w')
...
<QtHelpProject version="1.0">
    <namespace>org.labrps.usermanual_0.9</namespace>
    <virtualFolder>doc</virtualFolder>
    <!--
    <customFilter name="LabRPS 0.10">
        <filterAttribute>LabRPS</filterAttribute>
...
'''
    ...
    qfilename = folder + os.sep + "labrps.qhp"
    f = open(qfilename,'wb')
    f.write(qhelpfile)
    f.close()
