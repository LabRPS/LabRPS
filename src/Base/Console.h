 ...
 * \section Motivation
 *
 * LabRPS Base::Console() is capable of outputting to different targets, and has
 ...
 *
 * \code{.py}
 * LabRPS.setLogLevel('Default',level)
 * LabRPS.setLogLevel('DebugDefault',level)
 * \endcode
 *
 ...
 *  This class describes an Interface for logging within LabRPS. If you want to add a new
 *  "sink" to LabRPS's logging mechanism, then inherit this class. You'll also need to
...
public:
    ...
    /// Enables or disables message types of a certain console observer
    bool IsMsgTypeEnabled(const char* sObs, LabRPS_ConsoleMsgType type) const;
