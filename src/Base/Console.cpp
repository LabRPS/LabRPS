...
public:
    ConsoleSingleton::LabRPS_ConsoleMsgType msgtype;
    std::string msg;

    ConsoleEvent(ConsoleSingleton::LabRPS_ConsoleMsgType type, const std::string& msg)
...
/**
 * \a type can be OR'ed with any of the LabRPS_ConsoleMsgType flags to enable -- if \a b is true --
...
bool ConsoleSingleton::IsMsgTypeEnabled(const char* sObs, LabRPS_ConsoleMsgType type) const
{
    ILogger* pObs = Get(sObs);
    if (pObs) {
        switch (type) {
 ...
 *  LabRPS is running in GUI mode a Message appears on the status bar.
 ...
 *  LabRPS is in GUI mode a Message Box pops up. In console
 ...
 *  Errors are used to get the users attention. That means when LabRPS
