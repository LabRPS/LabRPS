...
public:
    ...
    {
        auto vpd = Base::labrps_dynamic_cast<ViewProviderDocumentObject>(const_cast<ViewProvider*>(&vp));
...
Application::Application(bool GUIenabled)
{
    ...
    if (GUIenabled) {
        ...
        PyDoc_STRVAR(LabRPSGui_doc,
            "The functions in the LabRPSGui module allow working with GUI documents,\n"
            "view providers, views, workbenches and much more.\n\n"
            "The LabRPSGui instance provides a list of references of GUI documents which\n"
            ...
            "The LabRPSGui module also provides a set of functions to work with so called\n"
            "workbenches."
            );

        // if this returns a valid pointer then the 'LabRPSGui' Python module was loaded,
        // otherwise the executable was launched
        PyObject* modules = PyImport_GetModuleDict();
        PyObject* module = PyDict_GetItemString(modules, "LabRPSGui");
        if (!module) {
            static struct PyModuleDef LabRPSGuiModuleDef = {
                PyModuleDef_HEAD_INIT,
                "LabRPSGui", LabRPSGui_doc, -1,
                Application::Methods,
                nullptr, nullptr, nullptr, nullptr
            };
            module = PyModule_Create(&LabRPSGuiModuleDef);

            PyDict_SetItemString(modules, "LabRPSGui", module);
        }
        else {
    ...
    Base::PyGILStateLocker lock;
    PyObject *module = PyImport_AddModule("LabRPSGui");
    PyMethodDef *meth = LabRPSGui_methods;
    ...
    _pcPythonPluginDictionary = PyDict_New();
    _pcPythonFeatureDictionary = PyDict_New();
    _pcRPSPythonFeatureMaker = PyDict_New();
...
Application::~Application()
{
    ...
    Py_DECREF(_pcPythonPluginDictionary);
    Py_DECREF(_pcPythonFeatureDictionary);
    Py_DECREF(_pcRPSPythonFeatureMaker);
    }
        ...
        try {
            if (File.hasExtension("FCStd")) {
                ...
                for (auto &v : d->documents) {
                    ...
                    if (filepath == fi) {
                        handled = true;
                        Command::doCommand(Command::App, "LabRPSGui.reload('%s')", doc->getName());
                        break;
                    }
                }

                if (!handled)
                    Command::doCommand(Command::App, "LabRPS.openDocument('%s')", unicodepath.c_str());
            }
            else {
        ...
        try {
            ...
            for (std::vector<App::DocumentObject*>::iterator it = sel.begin(); it != sel.end(); ++it) {
                if (unique_objs.insert(*it).second) {
                    str << "__objs__.append(LabRPS.getDocument(\"" << DocName << "\").getObject(\""
    ...
    if (doc != d->documents.end()) {
        ...
        if (d->activeDocument != doc->second) {
            ...
            if (d->activeDocument) {
                ...
                Py::Module("LabRPSGui").setAttr(std::string("ActiveDocument"),active);
                ...
                Base::PyGILStateLocker lock;
                Py::Module("LabRPSGui").setAttr(std::string("ActiveDocument"),Py::None());
            }
        }
...
void Application::initApplication()
{
    ...
    try {
        initTypes();
        new Base::ScriptProducer( "LabRPSGuiInit", LabRPSGuiInit );
        init_resources();
...
void Application::runApplication()
{
    ...
    QIcon::setThemeSearchPaths(QIcon::themeSearchPaths() << QString::fromLatin1(":/icons/LabRPS-default"));
    ...
    if (showVersion) {
        // set main window title with LabRPS Version
    ...
    try {
        ...
        Base::Console().Error("Error in LabRPSGuiInit.py: %s\n", e.what());
        mw.stopSplasher();
        throw;
    }
...
bool Application::isThisPluginInstalled(const QString& pluginName) const
{
...
bool Application::IsObjectInstalled(QString& object) const
{
    return WorkbenchManager::instance()->active()->isObjectInstalled(object);
}

bool Application::UpdateToBeInstalledObjectsMap(const QString& lstObject) const
{
    return WorkbenchManager::instance()->active()->updateToBeInstalledObjectsMap(lstObject);
}

bool Application::UpdateToBeUninstalledObjectsMap(const QString name, const QString pluginName) const
{
    return WorkbenchManager::instance()->active()->updateToBeUninstalledObjectsMap(name, pluginName);
}

void Application::emptyAllToBeUninstalledObjectsMap() const
{
    WorkbenchManager::instance()->active()->emptyAllToBeUninstalledObjectsMap();
}

void Application::showPluginList()
{
    std::string wbName = WorkbenchManager::instance()->active()->name();

    std::unique_ptr<PluginInstallerBrowser> dlg(new PluginInstallerBrowser(QString::fromLatin1(wbName.c_str()), getMainWindow()));
    dlg->exec();
    //emit pluginModified(nullptr);
}

bool Application::InitializePlugin(const QString& name, int instUninstMOd, QString& information)
{
    ...
    if (PlugLang == "Python") {
        QFileInfo fi(name);
        QString pluginName = fi.baseName();
        std::stringstream str;
        str << "Gui.initializePythonPlugin('" << pluginName.toUtf8().constData() << "')"
            << std::endl;
        Py::Object result = Base::Interpreter().runStringObject(str.str().c_str());
        Py::Boolean ret(result);
        return static_cast<bool>(ret);
    }
    else if (PlugLang == "C++") {
        bool result = App::PluginManager::GetInstance().InitializePlugin(name, instUninstMOd, information);
...
bool Application::InstallPlugin(const QString& name)
{
    ...
    if (PlugLang == "Python") {
        ...
        QString pluginName = fi.baseName();
        std::stringstream str;
        str << "Gui.registerPythonPlugin('" << pluginName.toUtf8().constData() << "')"
            << std::endl;
        Py::Object result = Base::Interpreter().runStringObject(str.str().c_str());
        Py::Boolean ret(result);
        return static_cast<bool>(ret);
...
bool Application::UnInstallPlugin(const QString& name, bool remove)
{
    ...
    if (PlugLang == "Python") {
        ...
        QString pluginName = fi.baseName();
        std::stringstream str;
        str << "Gui.unregisterPythonPlugin('" << pluginName.toUtf8().constData() << "')" << std::endl;
        Py::Object result = Base::Interpreter().runStringObject(str.str().c_str());
        Py::Boolean ret(result);
        return static_cast<bool>(ret);
    }
...
QStringList Application::pythonPlugins() const
{
    PyObject *key, *value;
    Py_ssize_t pos = 0;
    QStringList plug;
    // insert all items
    while (PyDict_Next(_pcPythonPluginDictionary, &pos, &key, &value)) {
        /* do something interesting with the values... */
        const char* wbName = PyUnicode_AsUTF8(key);

        plug.push_back(QString::fromLatin1(wbName));
    }

    return plug;
}

bool Application::initializePythonPlugin(const char* name)
{
    // get the python plugin object from the dictionary
    Base::PyGILStateLocker lock;
    PyObject* pcPythonPlugin = PyDict_GetItemString(_pcPythonPluginDictionary, name);
    // test if the plugin exists
    if (pcPythonPlugin) {

        Py::Object handler(pcPythonPlugin);

        // get its Initialize member if possible
        try {
            // If the method Initialize is available we call it
            if (handler.hasAttr(std::string("Initialize"))) {
                Py::Object method(handler.getAttr(std::string("Initialize")));
                if (method.isCallable()) {
                    Py::Tuple args;
                    Py::Callable initialize(method);
...
bool Application::registerPythonPlugin(const char* name)
{
    // get the python plugin object from the dictionary
    Base::PyGILStateLocker lock;
    PyObject* pcPythonPlugin = PyDict_GetItemString(_pcPythonPluginDictionary, name);
    ...
    if (pcPythonPlugin) {
        ...
        // get its Register member if possible
        try {
            // If the method Register is available we call it
            if (handler.hasAttr(std::string("Register"))) {
                Py::Object method(handler.getAttr(std::string("Register")));
                if (method.isCallable()) {
                    Py::Tuple args;
                    Py::Callable rregister(method);
                    rregister.apply(args);
                    return true;
                }
            }
        }
...
bool Application::unregisterPythonPlugin(const char* name)
{
    // get the python plugin object from the dictionary
    Base::PyGILStateLocker lock;
    PyObject* pcPythonPlugin = PyDict_GetItemString(_pcPythonPluginDictionary, name);
    ...
    if (pcPythonPlugin) {
        ...
        // get its Unregister member if possible
        try {
            // If the method Unregister is available we call it
            if (handler.hasAttr(std::string("Unregister"))) {
                Py::Object method(handler.getAttr(std::string("Unregister")));
                if (method.isCallable()) {
                    Py::Tuple args;
                    Py::Callable unregister(method);
                    unregister.apply(args);
                    return true;
                }
            }
        }
