...
void SoFCOffscreenRenderer::writeToImageFile(const char* filename, const char* comment, const SbMatrix& mat, const QImage& image)
{
    ...
    if (file.hasExtension("JPG") || file.hasExtension("JPEG")) {
        ...
        std::string com;
        if (strcmp(comment,"")==0)
            com = "Screenshot created by LabRPS";
        ...
        if (supported) {
            ...
            if (file.hasExtension("PNG")) {
                ...
                img.setText(QLatin1String("Author"), QLatin1String("LabRPS (http://www.freecadweb.org)"));
                if (strcmp(comment,"")==0)
                    img.setText(QLatin1String("Description"), QLatin1String("Screenshot created by LabRPS"));
                else if (strcmp(comment,"$MIBA")==0)
