    ...
    for (auto& v : NewObjects) {
        ...
        for (auto id : v.second) {
            ...
            if (docItem->ObjectMap.count(obj))
                continue;
            auto vpd = Base::labrps_dynamic_cast<ViewProviderDocumentObject>(gdoc->getViewProvider(obj));
            if (vpd)
...
void TreeWidget::selectLinkedObject(App::DocumentObject* linked) {
    ...
    auto linkedVp = Base::labrps_dynamic_cast<ViewProviderDocumentObject>(
...
ViewProviderDocumentObject* DocumentItem::getViewProvider(App::DocumentObject* obj) {
    return Base::labrps_dynamic_cast<ViewProviderDocumentObject>(
            Application::Instance->getViewProvider(obj));
}
