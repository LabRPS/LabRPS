...
bool Document::setEdit(Gui::ViewProvider* p, int ModNum, const char *subname)
{
    ...
    // Fix regression: https://forum.labrpsweb.org/viewtopic.php?f=19&t=43629&p=371972#p371972
...
void Document::SaveDocFile (Base::Writer &writer) const
{
    writer.Stream() << "<?xml version='1.0' encoding='utf-8'?>" << std::endl
                    << "<!--" << std::endl
                    << " LabRPS Document, see http://www.freecadweb.org for more information..."
                    << std::endl << "-->" << std::endl;
    ...
    if (scheme == 1) {
        ...
        for (int i=0;i<Cnt&&it!=obj.end();++i,++it) {
            ...
            if (pObj) {
                ...
                auto vpd = Base::labrps_dynamic_cast<ViewProviderDocumentObject>(pObj);
