...
#ifndef GUI_ALPHAPLOT_H
#define GUI_ALPHAPLOT_H
...
//#include <Libraries/Alphaplot/future\table\future_Table.h>
...
#include <Libraries/Alphaplot/scripting/Script.h>
#include <Libraries/Alphaplot/scripting/ScriptingEnv.h>
...
/* brief AlphaPlot's main window.
	 ...
	 * MyWidget subclasses in plug-ins. It will also make MainWindow more
	 ...
	 * depending on how much code is left in MainWindow after the above
	 * reorganizations. Think about whether a Model/View approach can be used for
	 * Project/ProjectExplorer.*/

class GuiExport MainAlphaPlot: public QWidget, public scripted, public QScriptable
{
    Q_OBJECT
public:
    MainAlphaPlot(MainWindow* app);
    ~MainAlphaPlot();
...
public Q_SLOTS:
    ...
    //@{
    void openAproj();
    MainAlphaPlot* openAproj(const QString& fileName);
   ...
   * This table is then plotted with the Graph::LineSymbols style.*/
    MainAlphaPlot* plotFile(const QString& fn);
   ...
   * param execute specifies if the script should be executed after opening.*/
    MainAlphaPlot* loadScript(const QString& fn, bool execute = false);

    QList<QMdiSubWindow*> subWindowsList();
    QList<QMdiSubWindow*> subWindowsListFromTreeRecursive(QList<QMdiSubWindow*> list,
    ...
    //! \name Surface Plots
    //@{
    Layout3D* newPlot3D(const QString& formula, const double xl, const double xr, const double yl,
                        const double yr, const double zl, const double zr);
    Layout3D* newPlot3D(const QString& caption, const QString& formula, const double xl,
    ...
    //@{
    bool newFunctionPlot(const int type, const QStringList& formulas, const QString& var,
                         const QList<double>& ranges, const int points);
    Curve2D* addFunctionPlot(const int type, const QStringList& formulas, const QString& var,
                             const QList<double>& ranges, const int points, AxisRect2D* axisrect);
    QPair<QVector<double>*, QVector<double>*>
    generateFunctiondata(const int type, const QStringList& formulas, const QString& var,
                         const QList<double>& ranges, const int points);
    ...
    void addGraph2DAxis(QAction* action);
    void legendReorder();
    ...
    Matrix* newMatrixShowVectors(const vec& x, const vec& y, const QString caption);
    Matrix* newMatrixShowVector(const vec& x, QString caption);
    Matrix* newMatrixShowComplexVectors(const vec& x, const cx_vec& y, const QString caption, const int& complexRealImag = 1);
    Matrix* newMatrixShowComplexMatrix(const cx_mat& x, const QString caption, const int& complexRealImag = 1);
   ...
   */
    ...
    void restoreWindowGeometry(MainAlphaPlot* app, MyWidget* w, const QString s);
    ...
    QList<QPair<QPair<double, double>, double>>* generateFunctiondata(const QString& formula,
                                                                      double xl, double xr,
                                                                      double yl, double yr,
                                                                      double zl, double zr);
    ...
    void analyzeCurve(AxisRect2D* axisrect, const QString& whichFit, const QString& curveTitle);
    ...
    void showCurvesDialog(QAction* action);
   ...
   */
    ...
    bool findRecursive(FolderTreeWidgetItem* item, FindItem findItem, QString s, bool labels,
...
private:
    ...
    bool isActiveSubWindow(QMdiSubWindow* subwindow, const SubWindowType& subwindowtype);

    // Stores the pointers to the dragged items from the FolderListViews objects
    QList<QTreeWidgetItem*> draggedItems;

    AlphaPlotAprojHandler* aprojhandler_;
...
public Q_SLOTS:
    ...
    void verticalTowLayers();
    void horizontalTowLayers();
    void fourLayers();
    void treeDWireFrame();
    void treeDSurface();
    void treeDWireSurface();
    void treeDWireFramePolar();
    void treeDSurfacePolar();
    void treeDWireFrameSurfacePolar();
    void treeDScatterPolar();
    void treeDPolarSpectrogram();
    bool isActiveSubWindowAGraph();
    bool isActiveSubWindowA3DPlot();
    bool isActiveSubWindowAScene();
...
public:
    ...
    void getActiveMatrixAsEigenMatrix(mat & matrix);
    Table* getActiveTable();
    void getActiveTableAsEigenMatrix(mat& matrix);
