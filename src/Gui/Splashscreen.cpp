...
void AboutDialog::setupLabels()
{
    ...
    // gets replaced to "<b>LabRPS</b>", for example
    ...
    if (it != config.end()) {
        ...
        if (auto url_itr = config.find("BuildRepositoryURL"); url_itr != config.end()) {
            ...
            if (url == QString::fromUtf8("Unknown"))
                url = QString::fromUtf8("https://github.com/LabRPS/LabRPS"); // Just take a guess
...
void AboutDialog::showCredits()
{
    ...
    creditsHTML += tr("LabRPS would not be possible without the contributions of");
