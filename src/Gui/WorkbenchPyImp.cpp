...
PyObject* WorkbenchPy::getAllPlugins(PyObject* args)
{
    PY_TRY
    {
        if (!PyArg_ParseTuple(args, ""))
        ...
        if (active)
        {
            std::list<std::string> plugins = active->getAllPlugins();
...
PyObject* WorkbenchPy::getAllInstalledPlugins(PyObject* args)
{
    PY_TRY
    {
        if (!PyArg_ParseTuple(args, ""))
        ...
        if (active) {
            std::list<std::string> plugins = active->getAllInstalledPlugins();
            for (std::list<std::string>::iterator it = plugins.begin(); it != plugins.end(); ++it) {
                list.append(Py::String(*it));
            }
        }
...
PyObject* WorkbenchPy::getPhenomenonName(PyObject* args)
{
    PY_TRY
    {
        if (!PyArg_ParseTuple(args, ""))
        ...
        Py::String ph = Py::String(name.toUtf8().constData());
        return Py::new_reference_to(ph);

    }
    PY_CATCH;
}

PyObject* WorkbenchPy::installPlugin(PyObject* args)
{
    PY_TRY
    {
        char* sPluginName;
        if (!PyArg_ParseTuple(args, "s", &sPluginName))
            return nullptr;

        Workbench* active = Gui::WorkbenchManager::instance()->active();
        bool ret;
        if (active)
            ret = active->installPlugin(sPluginName);
        Py::Boolean resutl = Py::Boolean(ret);
        return Py::new_reference_to(resutl);
    }
    PY_CATCH;
}

PyObject* WorkbenchPy::unInstallPlugin(PyObject* args)
{
    PY_TRY
    {
        char* sPluginName;
        ...
        bool ret;
        if (active)
            ret = active->unInstallPlugin(sPluginName);
        Py::Boolean resutl = Py::Boolean(ret);
        return Py::new_reference_to(resutl);
    }
    PY_CATCH;
...
PyObject* WorkbenchPy::createSimulation(PyObject* args)
{
    PY_TRY
    {
        char* sSimulationName;
        if (!PyArg_ParseTuple(args, "s", &sSimulationName))
            return nullptr;

        Workbench* active = Gui::WorkbenchManager::instance()->active();
        if (active)
            active->createSimulation(sSimulationName);
        Py_Return;
    }
    PY_CATCH;
}

PyObject* WorkbenchPy::createFeature(PyObject* args)
{
    PY_TRY
    {
        char* simulationName;
        ...
        if (!PyArg_ParseTuple(args, "ssssi", &simulationName, &featureTypeName, &featureName, &featureGroup, &featureType))
            return nullptr;

        Workbench* active = Gui::WorkbenchManager::instance()->active();
        if (active)
            active->createFeature(simulationName, featureTypeName, featureName, featureGroup, featureType);
        Py_Return;
    }
    PY_CATCH;
}
