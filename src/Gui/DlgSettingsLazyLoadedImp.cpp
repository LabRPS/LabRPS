...
{
    // There are two different "autoload" settings: the first, in LabRPS since 2004, 
    // controls the module the user sees first when starting LabRPS, and defaults to the Start workbench
...
void DlgSettingsLazyLoadedImp::buildUnloadedWorkbenchList()
{
    ...
    for (const auto& wbName : workbenches) {
        ...
        autoloadCheckbox->setToolTip(tr("If checked") + 
                                     ...
                                     tr("will be loaded automatically when LabRPS starts up"));
