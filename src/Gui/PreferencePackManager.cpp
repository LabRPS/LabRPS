...
{
    // Run the pre.RPSMacro, if it exists: if it raises an exception, abort the process
    auto preMacroPath = _path / "pre.RPSMacro";
    if (fs::exists(preMacroPath)) {
        try {
            Base::Interpreter().runFile(preMacroPath.string().c_str(), false);
        }
        catch (...) {
            Base::Console().Message("PreferencePack application aborted by the preferencePack's pre.RPSMacro");
            return false;
        }
    }
    ...
    // Run the Post.RPSMacro, if it exists
    auto postMacroPath = _path / "post.RPSMacro";
    if (fs::exists(postMacroPath)) {
        try {
            Base::Interpreter().runFile(postMacroPath.string().c_str(), false);
        }
        catch (...) {
            Base::Console().Message("PreferencePack application reverted by the preferencePack's post.RPSMacro");
...
void PreferencePackManager::save(const std::string& name, const std::vector<TemplateFile>& templates)
{
    ...
    if (fs::exists(savedPreferencePacksDirectory / "package.xml")) {
        ...
        metadata->addMaintainer(App::Meta::Contact("No Maintainer", "email@labrpsweb.org"));
        metadata->addLicense(App::Meta::License("(Unspecified)", "(Unspecified)"));
        metadata->addUrl(App::Meta::Url("https://github.com/LabRPS/LabRPS", App::Meta::UrlType::repository));
    }
