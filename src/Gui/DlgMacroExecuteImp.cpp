...
void DlgMacroExecuteImp::fillUpList()
{
    ...
    QDir dir(this->macroPath, QLatin1String("*.RPSMacro *.py"));
    ...
    dir = QDir(dirstr, QLatin1String("*.RPSMacro *.py"));
...
/**
 * Specify the location of your macro files. The default location is LabRPS's home path.
 */
...
void DlgMacroExecuteImp::on_createButton_clicked()
{
    ...
    if (!fn.isEmpty())
    {
        ...
        if (suffix != QLatin1String("rpsmacro") && suffix != QLatin1String("py"))
            fn += QLatin1String(".RPSMacro");
 ...
 * "MyMacro.RPSMacro" becomes "MyMacro@001.RPSMacro"
 * "MyMacro@002.RPSMacro.py" becomes "MyMacro@003.RPSMacro.py" unless there is
 * no already existing "MyMacro@001.RPSMacro.py"
 */
void DlgMacroExecuteImp::on_duplicateButton_clicked()
{
    ...
    //example: mymacro@005.fix_bug_in_dialog.RPSMacro
    //and then when duplicating to have the extra note removed so the suggested new name is:
    //mymacro@006.RPSMacro instead of mymacro@006.fix_bug_in_dialog.RPSMacro since the new duplicate will be given a new note
    ...
    //completeSuffix = RPSMacro or py or RPSMacro.py or else suffix will become RPSMacro below
    //if ignoreExtra any extra notes added between @NN. and .RPSMacro will be ignored
