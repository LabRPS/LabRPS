...
{
    ui->setupUi(this);
    ui->extractSource->setFilter(QString::fromLatin1("%1 (*.rps)").arg(tr("Project file")));
}
...
void DlgProjectUtility::on_createButton_clicked()
{
    ...
    dest = QDir(dest).absoluteFilePath(QString::fromUtf8("project.rps"));
    ...
    if (ui->checkLoadProject->isChecked())
        Application::Instance->open((const char*)dest.toUtf8(),"LabRPS");
}
