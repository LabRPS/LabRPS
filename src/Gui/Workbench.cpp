...
/** \defgroup workbench Workbench Framework
    ...
    LabRPS provides the possibility to have one or more workbenches for a module.
 ...
 * Once you have implemented your workbench class you have to register it to make it known to the LabRPS core system. You must make sure that the step
 ...
 * your workbench, if not, the default LabRPS icon is taken, and finally the most important method GetClassName(). that represents the link between
 ...
 * \note You must make sure to choose a unique name for your workbench (in this example "My workbench"). Since LabRPS doesn't provide a mechanism for
 ...
 * LabRPS and saves memory.
 * 
 * At startup LabRPS scans all module directories and invokes InitGui.py. So an item for a workbench gets created. If the user
 ...
 *
 * LabRPS provides also the possibility to define pure Python workbenches. Such workbenches are temporarily only and are lost after exiting
 * the LabRPS session. But if you want to keep your Python workbench you can write a macro and attach it with a user defined button or just
 * perform the macro during the next LabRPS session.
 ...
 * LabRPSGui.addWorkbench(w)                                 # Creates an item for our workbench now
...
MenuItem* StdWorkbench::setupMenuBar() const
{
    ...
    *AddAndRemoveOtherPlots << "Std_Graph_AddRemove_other_plots_y"
    ...
    AddLeftAxis->setCommand("Add Left Axis");
    *AddLeftAxis << "Std_AddLeftAxisValue"
                 << "Std_AddLeftAxisLog"
                 << "Std_AddLeftAxisPi"
                 << "Std_AddLeftAxisText"
                 << "Std_AddLeftAxisTime"
    ...
    // Add Bottom axis
    MenuItem* AddBottomAxis = new MenuItem;
    AddBottomAxis->setCommand("Add Bottom Axis");
    *AddBottomAxis << "Std_AddBottomAxisValue"
    ...
    // Add right axis
    MenuItem* AddRightAxis = new MenuItem;
    AddRightAxis->setCommand("Add Right Axis");
    *AddRightAxis << "Std_AddRightAxisValue"
                  << "Std_AddRightAxisLog"
                  << "Std_AddRightAxisPi"
    ...
    // Add top axis
    MenuItem* AddTopAxis = new MenuItem;
    AddTopAxis->setCommand("Add Top Axis");
    *AddTopAxis << "Std_AddTopAxisValue"
                << "Std_AddTopAxisLog"
                << "Std_AddTopAxisPi"
                << "Std_AddTopAxisText"
                << "Std_AddTopAxisTime"
    ...
    // Add axis
    MenuItem* AddAxis = new MenuItem;
    AddAxis->setCommand("Add Axis");
    *AddAxis << AddLeftAxis << AddBottomAxis << AddRightAxis << AddTopAxis;
...
bool Workbench::isObjectInstalled(QString& object) const
{
	return false;
}
bool Workbench::updateToBeInstalledObjectsMap(const QString& lstObject) const
{
	return false;
}
bool Workbench::updateToBeUninstalledObjectsMap(const QString name, const QString pluginName) const
{
	return false;
}

void Workbench::emptyAllToBeUninstalledObjectsMap() const
{

}

void Workbench::restoreSimulations(const App::Document &doc, const std::vector<App::DocumentObject*>& objs){}

bool Workbench::isThisPluginInstalled(const QString& pluginName) const
{
	return false;
}

QString Workbench::getPhenomenonName() const
{
	return QString::fromLatin1("");
}

std::list<std::string> Workbench::getAllPlugins() const
{ 
    return std::list<std::string>(); 
}
std::list<std::string> Workbench::getAllInstalledPlugins() const
{
    return std::list<std::string>();
}

bool Workbench::installPlugin(const std::string pluginName) { return false; }
