...
static std::string containerName(const App::PropertyContainer *c) {
    auto doc = Base::labrps_dynamic_cast<App::Document>(c);
    if(doc)
        return doc->getName();
    auto obj = Base::labrps_dynamic_cast<App::DocumentObject>(c);
    if(obj)
        return obj->getFullName();
    auto vpd = Base::labrps_dynamic_cast<ViewProviderDocumentObject>(c);
    if(vpd)
