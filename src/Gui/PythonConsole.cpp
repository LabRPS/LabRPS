...
void PythonConsole::runSource(const QString& line)
{
    ...
    try {
        ...
        QMessageBox::critical(this, tr("Python console"), tr("Unhandled LabRPS exception."));
    }
    ...
    if (source->hasUrls()) {
        ...
        for (QList<QUrl>::ConstIterator it = uri.begin(); it != uri.end(); ++it) {
            ...
            if (info.exists() && info.isFile()) {
                ...
                if (ext == QLatin1String("py") || ext == QLatin1String("rpsmacro"))
                    return true;
            }
        }
    }
...
void PythonConsole::insertFromMimeData (const QMimeData * source)
{
    ...
    if (source->hasUrls()) {
        ...
        for (QList<QUrl>::ConstIterator it = uri.begin(); it != uri.end(); ++it) {
            ...
            if (info.exists()) {
                existingFile = true;
                if (info.isFile() && (ext == QLatin1String("py") || ext == QLatin1String("rpsmacro"))) {
    ...
    // https://forum.labrpsweb.org/viewtopic.php?f=3&t=34618
...
void PythonConsole::onSaveHistoryAs()
{
    ...
    QString fn = FileDialog::getSaveFileName(this, tr("Save History"), cMacroPath,
        QString::fromLatin1("%1 (*.RPSMacro *.py)").arg(tr("Macro Files")));
    if (!fn.isEmpty()) {
