  ...
  : QMainWindow( parent, f/*WDestructiveClose*/ )
{
    ...
    if (hiddenDockWindows.find("Std_PythonView") == std::string::npos) {
        ...
        pDockMgr->registerDockWindow("Std_PythonView", pcPython);
         ...
         // Plot explorer
    if (hiddenDockWindows.find("Std_PlotExplorerView") == std::string::npos) {
        PlotExplorer* pcPlot = new PlotExplorer(this);
        pcPlot->setWindowIcon(Gui::BitmapFactory().iconFromTheme("applications-python"));
        pcPlot->setObjectName(
            QString::fromLatin1(QT_TRANSLATE_NOOP("QDockWidget", "Plot explorer")));
        pDockMgr->registerDockWindow("Std_PlotExplorerView", pcPlot);
    }

    // result view
    if (hiddenDockWindows.find("Std_ResultView") == std::string::npos) {
        ResultView* pcResult = new ResultView(this);
        pcResult->setWindowIcon(Gui::BitmapFactory().iconFromTheme("applications-python"));
        pcResult->setObjectName(QString::fromLatin1(QT_TRANSLATE_NOOP("QDockWidget", "Result view")));
        pDockMgr->registerDockWindow("Std_ResultView", pcResult);
    }

        myPlot = new MainAlphaPlot(this);
        // Plot explorer
    if (hiddenDockWindows.find("Std_PlotPropertyView") == std::string::npos) {
        PlotPropertyEditor* pcPlot = new PlotPropertyEditor(this,myPlot);
        pcPlot->setWindowIcon(Gui::BitmapFactory().iconFromTheme("applications-python"));
        pcPlot->setObjectName(
            QString::fromLatin1(QT_TRANSLATE_NOOP("QDockWidget", "Plot property")));
        pDockMgr->registerDockWindow("Std_PlotPropertyView", pcPlot);
    }
...
void MainWindow::closeEvent (QCloseEvent * e)
{
    ...
    if (e->isAccepted()) {
        ...
        // https://forum.labrpsweb.org/viewtopic.php?f=8&t=67748
...
void MainWindow::delayedStartup()
{
    ...
    if (App::Application::Config()["RunMode"] == "Internal") {
        try {
            Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("LabRPSTest"));
        }
        catch (const Base::SystemExitException&) {
            throw;
        }
...
void MainWindow::addSubWindow(MyWidget* w) { d->mdiArea->addSubWindow(w); }
void MainWindow::setActiveSubWindow(MyWidget* w) { d->mdiArea->setActiveSubWindow(w); }
bool MainWindow::isActiveWindow() { return d->mdiArea->isActiveWindow(); }
QList<QMdiSubWindow*> MainWindow::subWindowList() { return d->mdiArea->subWindowList(); }
QMdiSubWindow* MainWindow::getactiveMyWidget() { return d->mdiArea->activeSubWindow(); }
MainAlphaPlot* MainWindow::getAlphaPlot() { return myPlot; }

void MainWindow::showResults(bool ok)
{
    if (ok) {
        if (!logInfo.isEmpty())
        {
            auto resultView = getMainWindow()->findChild<ResultView*>();
            resultView->editor->setHtml(logInfo);
        }

        getMainWindow()->findChild<ResultView*>()->show();
        QTextCursor cursor = getMainWindow()->findChild<ResultView*>()->editor->textCursor();
        cursor.movePosition(QTextCursor::End);
        getMainWindow()->findChild<ResultView*>()->editor->setTextCursor(cursor);
    }
}

void MainWindow::showResults(const QString& text, bool ok)
{
    logInfo += text;
    if (!logInfo.isEmpty())
        getMainWindow()->findChild<ResultView*>()->editor->setHtml(logInfo);
    showResults(ok);
}

void MainWindow::clearResultLogInfo()
{
    if (!logInfo.isEmpty()) {
        logInfo = QString();
        getMainWindow()->findChild<ResultView*>()->editor->setHtml(logInfo);
...
void MainWindow::setResultLogInfo(QString info)
{ logInfo = info; }
