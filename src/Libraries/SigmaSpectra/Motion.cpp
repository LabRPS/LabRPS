...
const double Motion::dtime() const { return m_dt; }
...
double Motion::AI() const { return m_ariasInt; }
...
bool Motion::processFile() {
  ...
  m_vel = cumtrapz(m_acc, m_dt, 9806.65);
  ...
  for (int i = 1; i < n; i++) {
    arias[i] =
        arias.at(i - 1) +
        M_PI * 0.25 * m_dt * (pow(m_acc.at(i), 2) + pow(m_acc.at(i - 1), 2));
  }
