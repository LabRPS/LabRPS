...
ConfigDialog::ConfigDialog(QWidget *parent, Qt::WindowFlags fl)
  ...
  Q_ASSERT(app_);
  setWindowIcon(IconLoader::load(QString::fromLatin1("edit-preference"), IconLoader::LightDark));
  setModal(true);
  ...
  //titleFont = app_->plotTitleFont;//koffa the way
...
void ConfigDialog::initTablesPage() {
  ...
  tablesPageLayout->addStretch();*///koffa the way
}
...
//  QStringList llist;
//  llist.append(QString::fromLatin1("<None>"));
...
void ConfigDialog::initFittingPage() {
  ...
  connect(generatePointsBtn, SIGNAL(toggled(bool)), this,
          SLOT(showPointsBox(bool)));*///koffa the way (all above)
}
...
void ConfigDialog::initCurvesPage() {
  ...
  curvesPageLayout->addWidget(curvesGroupBox);*///koffa the way
}
...
void ConfigDialog::initConfirmationsPage() {
  ...
  confirmPageLayout->addWidget(groupBoxConfirm);*///koffa the way
}
...
//  itemsList->item(0)->setIcon(QIcon(QPixmap(QString::fromLatin1(":/config_general.xpm"))));
//  itemsList->item(1)->setIcon(QIcon(QPixmap(QString::fromLatin1(":/config_table.xpm"))));
//  itemsList->item(2)->setIcon(QIcon(QPixmap(QString::fromLatin1(":/config_plot2d.xpm"))));
//  itemsList->item(3)->setIcon(QIcon(QPixmap(QString::fromLatin1(":/config_plot3d.xpm"))));
//  itemsList->item(4)->setIcon(QIcon(QPixmap(QString::fromLatin1(":/config_fit.xpm"))));
...
//  boxCurveStyle->addItem(
//      IconLoader::load(QString::fromLatin1("graph2d-spline"), IconLoader::LightDark), tr(" Spline"));
//  boxCurveStyle->addItem(
...
//  boxResolution->setSpecialValueText(QString::fromLatin1("1 ") + tr("(all data shown)"));
...
//  groupBoxMultiPeak->setTitle(tr("Display Peak Curves for Multi-peak Fits"));
//  lblPeaksColor->setText(tr("Peaks Color"));//koffa the way
}
...
//
//  if (sep.contains(QRegExp(QString::fromLatin1("[0-9.eE+-]"))) != 0) {
...
//  itemsList->setMaximumWidth(itemsList->iconSize().width() + width + 50);
//  // resize the list to the maximum width
//  itemsList->resize(itemsList->maximumWidth(), itemsList->height());//koffa the way
}
...
void ConfigDialog::insertLanguagesList() {
  ...
  for (int i = 0; i < locales.size(); i++) {
    if (locales[i] == QString::fromLatin1("en"))
      languages.push_back(QString::fromLatin1("English"));
    else {
      QTranslator translator;
      translator.load(QString::fromLatin1("alphaplot_") + locales[i], app_->qmPath);
  ...
  boxLanguage->setCurrentIndex(lang);*///koffa the way
}
