...
Table::Table(const QString& fname, const QString& sep,
             ...
             ScriptingEnv* env)
    : TableView(label, parent, QString::fromLatin1(name), f), scripted(env) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    d_future_table = qobject_cast<Gui::TableViewProvider *>(filter.importAspect(file));
    if (!d_future_table)
      d_future_table = new Gui::TableViewProvider(0, 0, 0, label);
    else
...
Table::Table(int r, int c, const QString& label,
             QWidget *parent, const char *name, Qt::WindowFlags f, ScriptingEnv* env)
    : TableView(label, parent, QString::fromLatin1(name), f), scripted(env) {
  d_future_table = new Gui::TableViewProvider(0, r, c, label);
  init();
}
...
void Table::init() {
  ...
  for (int i = 0; i < columnCount(); i++)
    ui.add_reference_combobox->addItem(QString::fromLatin1("col(\"") + column(i)->name() + QString::fromLatin1("\")"));

  ui.add_function_combobox->addItems(scriptEnv->mathFunctions());
  updateFunctionDoc();

  connect(
      d_future_table, &Gui::TableViewProvider::columnModeLocked, this,
      [&](const QString &name) {
...
void Table::handleChange() { Q_EMIT modifiedWindow(this); }
...
void Table::handleColumnsAboutToBeRemoved(int first, int count) {
  for (int i = first; i < first + count; i++)
    Q_EMIT aboutToRemoveCol(this, colName(i));
}

void Table::handleColumnsRemoved(int first, int count) {
  for (int i = first; i < first + count; i++) Q_EMIT removedCol(colName(i));
}

void Table::handleRowChange() {
  for (int i = 0; i < numCols(); i++) Q_EMIT modifiedData(this, colName(i));
}
...
void Table::print(const QString &fileName) {
  ...
  if (!fileName.isEmpty()) {
    printer.setCreator(QString::fromLatin1("AlphaPlot"));
  ...
  for (i = 0; i < rows; i++) {
    right = margin;
    QString cell_text =
        d_view_widget->model()->headerData(i, Qt::Vertical).toString() + QString::fromLatin1("\t");
    ...
    for (int j = 0; j < cols; j++) {
      int w = columnWidth(j);
      cell_text = text(i, j) + QString::fromLatin1("\t");
      tr = p.boundingRect(tr, Qt::AlignCenter, cell_text);
      br.setTopLeft(QPoint(right, height));
      br.setWidth(w);
      br.setHeight(tr.height());
...
void Table::setColumnTypes(const QStringList &ctl) {
  ...
  for (int i = 0; i < n; i++) {
    QStringList l = ctl[i].split(QString::fromLatin1(";"));
    switch (l[0].toInt()) {
...
void Table::setCommands(const QString &com) {
  QStringList lst = com.split(QString::fromLatin1("\t"));
  lst.pop_front();
  setCommands(lst);
}
...
bool Table::recalculate(int col, bool only_selected_rows) {
  ...
  if (only_selected_rows) {
    ...
    Q_FOREACH (Interval<int> i, deselected)
      Interval<int>::subtractIntervalFromList(&formula_intervals, i);
  }
  Q_FOREACH (Interval<int> interval, formula_intervals) {
    QString formula = col_ptr->formula(interval.start());
    if (formula.isEmpty()) continue;

    Script *colscript =
        scriptEnv->newScript(formula, this, QString::fromLatin1("<%1>").arg(colName(col)));
...
QStringList Table::selectedColumns() {
  ...
  for (int i = 0; i < numCols(); i++) {
    if (isColumnSelected(i)) names << name() + QString::fromLatin1("_") + column(i)->name();
  }
  return names;
}
...
QStringList Table::YColumns() {
  ...
  for (int i = 0; i < numCols(); i++) {
    if (column(i)->plotDesignation() == AlphaPlot::Y)
      names << name() + QString::fromLatin1("_") + column(i)->name();
  }
  return names;
}
...
QStringList Table::columnsList() {
  // TODO for 0.3.0: Column * list
  QStringList names;
  for (int i = 0; i < numCols(); i++) names << name() + QString::fromLatin1("_") + column(i)->name();
...
QString Table::colName(
  ...
  return QString(name() + QString::fromLatin1("_") + column(col)->name());
}
...
void Table::setHeader(QStringList header) {
  ...
  for (int i = 0; i < header.count() && i < d_future_table->columnCount(); i++)
    quarantine << column(i);
  int i = 0;
  Q_FOREACH (Column *col, quarantine) {
...
bool Table::exportASCII(const QString &fname, const QString &separator,
  ...
  if (withLabels) {
    ...
    QStringList ls = header.filter(QRegExp(QString::fromLatin1("\\D")));
    if (exportSelection) {
      for (i = 1; i < selectedCols; i++) {
        if (ls.count() > 0)
          out << header[sCols[i]] + separator;
        else
          out << QString::fromLatin1("C") + header[sCols[i]] + separator;
      }

      if (ls.count() > 0)
        out << header[sCols[selectedCols]] + QString::fromLatin1("\n");
      else
        out << QString::fromLatin1("C") + header[sCols[selectedCols]] + QString::fromLatin1("\n");
    } else {
      if (ls.count() > 0) {
        for (j = 0; j < cols - 1; j++) out << header[j] + separator;
        out << header[cols - 1] + QString::fromLatin1("\n");
      } else {
          for (j = 0; j < cols - 1; j++) out << QString::fromLatin1("C") + header[j] + separator;
        out << QString::fromLatin1("C") + header[cols - 1] + QString::fromLatin1("\n");
      }
    }
...
void Table::handleAspectDescriptionChange(const AbstractAspect *aspect) {
  ...
  if (col && d_future_table->columnIndex(col) != -1 &&
    ...
    Q_EMIT changedColHeader(name() + QString::fromLatin1("_") + old_name, name() + QString::fromLatin1("_") + new_name);

    for (int i = 0; i < d_future_table->columnCount(); i++) {
      QList<Interval<int> > formula_intervals = column(i)->formulaIntervals();
      Q_FOREACH (Interval<int> interval, formula_intervals) {
        QString formula = column(i)->formula(interval.start());
        if (formula.contains(QString::fromLatin1("\"") + old_name + QString::fromLatin1("\""))) {
          formula.replace(QString::fromLatin1("\"") + old_name + QString::fromLatin1("\""), QString::fromLatin1("\"") + new_name + QString::fromLatin1("\""));
...
void Table::importASCII(const QString &fname, const QString &sep,
  ...
  QFile file(fname);
  if (file.open(QIODevice::ReadOnly)) {
    Gui::TableViewProvider *temp =
        static_cast<Gui::TableViewProvider *>(filter.importAspect(file));
    if (!temp) return;
