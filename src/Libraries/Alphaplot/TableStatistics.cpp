...
TableStatistics::TableStatistics(ScriptingEnv* env, QWidget* parent,
    ...
    : Table(1, 1, QString::fromLatin1(""), parent, "", Qt::SubWindow, env),
      d_base(base),
      d_type(t),
      d_targets(targets) {
  ...
  if (d_type == TableStatistics::StatRow) {
    setName(QString(d_base->name()) + QString::fromLatin1("-") + tr("RowStats"));
    ...
    setColName(7, tr("Max"));
    setColName(8, tr("Min"));
    setColName(9, QString::fromLatin1("N"));
    ...
    setName(QString(d_base->name()) + QString::fromLatin1("-") + tr("ColStats"));
    ...
    setColName(11, QString::fromLatin1("N"));
...
void TableStatistics::update(Table *t, const QString &colName) {
  ...
  if (d_type == TableStatistics::StatRow) {
    ...
    for (int destRow = 0; destRow < d_targets.size(); destRow++) {
      if (colName ==
          QString(d_base->name()) + QString::fromLatin1("_") +
        ...
        column(1)->setTextAt(destRow, QString::fromLatin1("[1:") + QString::number(rows) + QString::fromLatin1("]"));
        column(2)->setValueAt(destRow, mean);
...
void TableStatistics::renameCol(const QString &from, const QString &to) {
  ...
  for (int c = 0; c < d_targets.size(); c++)
    if (from == QString(d_base->name()) + QString::fromLatin1("_") + text(c, 0)) {
      column(0)->setTextAt(c, to.section(QChar::fromLatin1('_'), 1, 1));
      return;
    }
}
...
void TableStatistics::removeCol(const QString &col) {
  ...
  for (int c = 0; c < d_targets.size(); c++)
    if (col == QString(d_base->name()) + QString::fromLatin1("_") + text(c, 0)) {
