...
ImageItem2D::ImageItem2D(AxisRect2D *axisrect, Plot2D *plot,
    ...
    : QCPItemPixmap(plot),
      axisrect_(axisrect),
      layername_(
          QString::fromLatin1("<Image2DItem>") +
...
QPointF ImageItem2D::getposition_imageitem() const {
  return position(QString::fromLatin1("topLeft"))->pixelPosition();
}
...
void ImageItem2D::save(XmlStreamWriter *xmlwriter) {
  xmlwriter->writeStartElement(QString::fromLatin1("imageitem"));
  xmlwriter->writeAttribute(QString::fromLatin1("file"), imagefilename_);
  xmlwriter->writeAttribute(QString::fromLatin1("x"),
                            QString::number(position(QString::fromLatin1("topLeft"))->coords().x()));
  xmlwriter->writeAttribute(QString::fromLatin1("y"),
                            QString::number(position(QString::fromLatin1("topLeft"))->coords().y()));
  xmlwriter->writeAttribute(
      QString::fromLatin1("bottomrightx"), QString::number(position(QString::fromLatin1("bottomRight"))->coords().x()));
  xmlwriter->writeAttribute(
      QString::fromLatin1("bottomrighty"), QString::number(position(QString::fromLatin1("bottomRight"))->coords().y()));
  xmlwriter->writeAttribute(QString::fromLatin1("rotation"), QString::number(rotation_));
  xmlwriter->writePen(pen());
  xmlwriter->writeEndElement();
}

bool ImageItem2D::load(XmlStreamReader *xmlreader) {
  if (xmlreader->isStartElement() && xmlreader->name() == QString::fromLatin1("imageitem")) {
    bool ok;
    ...
    if (ok) {
      ...
      if (ok) {
        position(QString::fromLatin1("topLeft"))->setCoords(QPointF(orix, oriy));
      } else
    ...
    double botx = xmlreader->readAttributeDouble(QString::fromLatin1("bottomrightx"), &ok);
    if (ok) {
      double boty = xmlreader->readAttributeDouble(QString::fromLatin1("bottomrighty"), &ok);
      if (ok) {
        position(QString::fromLatin1("bottomRight"))->setCoords(QPointF(botx, boty));
      } else
        xmlreader->raiseWarning(
    ...
    // rotation
    int rotation = xmlreader->readAttributeInt(QString::fromLatin1("rotation"), &ok);
    (ok)
...
void ImageItem2D::mouseReleaseEvent(QMouseEvent *event,
  ...
  if (event->button() == Qt::LeftButton) {
    ...
    Q_EMIT axisrect_->ImageItem2DMoved();
  }
}
