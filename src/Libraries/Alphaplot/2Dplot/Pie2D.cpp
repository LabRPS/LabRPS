...
Pie2D::Pie2D(AxisRect2D *axisrect, const Graph2DCommon::PieStyle &style,
    ...
    : QCPAbstractItem(axisrect->parentPlot()),
      ...
      layername_(QString::fromLatin1("<Pie2D>") + QDateTime::currentDateTime().toString(
  ...
  (style == Graph2DCommon::PieStyle::Pie)
      ? icon_ = IconLoader::load(QString::fromLatin1("graph2d-pie"), IconLoader::LightDark)
      : icon_ = IconLoader::load(QString::fromLatin1("graph2d-halfpie"), IconLoader::LightDark);
...
Pie2D::~Pie2D() {
  ...
  pieColors_->clear();
  delete pieColors_;
  /*Q_FOREACH (PieLegendItem2D *item, *pieLegendItems_) {
...
void Pie2D::save(XmlStreamWriter *xmlwriter) {
  xmlwriter->writeStartElement(QString::fromLatin1("pie"));
  // data
  xmlwriter->writeAttribute(QString::fromLatin1("table"), table_->name());
  xmlwriter->writeAttribute(QString::fromLatin1("xcolumn"), xcolumn_->name());
  xmlwriter->writeAttribute(QString::fromLatin1("ycolumn"), ycolumn_->name());
  ...
  switch (style_) {
    case Graph2DCommon::PieStyle::Pie:
      xmlwriter->writeAttribute(QString::fromLatin1("style"), QString::fromLatin1("pie"));
      break;
    case Graph2DCommon::PieStyle::HalfPie:
      xmlwriter->writeAttribute(QString::fromLatin1("style"), QString::fromLatin1("halfpie"));
  }
  xmlwriter->writeAttribute(QString::fromLatin1("marginpercent"), QString::number(marginpercent_));
  xmlwriter->writePen(mPen);
  xmlwriter->writeEndElement();
}

bool Pie2D::load(XmlStreamReader *xmlreader) {
  if (xmlreader->isStartElement() && xmlreader->name() == QString::fromLatin1("pie")) {
    bool ok;

    // style property
    QString style = xmlreader->readAttributeString(QString::fromLatin1("style"), &ok);
    if (ok) {
      if (style == QString::fromLatin1("pie"))
        setstyle_pieplot(Graph2DCommon::PieStyle::Pie);
      else if (style == QString::fromLatin1("halfpie"))
    ...
    int marginpercent = xmlreader->readAttributeInt(QString::fromLatin1("marginpercent"), &ok);
    if (ok)
