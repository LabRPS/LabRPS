...
ErrDialog::ErrDialog(QWidget *parent, AxisRect2D *axisrect, Qt::WindowFlags fl)
    : QDialog(parent, fl),
      axisrect_(axisrect),
      app_(qobject_cast<QMainWindow *>(parent->parent())) {
  Q_ASSERT(axisrect);
  Q_ASSERT(app_);
  setWindowIcon(IconLoader::load(QString::fromLatin1("graph-y-error"), IconLoader::LightDark));
  ...
  columnBox->setText(QString::fromLatin1("Existing column"));
...
void ErrDialog::plotNames() {
  ...
  Q_FOREACH (LineSpecial2D *ls, lslist) {
    ...
    nameLabel->addItem(plotdata.table->name() + QString::fromLatin1("_") + plotdata.ycol->name() +
                       QString::fromLatin1("[") + QString::number(graphdata->getfrom() + 1) + QString::fromLatin1(":") +
  ...
  Q_FOREACH (Curve2D *curve, curvelist) {
    if (curve->getplottype_cplot() == Graph2DCommon::PlotType::Associated) {
      ...
      plotted_columns_ << plotdata;
      nameLabel->addItem(plotdata.table->name() + QString::fromLatin1("_") + plotdata.ycol->name() +
                         QString::fromLatin1("[") + QString::number(curvedata->getfrom() + 1) + QString::fromLatin1(":") +
  ...
  Q_FOREACH (Bar2D *bar, barlist) {
    if (!bar->ishistogram_barplot()) {
      ...
      plotted_columns_ << plotdata;
      nameLabel->addItem(plotdata.table->name() + QString::fromLatin1("_") + plotdata.ycol->name() +
                         QString::fromLatin1("[") + QString::number(bardata->getfrom() + 1) + QString::fromLatin1(":") +
...
void ErrDialog::errorColumnNames() {
  ...
  Q_FOREACH (columnpair, error_columns_) {
    tableNamesBox->addItem(columnpair.first->name() + QString::fromLatin1("_") +
  ...
  if (columnBox->isChecked()) {
    ...
    Q_FOREACH (LineSpecial2D *ls, lslist) {
      ...
      if (selectplotdata.table == graphdata->gettable() &&
        ...
        Column *errors = new Column(QString::fromLatin1("1"), AlphaPlot::Numeric);
...
Column *ErrDialog::getErrorCol(Table *parent) {
  if (tableNamesBox->count() < 1) {
    QMessageBox::critical(this, tr("Error"), tr("No Error Colums available!"));
    return nullptr;
  }
  QStringList tablist = tableNamesBox->currentText().split(QString::fromLatin1("_"));
