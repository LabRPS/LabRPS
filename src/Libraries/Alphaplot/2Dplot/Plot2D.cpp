...
Plot2D::Plot2D(QWidget *parent)
    : QCustomPlot(parent),
      ...
      layernamebackground2d_(QString::fromLatin1("background")),
      layernamegrid2d_(QString::fromLatin1("grid")),
      layernameaxis2d_(QString::fromLatin1("axes")),
      layernamepicker2d_(QString::fromLatin1("picker")),
      layernamelegend2d_(QString::fromLatin1("legend")) {
  setOpenGl(false);
  ...
  if (!removeLayer(layer(QString::fromLatin1("main")))) qDebug() << "unable to delete main layer";
}
...
void Plot2D::setBackgroundColor(const QColor &color, const bool backpixmap) {
  ...
  if (backpixmap) {
    QPixmap pixmap(QString::fromLatin1(":pixmap/transparent-background.png"));
  ...
  Q_EMIT backgroundColorChange(canvasBackground_);
}
...
bool Plot2D::saveSvg(const QString &fileName, int width, int height,
  ...
  generator.setTitle(QString::fromLatin1(svgTitle.toUtf8()));
  generator.setDescription(QString::fromLatin1(svgDescription.toUtf8()));
