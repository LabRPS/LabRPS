...
ImageExportDlg::ImageExportDlg(QWidget *parent, bool vector_options,
  ...
  QStringList filters;
  for (int i = 0; i < list.count(); i++) filters << QString::fromLatin1("*.") + QString::fromLatin1(list[i]).toLower();
...
void ImageExportDlg::initAdvancedOptions() {
  ...
  setPageSize((QPrinter::PageSize)QPrinter::Custom);
  d_standard_page->setChecked(false);
  d_box_page_size->setEnabled(false);
  ...
  d_box_page_orientation->setEnabled(false);
...
void ImageExportDlg::updateAdvancedOptions(const QString &filter) {
  if (filter.contains(QString::fromLatin1("*.svg"))) {
    d_extension_toggle->setChecked(false);
    d_extension_toggle->setEnabled(false);
    return;
  }
  d_extension_toggle->setEnabled(true);
  if (filter.contains(QString::fromLatin1("*.eps")) || filter.contains(QString::fromLatin1("*.ps")) ||
      filter.contains(QString::fromLatin1("*.pdf")))
