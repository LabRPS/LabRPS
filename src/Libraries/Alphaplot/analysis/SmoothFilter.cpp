...
SmoothFilter::SmoothFilter(Gui::MainWindow *parent, AxisRect2D *axisrect,
...
void SmoothFilter::calculateOutputData(double *x, double *y) {
  ...
  switch (static_cast<int>(d_method)) {
    case 1:
      d_explanation = QString::number(d_right_points) +  QString::fromLatin1(" ") + tr("points") +
                       QString::fromLatin1(" ") + tr("Savitzky-Golay smoothing");
      smoothSavGol(x, y);
      break;
    case 2:
      d_explanation = QString::number(d_right_points) +  QString::fromLatin1(" ") + tr("points") +
                       QString::fromLatin1(" ") + tr("FFT smoothing");
      smoothFFT(x, y);
      break;
    case 3:
      d_explanation = QString::number(d_right_points) +  QString::fromLatin1(" ") + tr("points") +
                       QString::fromLatin1(" ") + tr("average smoothing");
      smoothAverage(x, y);
      break;
  }
}
  ...
  if (int error = savitzkyGolayCoefficients(points, d_polynom_order, h)) {
    QMessageBox::critical(app_, tr("AlphaPlot") +  QString::fromLatin1(" - ") + tr("Error"),
                          tr("Internal error in Savitzky-Golay algorithm.\n") +
                              QString::fromLatin1(gsl_strerror(error)));
    gsl_matrix_free(h);
    return;
  }
