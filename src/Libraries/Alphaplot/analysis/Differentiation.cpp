...
Differentiation::Differentiation(Gui::MainWindow *parent,
                                 AxisRect2D *axisrect)
...
void Differentiation::output() {
  ...
  QString curveTitle = associateddata_->table->name() + QString::fromLatin1("_") +
                       associateddata_->xcol->name() + QString::fromLatin1("_") +
                       associateddata_->ycol->name();
  Table *table = app_->newHiddenTable(
      tableName,
      tr("Derivative") + QString::fromLatin1(" ") + tr("of", "Derivative of") + QString::fromLatin1(" ") + curveTitle,
