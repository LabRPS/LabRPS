...
void IntDialog::accept() {
  ...
  if (from == QString::fromLatin1("min")) {
    boxStart->setText(QString::number(minx));
    return;
  } else if (from == QString::fromLatin1("max")) {
  ...
  if (end == QString::fromLatin1("min")) {
    boxEnd->setText(QString::number(minx));
    return;
  } else if (end == QString::fromLatin1("max")) {
    ...
    //    // FunctionCurves!!(Ion)
    //    QMessageBox::warning((Gui::MainWindow *)parent(), tr("Input error"),
...
void IntDialog::activateCurve(const QString &curveName) {
  ...
  boxEnd->setText(
      QString::number(std::max(xmin_, xmax_), 'g', app_->d_decimal_digits));*///koffa the way
}
