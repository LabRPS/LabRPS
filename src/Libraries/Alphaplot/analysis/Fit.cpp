...
Fit::Fit(Gui::MainWindow *parent, AxisRect2D *axisrect, QString name)
  ...
  is_non_linear = true;
  //d_prec = parent->fit_output_precision;//koffa the way
  d_init_err = false;
  chi_2 = -1;
...
QString Fit::logFitInfo(const std::vector<double> &par, int iterations,
  ...
  QString info = QString::fromLatin1("[") + dt.toString(Qt::LocalDate) + QString::fromLatin1("\t") + tr("Plot") + QString::fromLatin1(": ''") +
                 plotName + QString::fromLatin1("'']\n");
  info += d_explanation + QString::fromLatin1(" ") + tr("fit of dataset") +QString::fromLatin1( ": ") +
          associateddata_->table->name() + QString::fromLatin1( "_") + associateddata_->xcol->name() +
          QString::fromLatin1( "_") + associateddata_->ycol->name();
  if (!d_formula.isEmpty())
    info += QString::fromLatin1( ", ") + tr("using function") +QString::fromLatin1( ": ") + d_formula + QString::fromLatin1("\n");
  else
    info += QString::fromLatin1("\n");

  info += tr("Y standard errors") +QString::fromLatin1( ": ");
  switch (d_y_error_source) {
    ...
    case PoissonErrors:
      ...
      break;
    case CustomErrors:
      info += tr("Arbitrary Dataset") +QString::fromLatin1( ": ") + d_y_error_dataset;
      break;
  }
  info += QString::fromLatin1("\n");
  ...
  if (is_non_linear) {
    ...
    info += tr(" algorithm with tolerance = ") +
            QLocale().toString(d_tolerance) + QString::fromLatin1("\n");
  }

  info += tr("From x") + QString::fromLatin1(" = ") + QLocale().toString(d_x[0], 'g', 15) + QString::fromLatin1(" ") +
          tr("to x") + QString::fromLatin1(" = ") + QLocale().toString(d_x[d_n - 1], 'g', 15) + QString::fromLatin1("\n");
  double chi_2_dof = chi_2 / (d_n - d_p);
  for (int i = 0; i < d_p; i++) {
    info += d_param_names[i] + QString::fromLatin1(" ") + d_param_explain[i] + QString::fromLatin1(" = ") +
            QLocale().toString(par[i], 'g', d_prec) + QString::fromLatin1(" +/- ");
    if (d_scale_errors)
  ...
  info +=
      QString::fromLatin1("------------------------------------------------------------------------"
      "--------------\n");
  info += QString::fromLatin1("Chi^2/doF = ") + QLocale().toString(chi_2_dof, 'g', d_prec) + QString::fromLatin1("\n");

  info += tr("R^2") + QString::fromLatin1(" = ") + QLocale().toString(rSquare(), 'g', d_prec) + QString::fromLatin1("\n");
  info +=
      QString::fromLatin1("------------------------------------------------------------------------"
      "---------------\n");
  if (is_non_linear) {
    info += tr("Iterations") + QString::fromLatin1(" = ") + QString::number(iterations) + QString::fromLatin1("\n");
    info += tr("Status") + QString::fromLatin1(" = ") + QString::fromLatin1(gsl_strerror(status)) + QString::fromLatin1("\n");
    info +=
        QString::fromLatin1("----------------------------------------------------------------------"
        "-----------------\n");
  }
...
QString Fit::legendInfo() {
  QString info = tr("Dataset") +QString::fromLatin1( ": ") + associateddata_->table->name() + QString::fromLatin1( "_") +
                 associateddata_->xcol->name() + QString::fromLatin1( "_") +
                 associateddata_->ycol->name() + QString::fromLatin1("\n");
  info += tr("Function") +QString::fromLatin1( ": ") + d_formula + QString::fromLatin1("\n\n");
  ...
  for (int i = 0; i < d_p; i++) {
    info += d_param_names[i] + QString::fromLatin1(" = ") +
            QLocale().toString(d_results[i], 'g', d_prec) + QString::fromLatin1(" +/- ");
    if (d_scale_errors)
      info += QLocale().toString(
                  sqrt(chi_2_dof * gsl_matrix_get(covar, static_cast<size_t>(i),
                                                  static_cast<size_t>(i))),
...
bool Fit::setYErrorSource(ErrorSource err, const QString &colName,
  ...
  //            (Gui::MainWindow *)parent(), tr("Error"),
  ...
  //  */} break;
  //    case PoissonErrors: {
  //      d_y_error_dataset = associateddata_->table->name() + QString::fromLatin1( "_") +
  //                          associateddata_->xcol->name() + QString::fromLatin1( "_") +
...
const std::vector<double> &Fit::errors() {
  ...
  return d_result_errors;*/ //koffa the way
    return nullptr;
}
...
void Fit::fit() {
  ...
  //status = false;
  ///*d_script.reset(
  //    scriptEnv->newScript(d_formula, this, QString::fromLatin1(metaObject()->className())));
  ...
  //QApplication::restoreOverrideCursor(); //koffa the way
}
