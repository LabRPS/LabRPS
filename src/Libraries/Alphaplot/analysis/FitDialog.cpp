...
FitDialog::FitDialog(QWidget *parent, Qt::WindowFlags fl)
    : QDialog(parent, fl), app_(qobject_cast<Gui::MainWindow *>(parent)) {
  Q_ASSERT(app_);
...
void FitDialog::initFitPage() {
  ...
  boxTolerance = new QLineEdit(QString::fromLatin1("1e-4"));
...
void FitDialog::initEditPage() {
  ...
  boxParam = new QLineEdit(QString::fromLatin1("a, b"));
...
void FitDialog::initAdvancedPage() {
 /* generatePointsBtn = new QRadioButton(tr("&Uniform X Function"));
  ...
  connect(generatePointsBtn, SIGNAL(toggled(bool)), this,
          SLOT(showPointsBox(bool)));
  connect(buttonCancel3, SIGNAL(clicked()), this, SLOT(close()));*///koffa the way
}
...
void FitDialog::applyChanges() {
  ...
  app_->saveSettings();
  btnApply->setEnabled(false);*///koffa the way
}
...
void FitDialog::showCovarianceMatrix() {
  ...
  d_fitter->covarianceMatrix(matrixName);*///koffa the way
}
...
void FitDialog::saveUserFunction() {
  if (editBox->toPlainText().isEmpty()) {
    ...
    boxName->setFocus();
    return;
  } else if (boxParam->text().remove(QRegExp(QString::fromLatin1("[,;\\s]"))).isEmpty()) {
    QMessageBox::critical(this, tr("Input function error"),
  ...
  if (d_built_in_function_names.contains(boxName->text())) {
    QMessageBox::critical(
        ...
        QString::fromLatin1("<p><b>") + boxName->text() + QString::fromLatin1("</b>") +
  ...
  QString f = name + QString::fromLatin1("(x, ") + boxParam->text() +
              QString::fromLatin1(")=") + editBox->toPlainText().remove(QString::fromLatin1("\n"));
...
void FitDialog::showFitPage() {
  ...
  QStringList paramList =
      par.split(QRegExp(QString::fromLatin1("[,;]+[\\s]*")), QString::SkipEmptyParts);
  ...
  lblFunction->setText(boxName->text() + QString::fromLatin1(" (x, ") + par
                       + QString::fromLatin1(")"));
...
void FitDialog::setFunction(bool ok) {
  ...
  if (ok) {
    ...
    if (categoryBox->currentRow() == 0 && d_user_function_params.size() > 0)
      ...
      boxParam->setText(lst.join(QString::fromLatin1(", ")));
...
void FitDialog::addUserFunctions(const QStringList &list) {
  ...
  for (int i = 0; i < list.count(); i++) {
    ...
    if (!s.isEmpty()) {
      ...
      int pos1 = s.indexOf(QString::fromLatin1("("), 0);
      d_user_function_names << s.left(pos1);

      int pos2 = s.indexOf(QString::fromLatin1(")"), pos1);
...
void FitDialog::loadPlugins() {
  ...
  QString path = app_->fitPluginsPath + QString::fromLatin1("/");
  QDir dir(path);
  ...
  for (int i = 0; i < lst.count(); i++) {
    ...
    if (name && function && params) {
      d_plugin_function_names << QString::fromLatin1(name());
      d_plugin_functions << QString::fromLatin1(function());
      d_plugin_params << QString::fromLatin1(params());
...
void FitDialog::setBuiltInFunctionNames() {
  d_built_in_function_names << QString::fromLatin1("Boltzmann")
                            << QString::fromLatin1("ExpGrowth")
                            << QString::fromLatin1("ExpDecay1")
                            << QString::fromLatin1("ExpDecay2")
                            << QString::fromLatin1("ExpDecay3")
                            << QString::fromLatin1("GaussAmp")
                            << QString::fromLatin1("Gauss")
                            << QString::fromLatin1("Lorentz")
                            << QString::fromLatin1("Polynomial");
}

void FitDialog::setBuiltInFunctions() {
  d_built_in_functions << QString::fromLatin1("(A1-A2)/(1+exp((x-x0)/dx))+A2");
  d_built_in_functions << QString::fromLatin1("y0+A*exp(x/t)");
  d_built_in_functions << QString::fromLatin1("y0+A*exp(-x/t)");
  d_built_in_functions << QString::fromLatin1("y0+A1*exp(-x/t1)+A2*exp(-x/t2)");
  d_built_in_functions << QString::fromLatin1("y0+A1*exp(-x/t1)+A2*exp(-x/t2)+A3*exp(-x/t3)");
  d_built_in_functions << QString::fromLatin1("y0+A*exp(-(x-xc)*(x-xc)/(2*w*w))");
}
...
void FitDialog::showExpression(int function) {
 ...
 /* if (categoryBox->currentRow() == 2) {
    ...
    if (d_user_functions.size() > function) {
      QStringList l = d_user_functions[function].split(QString::fromLatin1("="));
...
void FitDialog::pasteExpression() {
  ...
  if (categoryBox->currentRow() == 2) {  // basic parser function
    f = f.left(f.indexOf(QString::fromLatin1("("), 0) + 1);
    if (editBox->textCursor().hasSelection()) {
      QString markedText = editBox->textCursor().selectedText();
      editBox->insertPlainText(f + markedText + QString::fromLatin1(")"));
    } else
      editBox->insertPlainText(f + QString::fromLatin1(")"));
  } else
...
void FitDialog::accept() {
 ...
 //       QStringList l = d_user_functions[i].split(QString::fromLatin1("="));
 ...
 //     formula.prepend(QString::fromLatin1("%1=%2\n")
 ...
 //         tableNamesBox->currentText() + QString::fromLatin1("_") + colNamesBox->currentText())) {
 //   delete d_fitter;
 //   d_fitter = nullptr;
 //   return;
 // }
...
void FitDialog::setSrcTables(QList<QMdiSubWindow *> *tables) {
  ...
  d_src_table = tables;
  tableNamesBox->clear();
  Q_FOREACH (QWidget *i, *d_src_table)
    tableNamesBox->addItem(i->objectName());

  tableNamesBox->setCurrentIndex(tableNamesBox->findText(
      boxCurve->currentText().split(QString::fromLatin1("_"), QString::SkipEmptyParts)[0]));
