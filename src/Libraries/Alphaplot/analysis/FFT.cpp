...
FFT::FFT(Gui::MainWindow *parent, Table *table, const QString &realColName,
...
FFT::FFT(Gui::MainWindow *parent, AxisRect2D *axisrect,
...
QList<Column *> FFT::fftCurve() {
  ...
  if (!d_inverse) {
    ...
    d_explanation =
        tr("Forward") + QString::fromLatin1(" ") + tr("FFT") + QString::fromLatin1(" ") + tr("of") + QString::fromLatin1(" ") + label;
    ...
    QString label = associateddata_->table->name() + QString::fromLatin1("_") +
                    ...
                    associateddata_->ycol->name();
    d_explanation =
        tr("Inverse") + QString::fromLatin1(" ") + tr("FFT") + QString::fromLatin1(" ") + tr("of") + QString::fromLatin1(" ") + label;
...
QList<Column *> FFT::fftTable() {
  ...
  if (!amp || !wavetable || !workspace) {
   /* QMessageBox::critical(app_, tr("AlphaPlot") + QString::fromLatin1(" - ") + tr("Error"),
                          tr("Could not allocate memory, operation aborted!"));*///koffa the way
    delete[] amp;
    d_init_err = true;
    return QList<Column *>();
  }
...
void FFT::output(QList<Column *> columns) {
 /* QString tableName = app_->generateUniqueName(objectName());
  ...
  layout->generateCurve2DPlot(AxisRect2D::LineScatterType::Line2D, table,
                              ...
                              0, table->numRows() - 1);*///koffa the way
}
...
void FFT::setDataFromTable(Table *t, const QString &realColName,
  ...
  if (d_real_col < 0) {
    QMessageBox::warning(
        qobject_cast<Gui::MainWindow *>(parent()),
