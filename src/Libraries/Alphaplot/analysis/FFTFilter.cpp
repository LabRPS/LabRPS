...
FFTFilter::FFTFilter(Gui::MainWindow *parent, AxisRect2D *axisrect,
...
void FFTFilter::calculateOutputData(double *x, double *y) {
  ...
  d_explanation = QLocale().toString(d_low_freq) + QString::fromLatin1(" ");
  if (d_filter_type > 2)
    d_explanation += tr("to") + QString::fromLatin1(" ") + QLocale().toString(d_high_freq) + QString::fromLatin1(" ");
  d_explanation += tr("Hz") + QString::fromLatin1(" ");
