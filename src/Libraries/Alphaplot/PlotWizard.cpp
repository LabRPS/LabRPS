...
PlotWizard::PlotWizard(QWidget *parent, Qt::WindowFlags fl)
  ...
  buttonX = new QPushButton(QString::fromLatin1("<->") + tr("&X"));
  buttonX->setAutoDefault(false);
  gl1->addWidget(buttonX, 0, 0);

  buttonXErr = new QPushButton(QString::fromLatin1("<->") + tr("x&Err"));
  buttonXErr->setAutoDefault(false);
  gl1->addWidget(buttonXErr, 0, 1);

  buttonY = new QPushButton(QString::fromLatin1("<->") + tr("&Y"));
  buttonY->setAutoDefault(false);
  gl1->addWidget(buttonY, 1, 0);

  buttonYErr = new QPushButton(QString::fromLatin1("<->") + tr("yE&rr"));
  buttonYErr->setAutoDefault(false);
  gl1->addWidget(buttonYErr, 1, 1);

  buttonZ = new QPushButton(QString::fromLatin1("<->") + tr("&Z"));
...
void PlotWizard::accept() {
  ...
  for (int i = 0; i < plotAssociations->count(); i++) {
    ...
    if (text.endsWith(QString::fromLatin1("(X)"))) {
      QMessageBox::critical(
          this, tr("Error"),
          tr("Please define a Y column for the following curve") + QString::fromLatin1(":\n\n") +
              text);
      return;
    }

    if (text.contains(QString::fromLatin1("(Z)"))) {
      if (text.contains(QString::fromLatin1("(Y)")) && !curves3D.contains(text))
        curves3D << text;
      else if (!text.contains(QString::fromLatin1("(Y)")) && !ribbons.contains(text))
        ribbons << text;
    } else if (text.contains(QString::fromLatin1("(xErr)")) || text.contains(QString::fromLatin1("(yErr)"))) {
      QStringList lst = text.split(QString::fromLatin1(","), QString::SkipEmptyParts);
      lst.pop_back();
      QString master_curve = lst.join(QString::fromLatin1(","));
  ...
  if (curves.count() > 0) Q_EMIT plot(curves);
...
void PlotWizard::changeColumnsList(const QString &table) {
  ...
  for (int i = 0; i < columns.count(); i++) {
    QString s = columns[i];
      if (s.contains(table)) newList << s.remove(table + QString::fromLatin1("_"));
  }
...
void PlotWizard::addXCol() {
  ...
  if (text.contains(QString::fromLatin1("(X)")))
    QMessageBox::warning(this, tr("Error"),
                         tr("You have already defined a X column!"));
  else {
    plotAssociations->currentItem()->setText(
        text + columnsList->currentItem()->text() + QString::fromLatin1("(X)"));
  }
}
...
void PlotWizard::addYCol() {
  ...
  else if (text.contains(QString::fromLatin1("(Y)")))
    QMessageBox::warning(this, tr("Error"),
                         tr("You have already defined a Y column!"));
  else {
    plotAssociations->currentItem()->setText(
        text + QString::fromLatin1(", ") + columnsList->currentItem()->text() + QString::fromLatin1("(Y)"));
  }
}
...
void PlotWizard::addZCol() {
  ...
  QString text = plotAssociations->currentItem()->text();
  if (text.contains(QString::fromLatin1("(xErr)")) || text.contains(QString::fromLatin1("(yErr)")))
    QMessageBox::warning(this, tr("Error"),
                         tr("This kind of curve is not handled by AlphaPlot!"));
  else if (!text.contains(QString::fromLatin1("(X)")))
    QMessageBox::warning(this, tr("Error"),
                         tr("You must define a X column first!"));
  else if (text.contains(QString::fromLatin1("(Z)")))
    QMessageBox::warning(this, tr("Error"),
                         tr("You have already defined a Z column!"));
  else {
    plotAssociations->currentItem()->setText(
        text + QString::fromLatin1(", ") + columnsList->currentItem()->text() + QString::fromLatin1("(Z)"));
  }
}
...
void PlotWizard::addXErrCol() {
  ...
  if (text.contains(QString::fromLatin1("(Z)")))
    ...
    plotAssociations->currentItem()->setText(
        text + QString::fromLatin1(", ") + columnsList->currentItem()->text() + QString::fromLatin1("(xErr)"));
  }
}
...
void PlotWizard::addYErrCol() {
  ...
  if (text.contains(QString::fromLatin1("(Z)")))
    ...
    plotAssociations->currentItem()->setText(
        text + QString::fromLatin1(", ") + columnsList->currentItem()->text() + QString::fromLatin1("(yErr)"));
  }
}

void PlotWizard::addCurve() {
  plotAssociations->addItem(boxTables->currentText() + QString::fromLatin1(": "));
