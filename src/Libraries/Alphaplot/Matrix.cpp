...
Matrix::Matrix(int r, int c, const QString& label, QWidget* parent, const char* name,
               Qt::WindowFlags f, ScriptingEnv* env)
    : MatrixView(label, parent, QString::fromLatin1(name), f), scripted(env)
{
    d_future_matrix = new Gui::MatrixViewProvider(0, r, c, label);
    init(r, c);
}

Matrix::Matrix(Gui::MatrixViewProvider* future_matrix, int r, int c, const QString& label,
...
void Matrix::init(int rows, int cols) {
  ...
  ui.add_cell_combobox->addItem(QString::fromLatin1("cell(i, j)"));
...
void Matrix::handleChange() { Q_EMIT modifiedWindow(this); }
...
void Matrix::setText(int row, int col, const QString &new_text) {
  ...
  if (ok)
    ...
    Script *script = scriptEnv->newScript(
        new_text, this, QString::fromLatin1("<%1_%2_%3>").arg(name()).arg(row).arg(col));
...
void Matrix::setNumericFormat(const QChar &f, int prec) {
  if (QChar::fromLatin1(d_future_matrix->numericFormat()) == f &&
...
bool Matrix::recalculate() {
  QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
  Script *script =
      scriptEnv->newScript(formula(), this, QString::fromLatin1("<%1>").arg(name()));
  connect(script, SIGNAL(error(const QString &, const QString &, int)),
          scriptEnv, SIGNAL(error(const QString &, const QString &, int)));
...
Matrix *Matrix::fromImage(const QImage &image) {
  Gui::MatrixViewProvider *fm = Gui::MatrixViewProvider::fromImage(image);
  if (!fm) return nullptr;
  return new Matrix(fm, image.height(), image.width(),
                    tr("Matrix %1").arg(1));
}
