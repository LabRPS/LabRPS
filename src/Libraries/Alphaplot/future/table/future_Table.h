...
*/
 ...
 class AlphaplotExport TableViewProvider: public AbstractPart
{
  Q_OBJECT
 ...
 public:
  ...
  TableViewProvider(void* engine, int rows, int columns, const QString& name);
  void setView(TableView* view);
  friend class ::TableStatistics;
  virtual ~TableViewProvider();
  ...
  void copy(TableViewProvider* other);
...
/**
  ...
  automatically.
  */
class AlphaplotExport TableViewProvider::Private
{
 public:
  Private(TableViewProvider* owner)
      : d_owner(owner),
        d_column_count(0),
        d_row_count(0),
        d_current_column{-1} {}
