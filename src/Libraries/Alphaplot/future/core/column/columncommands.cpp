...
void ColumnFullCopyCmd::redo() {
  if (d_backup == 0) {
    d_backup_owner = new Column(QString::fromLatin1("temp"), d_src->columnMode());
...
void ColumnPartialCopyCmd::redo() {
  if (d_src_backup == 0) {
    ...
    // columns
    d_src_backup_owner = new Column(QString::fromLatin1("temp"), d_col->columnMode());
    d_src_backup = new Column::Private(d_src_backup_owner, d_col->columnMode());
    d_src_backup->copy(d_src, d_src_start, 0, d_num_rows);
    d_col_backup_owner = new Column(QString::fromLatin1("temp"), d_col->columnMode());
...
void ColumnRemoveRowsCmd::redo() {
  if (d_backup == 0) {
    ...
    d_backup_owner = new Column(QString::fromLatin1("temp"), d_col->columnMode());
