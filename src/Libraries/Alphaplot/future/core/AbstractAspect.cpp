...
#include "future/core/AspectPrivate.h"
#include "future/core/aspectcommands.h"
...
void AbstractAspect::writeCommentElement(QXmlStreamWriter *writer) const {
  writer->writeStartElement(QString::fromLatin1("comment"));
  QString temp = comment();
  temp.replace(QString::fromLatin1("\n"), QString::fromLatin1("\\n"));
  writer->writeCDATA(temp);
  writer->writeEndElement();
}

bool AbstractAspect::readCommentElement(XmlStreamReader *reader) {
  Q_ASSERT(reader->isStartElement() && reader->name() == QString::fromLatin1("comment"));
...
void AbstractAspect::writeBasicAttributes(QXmlStreamWriter *writer) const {
  writer->writeAttribute(QString::fromLatin1("creation_time"),
                         creationTime().toString(QString::fromLatin1("yyyy-dd-MM hh:mm:ss:zzz")));
  writer->writeAttribute(QString::fromLatin1("caption_spec"), captionSpec());
...
bool AbstractAspect::readBasicAttributes(XmlStreamReader *reader) {
  ...
  // read creation time
  str = attribs.value(reader->namespaceUri().toString(), QString::fromLatin1("creation_time"))
            .toString();
  ...
  // read caption spec
  str = attribs.value(reader->namespaceUri().toString(), QString::fromLatin1("caption_spec"))
            .toString();
  setCaptionSpec(str);
  // read name
  str = attribs.value(reader->namespaceUri().toString(), QString::fromLatin1("name")).toString();
  if (str.isEmpty()) {
...
void AbstractAspect::setName(const QString &value) {
  if (value.isEmpty()) {
    setObjectName(QString::fromLatin1("1"));
    return;
  }
  ...
  sanitized_value.remove(QChar::fromLatin1('\n'));
  sanitized_value.remove(QChar::fromLatin1('\r'));
  sanitized_value.remove(QChar::fromLatin1('\t'));
...
Gui::FolderViewProvider *AbstractAspect::folder() {
  if (inherits("Gui::FolderViewProvider")) return static_cast<Gui::FolderViewProvider *>(this);
  AbstractAspect *parent_aspect = parentAspect();
  while (parent_aspect && !parent_aspect->inherits("Gui::FolderViewProvider"))
    parent_aspect = parent_aspect->parentAspect();
  return static_cast<Gui::FolderViewProvider *>(parent_aspect);
}
...
QVariant AbstractAspect::global(const QString &key) {
  QString qualified_key = QString::fromLatin1(staticMetaObject.className()) + QString::fromLatin1("/") + key;
...
void AbstractAspect::setGlobal(const QString &key, const QVariant &value) {
  Private::g_settings->setValue(
      QString::fromLatin1(staticMetaObject.className()) + QString::fromLatin1("/") + key, value);
}

void AbstractAspect::setGlobalDefault(const QString &key,
                                      const QVariant &value) {
  Private::g_defaults[QString::fromLatin1(staticMetaObject.className()) + QString::fromLatin1("/") + key] =
      value;
}
