...
AbstractSimpleFilter::AbstractSimpleFilter()
    : AbstractFilter(QString::fromLatin1("SimpleFilter")),
...
void AbstractSimpleFilter::inputRowsAboutToBeInserted(
  ...
  Q_UNUSED(source);
  Q_UNUSED(count);
  Q_FOREACH (Interval<int> output_range,
           dependentRows(Interval<int>(before, before)))
    Q_EMIT d_output_column->rowsAboutToBeInserted(
...
void AbstractSimpleFilter::inputRowsAboutToBeRemoved(
  ...
  Q_FOREACH (Interval<int> output_range,
    ...
    Q_EMIT d_output_column->rowsAboutToBeRemoved(
        d_output_column, output_range.start(), output_range.size());
}
...
void AbstractSimpleFilter::save(QXmlStreamWriter *writer) const {
  writer->writeStartElement(QString::fromLatin1("simple_filter"));
  writeBasicAttributes(writer);
  writeExtraAttributes(writer);
  writer->writeAttribute(QString::fromLatin1("filter_name"), QString::fromLatin1(metaObject()->className()));
  writeCommentElement(writer);
  writer->writeEndElement();
}

bool AbstractSimpleFilter::load(XmlStreamReader *reader) {
  if (reader->isStartElement() && reader->name() == QString::fromLatin1("simple_filter")) {
    if (!readBasicAttributes(reader)) return false;

    QXmlStreamAttributes attribs = reader->attributes();
    QString str =
        attribs.value(reader->namespaceUri().toString(), QString::fromLatin1("filter_name"))
            .toString();
    if (str != QString::fromLatin1(metaObject()->className())) {
