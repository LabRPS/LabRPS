...
class AlphaplotExport String2DoubleFilter: public AbstractSimpleFilter
{
  Q_OBJECT
 ...
 private:
  ...
  bool isAnyDecimalSeparatorAllowed() const {
    ...
    return settings.value(QString::fromLatin1("LocaleUseGroupSeparator")).toBool();
  }
  ...
  bool convertToDouble(const QString &str, double &value, const QLocale &locale,
    ...
    if (accept_any_decimal_separator) {
      ...
      if (QChar::fromLatin1('.') == decimalSeparator) foreignSeparator = QChar::fromLatin1(',');
      if (QChar::fromLatin1(',') == decimalSeparator) foreignSeparator = QChar::fromLatin1('.');
