...
class AlphaplotExport String2DayOfWeekFilter: public AbstractSimpleFilter
{
  Q_OBJECT
 ...
 public:
  ...
  virtual QDateTime dateTimeAt(int row) const {
    ...
    if (!ok) {
      QDate temp = QDate::fromString(input_value, QString::fromLatin1("ddd"));
        if (!temp.isValid()) temp = QDate::fromString(input_value, QString::fromLatin1("dddd"));
      if (!temp.isValid())
        return QDateTime();
      else
