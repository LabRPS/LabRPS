...
class AlphaplotExport String2MonthFilter: public AbstractSimpleFilter
{
  Q_OBJECT
 ...
 public:
  ...
  virtual QDateTime dateTimeAt(int row) const {
    ...
    if (!ok) {
      QDate temp = QDate::fromString(input_value, QString::fromLatin1("MMM"));
        if (!temp.isValid()) temp = QDate::fromString(input_value, QString::fromLatin1("MMMM"));
      if (!temp.isValid())
        return QDateTime();
      else
