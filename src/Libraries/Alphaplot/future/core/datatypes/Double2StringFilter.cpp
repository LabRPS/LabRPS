...
void Double2StringFilter::writeExtraAttributes(QXmlStreamWriter* writer) const {
  writer->writeAttribute(QString::fromLatin1("format"), QString(QChar::fromLatin1(numericFormat())));
  writer->writeAttribute(QString::fromLatin1("digits"), QString::number(numDigits()));
}
...
bool Double2StringFilter::load(XmlStreamReader* reader) {
  ...
  QString digits_str =
      attribs.value(reader->namespaceUri().toString(), QString::fromLatin1("digits")).toString();
