...
class AlphaplotExport MatrixViewProvider : public AbstractPart/*,
               public scripted*/
#else
class AlphaplotExport MatrixViewProvider : public AbstractPart
#endif
 ...
 */
  ...
  //       const QString &name);
#else
  MatrixViewProvider(void *, int rows, int cols, const QString &name);
  ...
  void copy(MatrixViewProvider* other);
  double xStart() const;
  double yStart() const;
  double xEnd() const;
  double yEnd() const;
 ...
 public:
  static MatrixViewProvider* fromImage(const QImage &image);
...
/**
  ...
  */
class AlphaplotExport MatrixViewProvider::Private {
 public:
  Private(MatrixViewProvider* owner);
   ...
   * Q_EMIT dataChanged(0, 0, rowCount()-1, columnCount()-1);
   * </code>
   */
