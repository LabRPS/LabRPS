 ...
 public:
  ...
  QList<Interval<T> > operator-(QList<Interval<T> > subtrahend) {
    ...
    *tmp1 << *static_cast<Interval<T>*>(this);
    Q_FOREACH (Interval<T> i, subtrahend) {
      tmp2 = new QList<Interval<T> >();
      Q_FOREACH (Interval<T> j, *tmp1)
        *tmp2 << subtract(j, i);
      delete tmp1;
      tmp1 = tmp2;
    }
  ...
  //QString rpstoString() const {
  //  return QString::fromLatin1("[") + QString::number(d_start) + QString::fromLatin1(",") + QString::number(d_end) + QString::fromLatin1("]");
  //}
