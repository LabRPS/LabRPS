...
std::function<std::vector<std::vector<double>>(int, double)> hp_butterworth() {
  return [](int filter_order,
    ...
    //       "\nERROR: in signal_processing::hp_butterworth: Error in buffer
    //       size " "calculations\n");
    // }

    // // Divide by 2 to make cutoff frequency match the definition given in
    // MATLAB Ipp8u* internal_calcs = ippsMalloc_8u(internal_buffer_size);
    // status =
    //     ippsIIRGenHighpass_64f(cutoff_freq / 2.0, 0, filter_order,
    //     taps.data(),
    ...
    // if (status != ippStsNoErr) {
    //   throw std::runtime_error(
    //       "\nERROR: in signal_processing::hp_butterworth: Error in
    ...
    if ((filter_order == 4) && (cutoff_freq == 0.004)) {
      ...
      throw std::runtime_error(
          "We only support filter_order==4 and cutoff_freq==0.004. Other "
          "setups require manual computation of butterworth highpass filter "
          "parameters by changing the source code");
    }

    std::vector<double> numerator{0.983715174129757, -3.934860696519026,
                                  5.902291044778539, -3.934860696519026,
                                  0.983715174129757};
    std::vector<double> denominator{1.000000000000000, -3.967162595948848,
                                    5.902025861490879, -3.902558784823240,
                                    0.967695543813137};
...
std::function<std::vector<double>(std::vector<double>, std::vector<double>, int,
  ...
  return [](const std::vector<double>& numerator_coeffs,
    ...
    Eigen::VectorXd b = Eigen::Map<const Eigen::VectorXd>(
        numerator_coeffs.data(), numerator_coeffs.size());
    Eigen::VectorXd a = Eigen::Map<const Eigen::VectorXd>(
        denominator_coeffs.data(), denominator_coeffs.size());

    if (a[0] == 0) {
      throw std::runtime_error(
          "Denominator's first coefficient (a[0]) cannot be zero");
    }
...
std::function<std::vector<double>(double, double, unsigned int, unsigned int)>
  ...
  return [](double freq_corner, double time_step, unsigned int order,
    ...
    for (unsigned int i = 0; i < freq_steps.size(); ++i) {
      filter(i) = std::sqrt(
          1.0 / (1.0 + std::pow(freq_corner / freq_steps[i], 2.0 * order)));
    }
