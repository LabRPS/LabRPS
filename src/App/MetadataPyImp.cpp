...
Py::Object MetadataPy::getLabRPSMin() const
{
    return Py::String(getMetadataPtr()->labrpsmin().str());
}

void MetadataPy::setLabRPSMin(Py::Object args)
{
    ...
    getMetadataPtr()->setLabRPSMin(App::Meta::Version(version));
}

Py::Object MetadataPy::getLabRPSMax() const
{
    return Py::String(getMetadataPtr()->labrpsmax().str());
}

void MetadataPy::setLabRPSMax(Py::Object args)
{
    char* version = nullptr;
    PyObject* p = args.ptr();
    if (!PyArg_ParseTuple(p, "s", &version))
    ...
    getMetadataPtr()->setLabRPSMax(App::Meta::Version(version));
}

PyObject* MetadataPy::getFirstSupportedLabRPSVersion(PyObject* p)
{
    if (!PyArg_ParseTuple(p, ""))
        return nullptr;

    // Short-circuit: if the toplevel sets a version, then the lower-levels are overridden
    if (getMetadataPtr()->labrpsmin() != App::Meta::Version())
        return Py::new_reference_to(Py::String(getMetadataPtr()->labrpsmin().str()));

    auto content = getMetadataPtr()->content();
    auto result = App::Meta::Version();
    for (const auto& item : content) {
        auto minVersion = item.second.labrpsmin();
...
PyObject* MetadataPy::getLastSupportedLabRPSVersion(PyObject* p)
{
    if (!PyArg_ParseTuple(p, ""))
        return nullptr;

    // Short-circuit: if the toplevel sets a version, then the lower-levels are overridden
    if (getMetadataPtr()->labrpsmax() != App::Meta::Version())
        return Py::new_reference_to(Py::String(getMetadataPtr()->labrpsmax().str()));

    auto content = getMetadataPtr()->content();
    auto result = App::Meta::Version();
    for (const auto& item : content) {
        auto maxVersion = item.second.labrpsmax();
...
PyObject* MetadataPy::supportsCurrentLabRPS(PyObject* p)
{
    if (!PyArg_ParseTuple(p, ""))
        return nullptr;

    bool result = getMetadataPtr()->supportsCurrentLabRPS();
