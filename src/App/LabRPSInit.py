...
def removeFromPath(module_name):
    """removes the module from the sys.path. The entry point for imports
        will therefore always be LabRPS.
        eg.: from LabRPS.Module.submodule import function"""
    import sys
    paths = sys.path
    for path in paths:
...
def setupSearchPaths(PathExtension):
    ...
    if sys.platform == 'win32' and hasattr(os, "add_dll_directory"):
        if "LABRPS_LIBPACK_BIN" in os.environ:
            os.add_dll_directory(os.environ["LABRPS_LIBPACK_BIN"])
...
LabRPS._importFromLabRPS = removeFromPath


def InitApplications():
    # Checking on LabRPS module path ++++++++++++++++++++++++++++++++++++++++++
    ModDir = LabRPS.getHomePath()+'Mod'
    ModDir = os.path.realpath(ModDir)
    ExtDir = LabRPS.getHomePath()+'Ext'
    ExtDir = os.path.realpath(ExtDir)
    BinDir = LabRPS.getHomePath()+'bin'
    BinDir = os.path.realpath(BinDir)
    libpaths = []
    LibDir = LabRPS.getHomePath()+'lib'
    LibDir = os.path.realpath(LibDir)
    if os.path.exists(LibDir):
        libpaths.append(LibDir)
    Lib64Dir = LabRPS.getHomePath()+'lib64'
    Lib64Dir = os.path.realpath(Lib64Dir)
    if os.path.exists(Lib64Dir):
        libpaths.append(Lib64Dir)
    LibPyDir = LabRPS.getHomePath()+'lib-py3'
    LibPyDir = os.path.realpath(LibPyDir)
    if (os.path.exists(LibPyDir)):
        libpaths.append(LibPyDir)
    LibFcDir = LabRPS.getLibraryDir()
    LibFcDir = os.path.realpath(LibFcDir)
    if (os.path.exists(LibFcDir) and not LibFcDir in libpaths):
        libpaths.append(LibFcDir)
    AddPath = LabRPS.ConfigGet("AdditionalModulePaths").split(";")
    HomeMod = LabRPS.getUserAppDataDir()+"Mod"
    HomeMod = os.path.realpath(HomeMod)
    MacroStd = App.getUserMacroDir(False)
    MacroDir = App.getUserMacroDir(True)
    MacroMod = os.path.realpath(MacroDir+"/Mod")
    SystemWideMacroDir = LabRPS.getHomePath()+'Macro'
    SystemWideMacroDir = os.path.realpath(SystemWideMacroDir)

    PluginStd = App.getUserPluginDir(False)
    PluginDir = App.getUserPluginDir(True)
    PluginMod = os.path.realpath(PluginDir+"/Plugin")
    SystemWidePluginDir = LabRPS.getHomePath()+'Plugin'
    SystemWidePluginDir = os.path.realpath(SystemWidePluginDir)

    #print LabRPS.getHomePath()
    if os.path.isdir(LabRPS.getHomePath()+'src\\Tools'):
        sys.path.append(LabRPS.getHomePath()+'src\\Tools')
    ...
    elif os.path.isdir(os.path.join(os.path.expanduser("~"),".LabRPS","Mod")):
        # Check if old location exists
        Wrn ("User path has changed to " + LabRPS.getUserAppDataDir() + ". Please move user modules and macros\n")
    ...
    # to have all the module-paths available in LabRPSGuiInit.py:
    LabRPS.__ModDirs__ = list(ModDict.values())

    # this allows importing with:
    # from LabRPS.Module import package
    LabRPS.__path__ = [ModDir] + libpaths + [HomeMod]
    ...
    # this path:
    additional_packages_path = os.path.join(LabRPS.getUserAppDataDir(),"AdditionalPythonPackages")
        ...
        if ((Dir != '') & (Dir != 'CVS') & (Dir != '__init__.py')):
            ...
            if os.path.exists(MetadataFile):
                ...
                if not meta.supportsCurrentLabRPS():
                    Msg(f'NOTICE: {meta.Name} does not support this version of LabRPS, so is being skipped\n')
                    continue
                content = meta.Content
                if "workbench" in content:
                    workbenches = content["workbench"]
                    for workbench in workbenches:
                        if not workbench.supportsCurrentLabRPS():
                            Msg(f'NOTICE: {meta.Name} content item {workbench.Name} does not support this version of LabRPS, so is being skipped\n')
                            continue
        ...
        for _, labrps_module_name, labrps_module_ispkg in pkgutil.iter_modules(labrps.__path__, "labrps."):
            if labrps_module_ispkg:
                ...
                try:
                    ...
                    if os.path.exists(stopFile):
                        Msg(f'NOTICE: Addon "{labrps_module_name}" disabled by presence of ADDON_DISABLED stopfile\n')
                        continue
                    ...
                    if os.path.exists(MetadataFile):
                        meta = LabRPS.Metadata(MetadataFile)
                        if not meta.supportsCurrentLabRPS():
                            Msg(f'NOTICE: Addon "{labrps_module_name}" does not support this version of LabRPS, so is being skipped\n')
                            continue

                    labrps_module = importlib.import_module(labrps_module_name)
                    extension_modules += [labrps_module_name]
                    if any (module_name == 'init' for _, module_name, ispkg in pkgutil.iter_modules(labrps_module.__path__)):
                        importlib.import_module(labrps_module_name + '.init')
                        Log('Init: Initializing ' + labrps_module_name + '... done\n')
                    else:
                        Log('Init: No init module found in ' + labrps_module_name + ', skipping\n')
                except Exception as inst:
    ...
    # add PluginDir to path (RFE #0000504)
    sys.path.append(PluginStd)
    sys.path.append(PluginDir)
    # add SystemWidePluginDir to path
    sys.path.append(SystemWidePluginDir)

    # add special path for MacOSX (bug #0000307)
    import platform
    if len(platform.mac_ver()[0]) > 0:
        sys.path.append(os.path.expanduser('~/Library/Application Support/LabRPS/Mod'))

# some often used shortcuts (for lazy people like me  ;-)
App = LabRPS
Log = LabRPS.Console.PrintLog
Msg = LabRPS.Console.PrintMessage
Err = LabRPS.Console.PrintError
Wrn = LabRPS.Console.PrintWarning
...
Log ('Init: starting App::LabRPSInit.py\n')
...
class FCADLogger(object):
    '''Convenient class for tagged logging.
       ...
       To set/get logger level:
           >>> LabRPS.setLogLevel('MyModule','Trace')
           >>> LabRPS.getLogLevel('MyModule')
           4
        ...
        LabRPS.setLogLevel() supports both text and integer value, which allows
    ...
    def __init__(self, tag, **kargs):
        ...
        * tag: a string tag for this logger. The log level of this logger can be
               accessed using LabRPS.getLogLevel(tag)/setLogLevel(tag,level).
        ...
        * title ('LabRPS'): message box title used by report()
        '''
        self.tag = tag
    ...
    def _isEnabledFor(self,level):
        ...
        if self.parent and not self.parent._isEnabledFor(level):
            return False
        return LabRPS.getLogLevel(self.tag) >= level
    ...
    def _log(self,level,msg,frame=0,args=(),kargs=None):
        ...
        if not self.noUpdateUI and LabRPS.GuiUp:
            import LabRPSGui
            try:
                LabRPSGui.updateGui()
            except Exception:
    ...
    def report(self,msg,func,*args,**kargs):
        ...
        try:
            ...
            if LabRPS.GuiUp:
                import LabRPSGui,PySide
                PySide.QtGui.QMessageBox.critical(
                        LabRPSGui.getMainWindow(),self.title,str(e))

LabRPS.Logger = FCADLogger
...
LabRPS.addImportType("LabRPS document (*.FCStd)","LabRPS")
...
App.Units.RadianPerSecond= App.Units.Quantity('rad/s')
...
Log ('Init: App::LabRPSInit.py done\n')
