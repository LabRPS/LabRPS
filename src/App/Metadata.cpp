...
Metadata::Metadata(const fs::path& metadataFile)
{
    ...
    case 1:
        ...
        break;
    default:
        throw Base::XMLBaseException("package.xml format version is not supported by this version of LabRPS");
    }
}
...
Meta::Version App::Metadata::labrpsmin() const
{
    return _labrpsmin;
}

Meta::Version App::Metadata::labrpsmax() const
{
    return _labrpsmax;
}
...
void App::Metadata::setLabRPSMin(const Meta::Version& version)
{
    _labrpsmin = version;
}

void App::Metadata::setLabRPSMax(const Meta::Version& version)
{
    _labrpsmax = version;
}
...
bool Metadata::satisfies(const Meta::Dependency& dep)
{
    ...
    // LabRPS Expression. If it evaluates to false, this dependency is bypassed (e.g. this function returns false).
...
bool App::Metadata::supportsCurrentLabRPS() const
{
    ...
    if (_labrpsmin != Meta::Version() && _labrpsmin > fcVersion)
        return false;
    else if (_labrpsmax != Meta::Version() && _labrpsmax < fcVersion)
        return false;
    return true;
}
...
void Metadata::appendToElement(DOMElement* root) const
{
    ...
    if (_labrpsmin != Meta::Version())
        appendSimpleXMLNode(root, "labrpsmin", _labrpsmin.str());

    if (_labrpsmax != Meta::Version())
        appendSimpleXMLNode(root, "labrpsmax", _labrpsmin.str());
...
void Metadata::parseVersion1(const DOMNode* startNode)
{
    ...
    for (XMLSize_t i = 0; i < children->getLength(); ++i) {
        ...
        else if (tagString == "labrpsmin")
            _labrpsmin = Meta::Version(StrXUTF8(element->getTextContent()).str);
        else if (tagString == "labrpsmax")
            _labrpsmax = Meta::Version(StrXUTF8(element->getTextContent()).str);
