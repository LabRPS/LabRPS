...
Py::Object DocumentObjectPy::getViewObject(void) const
{
    try {
        ...
        // check if the LabRPSGui module is already loaded, if not then don't try to load it
        Py::Dict sysmod(dict);
        if (!sysmod.hasKey("LabRPSGui")) {
            return Py::None();
        }

        // double-check that the module doesn't have a null pointer
        Py::Module module(PyImport_ImportModule("LabRPSGui"),true);
        ...
        // LabRPSGui is loaded, so there must be wrong something else
        throw; // re-throw
    }
}
...
PyObject*  DocumentObjectPy::evalExpression(PyObject *self, PyObject * args)
{
    ...
    // For LabRPS-specific types the behaviour is a bit different:
