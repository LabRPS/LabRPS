...
App::DocumentObjectExecReturn *LinkBaseExtension::extensionExecute() {
    ...
    if(getLinkedObjectProperty()) {
        ...
        if(!linked) {
            ...
            ss << "Link broken!";
            auto xlink = Base::labrps_dynamic_cast<PropertyXLink>(
                    ...
                    || !container->getDocument()->getObjectByID(_LinkOwner.getValue())))
        {
            ...
            proxy = Base::labrps_dynamic_cast<PropertyPythonObject>(
        ...
        App::DocumentObject *src)
{
    ...
    for (auto it=res.begin(); it!=res.end();) {
        ...
        auto prop = Base::labrps_dynamic_cast<PropertyMap>(
...
void LinkBaseExtension::syncCopyOnChange()
{
    ...
    if (auto prop = getLinkCopyOnChangeGroupProperty()) {
        copyOnChangeGroup = Base::labrps_dynamic_cast<LinkGroup>(prop->getValue());
        if (!copyOnChangeGroup) {
            ...
            for (auto obj : objs) {
                if (!obj->getNameInDocument())
                    continue;
                auto prop = Base::labrps_dynamic_cast<PropertyUUID>(
    ...
    for (const auto &objT : oldObjs) {
        auto prop = Base::labrps_dynamic_cast<PropertyUUID>(objT.getProperty());
        if (!prop)
            continue;
...
void LinkBaseExtension::syncElementList() {
    ...
    for(size_t i=0;i<elements.size();++i) {
        auto element = labrps_dynamic_cast<LinkElement>(elements[i]);
        if(!element
    ...
    const char *subname, const std::vector<std::string> &subElements)
{
    ...
    auto xlink = labrps_dynamic_cast<PropertyXLink>(linkProp);
    if(obj) {
        if(!obj->getNameInDocument())
