...
ObjectIdentifier ObjectIdentifier::parse(const DocumentObject *docObj, const std::string &str)
{
    ...
    VariableExpression * v = labrps_dynamic_cast<VariableExpression>(expr.get());
...
#define GET_MODULE(_name) do {\
    ...
    auto setPropDep = [deps](DocumentObject *obj, Property *prop, const char *propName) {
        ...
        if(prop && prop->getContainer()!=obj) {
            auto linkTouched = Base::labrps_dynamic_cast<PropertyBool>(
                    obj->getPropertyByName("_LinkTouched"));
            if(linkTouched) 
                propName = linkTouched->getName();
            else {
                auto propOwner = Base::labrps_dynamic_cast<DocumentObject>(prop->getContainer());
                if(propOwner) 
                    obj = propOwner;
                else 
                    propName = nullptr;
