...
Expression *OperatorExpression::simplify() const
{
    ...
    if (labrps_dynamic_cast<NumberExpression>(v1) && labrps_dynamic_cast<NumberExpression>(v2)) {
        delete v1;
        delete v2;
        return eval();
    }
...
void OperatorExpression::_toString(std::ostream &s, bool persistent,int) const
{
    ...
    needsParens = false;
    if (labrps_dynamic_cast<OperatorExpression>(left))
    ...
    needsParens = false;
    if (labrps_dynamic_cast<OperatorExpression>(right))
    ...
    for (auto &arg : args) {
        if (arg->isDerivedFrom(RangeExpression::getClassTypeId())) {
            ...
            do {
                ...
                if ((qp = labrps_dynamic_cast<PropertyQuantity>(p)) != nullptr)
                    c->collect(qp->getQuantityValue());
                else if ((fp = labrps_dynamic_cast<PropertyFloat>(p)) != nullptr)
                    c->collect(Quantity(fp->getValue()));
                else if ((ip = labrps_dynamic_cast<PropertyInteger>(p)) != nullptr)
...
Expression *FunctionExpression::simplify() const
{
    ...
    for (auto it = args.begin(); it != args.end(); ++it) {
        ...
        if (labrps_dynamic_cast<NumberExpression>(v))
            ++numerics;
        a.push_back(v);
    }
...
void VariableExpression::addComponent(Component *c) {
    do {
        ...
        long l1=0,l2=0,l3=1;
        if(c->e3) {
            auto n3 = labrps_dynamic_cast<NumberExpression>(c->e3);
            if(!n3 || !essentiallyEqual(n3->getValue(),(double)l3))
                break;
        }
        if(c->e1) {
            auto n1 = labrps_dynamic_cast<NumberExpression>(c->e1);
            if(!n1) {
                if(c->e2 || c->e3)
                    break;
                auto s = labrps_dynamic_cast<StringExpression>(c->e1);
                if(!s)
                    break;
        ...
        auto n2 = labrps_dynamic_cast<NumberExpression>(c->e2);
...
Expression *ConditionalExpression::simplify() const
{
    ...
    NumberExpression * v = labrps_dynamic_cast<NumberExpression>(e.get());
...
UnitExpression * ExpressionParser::parseUnit(const App::DocumentObject *owner, const char* buffer)
{
    ...
    if (!unitExpression) {
        OperatorExpression * fraction = labrps_dynamic_cast<OperatorExpression>(ScanResult);

        if (fraction && fraction->getOperator() == OperatorExpression::DIV) {
            NumberExpression * nom = labrps_dynamic_cast<NumberExpression>(fraction->getLeft());
            UnitExpression * denom = labrps_dynamic_cast<UnitExpression>(fraction->getRight());
    ...
    if (unitExpression) {
        NumberExpression * num = labrps_dynamic_cast<NumberExpression>(simplified);

        if (num) {
           simplified = new UnitExpression(num->getOwner(), num->getQuantity());
            delete num;
        }
        return labrps_dynamic_cast<UnitExpression>(simplified);
    }
    else {
        delete simplified;
