...
PyDoc_STRVAR(LabRPS_doc,
     "The functions in the LabRPS module allow working with documents.\n"
     "The LabRPS instance provides a list of references of documents which\n"
...
// during initialization, to make the built-in __LabRPSBase__
// module known to Python.
PyMODINIT_FUNC
init_labrps_base_module(void)
{
    static struct PyModuleDef BaseModuleDef = {
        PyModuleDef_HEAD_INIT,
        "__LabRPSBase__", Base_doc, -1,
...
PyMODINIT_FUNC
init_labrps_module(void)
{
    static struct PyModuleDef LabRPSModuleDef = {
        PyModuleDef_HEAD_INIT,
        "LabRPS", LabRPS_doc, -1,
        __AppMethods,
        nullptr, nullptr, nullptr, nullptr
    };
    return PyModule_Create(&LabRPSModuleDef);
}
...
void Application::setupPythonTypes()
{
    ...
    PyObject* pAppModule = PyImport_ImportModule ("LabRPS");
    if (!pAppModule) {
        PyErr_Clear();
        pAppModule = init_labrps_module();
        PyDict_SetItemString(modules, "LabRPS", pAppModule);
    }
    Py::Module(pAppModule).setAttr(std::string("ActiveDocument"),Py::None());

    static struct PyModuleDef ConsoleModuleDef = {
        PyModuleDef_HEAD_INIT,
        "__LabRPSConsole__", Console_doc, -1,
    ...
    // remove these types from the LabRPS module.

    PyObject* pBaseModule = PyImport_ImportModule ("__LabRPSBase__");
    if (!pBaseModule) {
        PyErr_Clear();
        pBaseModule = init_labrps_base_module();
        PyDict_SetItemString(modules, "__LabRPSBase__", pBaseModule);
    }
...
Document* Application::newDocument(const char * Name, const char * UserName, bool createView, bool tempDoc)
{
    ...
    // make sure that the active document is set in case no GUI is up
    {
        ...
        Py::Module("LabRPS").setAttr(std::string("ActiveDocument"),active);
    }
...
void Application::setActiveDocument(Document* pDoc)
{
    ...
    if (pDoc) {
        ...
        Base::PyGILStateLocker lock;
        Py::Module("LabRPS").setAttr(std::string("ActiveDocument"),Py::None());
    }
...
std::string Application::getUserPluginDir()
{
    return mConfig["UserPluginPath"];
}

int Application::getUserPluginSource(std::string& customPluginSourcePlath)
{
    ParameterGrp::handle hGrp = App::GetApplication().GetParameterGroupByPath("User parameter:BaseApp/Preferences/Plugin");
    if (hGrp->GetBool("localSource", false))
        return 1;
    else if (hGrp->GetBool("onlineSource", false))
        return 2;
    else if (hGrp->GetBool("localAndOnlineSources", false))
        return 3;
    else if (hGrp->GetBool("customSource", false))
    {
...
void Application::addImportType(const char* Type, const char* ModuleName)
{
    ...
    // Due to branding stuff replace "LabRPS" with the branded application name
    if (strncmp(Type, "LabRPS", 7) == 0) {
...
void Application::cleanupUnits()
{
    try {
        Base::PyGILStateLocker lock;
        Py::Module mod (Py::Module("LabRPS").getAttr("Units").ptr());
 ...
 */
#ifdef _MSC_VER // New handler for Microsoft Visual C++ compiler
int __cdecl labrpsNewHandler(size_t size )
{
    // throw an exception
...
void Application::init(int argc, char ** argv)
{
    try {
        ...
        _set_new_handler ( labrpsNewHandler ); // Setup new handler
        _set_new_mode( 1 ); // Re-route malloc failures to new handler !
#else   // Ansi compiler
        std::set_new_handler (labrpsNewHandler); // ANSI new handler
#endif
...
void parseProgramOptions(int ac, char ** av, const string& exe, variables_map& vm)
{
    ...
    if (vm.count("help")) {
        std::stringstream str;
        str << exe << endl << endl;
        str << "For a detailed description see https://www.wiki.labrps.com/Start_up_and_Configuration" << endl<<endl;
...
void processProgramOptions(const variables_map& vm, std::map<std::string,std::string>& mConfig)
{
    ...
    if (vm.count("run-test")) {
        ...
        mConfig["ScriptFileName"] = "LabRPSTest";
        //sScriptName = LabRPSTest;
    }
...
#   endif
    ...
    // init python
    PyImport_AppendInittab ("LabRPS", init_labrps_module);
    PyImport_AppendInittab ("__LabRPSBase__", init_labrps_base_module);
    ...
    if (!(mConfig["RunMode"] == "Cmd")) {
        // Remove banner if LabRPS is invoked via the -c command as regular
        // Python interpreter
        if (!(mConfig["Verbose"] == "Strict"))
...
void Application::initApplication(void)
{
    ...
    new ScriptProducer( "LabRPSInit",    LabRPSInit    );
    new ScriptProducer( "LabRPSTest",    LabRPSTest    );
    ...
    try {
        ...
        Interpreter().runString(Base::ScriptFactory().ProduceScript("LabRPSInit"));
    }
    catch (const Base::Exception& e) {
        e.ReportException();
    }
...
void Application::runApplication()
{
    ...
    if (mConfig["RunMode"] == "Cmd") {
        ...
        Interpreter().runCommandLine("LabRPS Console mode");
    }
    ...
    try {
        if (_pcSysParamMngr->LoadOrCreateDocument() && !(mConfig["Verbose"] == "Strict")) {
            ...
            if (!Py_IsInitialized()) {
                ...
                Console().Message("   This warning normally means that LabRPS is running for the first time\n"
                                  "   or the configuration was deleted or moved. LabRPS is generating the standard\n"
                                  "   configuration.\n");
            }
        }
    }
...
std::tuple<QString, QString, QString> getCustomPaths()
{
    ...
    QString userHome = env.value(QString::fromLatin1("LABRPS_USER_HOME"));
    QString userData = env.value(QString::fromLatin1("LABRPS_USER_DATA"));
    QString userTemp = env.value(QString::fromLatin1("LABRPS_USER_TEMP"));
    ...
    // if LABRPS_USER_HOME is set but not LABRPS_USER_DATA
    if (!userHome.isEmpty() && userData.isEmpty()) {
        userData = userHome;
    }

    // if LABRPS_USER_HOME is set but not LABRPS_USER_TEMP
...
void Application::ExtractUserPath()
{
    ...
    // this is to support a portable version of LabRPS
    ...
    //
    std::vector<std::string> plugindirs = subdirs;
    plugindirs.emplace_back("Plugin");
    boost::filesystem::path plugin = findPath(dataHome, customData, plugindirs, true);
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    // import the LabRPS.so module from a running Python session. In the latter case the
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    // * import the LabRPS.pyd module from a running Python session. In this case the
    //   Python interpreter is already initialized.
    // * use a custom dll that links LabRPS core dlls and that is loaded by its host application
    //   In this case the calling name should be set to LabRPSBase.dll or LabRPSApp.dll in order
    ...
    // fixes #0001638 to avoid to load DLLs from Windows' system directories before LabRPS's bin folder
