...
{
    DocumentObject * docObj = labrps_dynamic_cast<DocumentObject>(getContainer());
...
DocumentObjectExecReturn *App::PropertyExpressionEngine::execute(ExecuteOption option, bool *touched)
{
    ...
    for (;it != evaluationOrder.end();++it) {
        ...
        DocumentObject* parent = labrps_dynamic_cast<DocumentObject>(prop->getContainer());
...
{
    DocumentObject * owner = labrps_dynamic_cast<DocumentObject>(getContainer());
