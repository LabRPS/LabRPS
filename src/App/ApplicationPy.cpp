...
PyMethodDef Application::Methods[] = {
    ...
    {"getUserPluginDir", (PyCFunction) Application::sGetUserPluginPath, METH_VARARGS,
     "getUserPluginDir(bool=False) -> string"
     "Get the directory of the user's plugin directory\n"
     "If parameter is False (the default) it returns the standard path in the"
     "user's home directory, otherwise it returns the user-defined path."},
...
//  {"saveDocumentAs", (PyCFunction) Application::sSaveDocumentAs, METH_VARARGS},
    ...
    {"checkAbort", (PyCFunction) Application::sCheckAbort, METH_VARARGS,
     ...
     "trigger a Base.LabRPSAbort exception."},
...
PyObject* Application::sGetUserPluginPath(PyObject * /*self*/, PyObject *args)
{
    PyObject *actual = Py_False;
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &actual))
        return nullptr;

    std::string pluginDir = Application::getUserPluginDir();
    if (PyObject_IsTrue(actual)) {
        pluginDir = App::GetApplication().
            GetParameterGroupByPath("User parameter:BaseApp/Preferences/Plugin")
            ->GetASCII("PluginPath",pluginDir.c_str());
    }

    Py::String user_plugin_dir(pluginDir,"utf-8");
    return Py::new_reference_to(user_plugin_dir);
}
