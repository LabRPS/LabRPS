...
\ingroup APP
\brief The Base class of the LabRPS Document

This (besides the App::Application class) is the most important class in LabRPS.
It contains all the data of the opened, saved, or newly created LabRPS Document.
The App::Document manages the Undo and Redo mechanism and the linking of documents.

\namespace App \class App::Document
This is besides the Application class the most important class in LabRPS
It contains all the data of the opened, saved or newly created LabRPS Document.
...
As the document is the main data structure of LabRPS we have to take a close
...
The LabRPS document handles the dependencies of its DocumentObjects with
...
Document::importObjects(Base::XMLReader& reader)
{
    ...
    for(auto o : objs) {
        if(o && o->getNameInDocument()) {
            ...
            {
                auto propSource = Base::labrps_dynamic_cast<PropertyUUID>(
...
bool Document::saveToFile(const char* filename) const
{
    ...
    // open extra scope to close ZipWriter properly
    {
        ...
        writer.setComment("LabRPS Document");
...
bool Document::afterRestore(const std::vector<DocumentObject *> &objArray, bool checkPartial)
{
    ...
    for (auto obj : objs) {
        ...
        if(obj->isValid()) {
            ...
            obj->getPropertyList(props);
            for(auto prop : props) {
                auto link = Base::labrps_dynamic_cast<PropertyLinkBase>(prop);
                int res;
                std::string errMsg;
...
void Document::updateObject(DocumentObject* pcObject)
{
    if (!pcObject->getDocument()) {
        throw Base::RuntimeError("Document object is not yet added to a document");
    }

    // insert in the name map
    d->objectMap[pcObject->Label.getStrValue()] = pcObject;

    // need a mechanism to update the same object in this array as well.
    //d->objectArray.push_back(pcObject);

    // mark the object as new (i.e. set status bit 2) and send the signal
    pcObject->setStatus(ObjectStatus::Touch, true);
...
Document::importLinks(const std::vector<App::DocumentObject*> &objArray)
{
    ...
    for(auto obj : links) {
        ...
        for(auto prop : propList) {
            auto linkProp = Base::labrps_dynamic_cast<PropertyLinkBase>(prop);
