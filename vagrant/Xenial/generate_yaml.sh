#!/bin/bash
# snapcraft yaml automatic generation tool for LabRPS 0.17
# (c) 2017 vejmarie 
# Distributed under LGPL v2.0
# Please report bugs at vejmarie@ruggedpod.qyshare.com

rm snapcraft.yaml
lib_list=`ls /opt/local/LabRPS-0.17/lib/*.so`
mod_list=`ls -ltd $(find /opt/local/LabRPS-0.17/Mod) | awk '{ print $9}'`
bin_list=`ls -ltd $(find /opt/local/LabRPS-0.17/bin) | awk '{ print $9}'`
data_list=`ls -ltd $(find /opt/local/LabRPS-0.17/data) | awk '{ print $9}'`
...
description: development version This version is far from being bug free but integrates the latest LabRPS technologies
parts:
 example-part:
    plugin: copy
    files:
      bin/launcher : bin/launcher" >> snapcraft.yaml
#      bin/LabRPS : opt/local/LabRPS-0.17/bin/LabRPS" >> snapcraft.yaml
for i in $bin_list
do
        if [ ! -d "$i" ]
        then
...
echo "
    snap:
     - bin/launcher
     - opt/local/LabRPS-0.17/bin/LabRPS
     - opt
     - usr" >> snapcraft.yaml
for i in $lib_list
do
